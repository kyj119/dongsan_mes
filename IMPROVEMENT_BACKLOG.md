...
> - 227개 마이그레이션 전수 분석 + 100개 라우트 파일 교차 검증
> - entity_id 컬럼 있으나 인덱스 누락 8개 테이블 → A-009 자동 수정 (0228 migration)
>   - adjustments, payment_requests, purchase_payments, inventory_transactions (0150에서 컬럼 추가 시 인덱스 미생성)
>   - hometax_jobs, portal_access_logs (0193에서 컬럼 추가 시 인덱스 미생성)
>   - journal_entries (0220 테이블 생성 시 미생성), budgets (0219 테이블 생성 시 미생성)
> - fixed_assets + depreciation_records entity_id 컬럼 자체 누락 → I-025 등록 (MEDIUM, 1~2h)
> - returns/return_items/purchase_invoices/waste_records entity_id 누락 → I-026 등록 (MEDIUM, 3~4h)
> - 자동 수정 1건 (A-009: 인덱스 8개), 신규 이슈 2건 (I-025/I-026)
> - ⚠️ GitHub 이슈 생성 실패 (토큰 만료) — 이슈 본문은 백로그에 기록
>
...
| ID | 제목 | 영역 | Issue | 공수 |
|----|------|------|-------|------|
| I-025 | fixed_assets + depreciation_records entity_id 컬럼 누락 (재무 자산) | Area 4 | — | 1~2h |
| I-026 | returns/purchase_invoices/waste_records entity_id 컬럼 누락 (5개 테이블) | Area 4 | — | 3~4h |
...
| ID | 제목 | 커밋 | 날짜 |
|----|------|------|------|
| A-009 | entity_id 인덱스 누락 8개 테이블 (adjustments/payment_requests/purchase_payments/inventory_tx/hometax_jobs/portal_access_logs/journal_entries/budgets) | 0228 migration | 2026-05-18 |
