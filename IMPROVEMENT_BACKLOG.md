...
> - 12개 테이블 entity_id 인덱스 누락 → A-009 자동 수정 (0231 migration): inventory_transactions/journal_entries/adjustments/purchase_invoices/returns/fixed_assets/waste_records/budgets/employees/hometax_jobs/payment_requests/purchase_payments
> - cards/lifecycle.ts syncOrderStatusFromCards 별도 호출 → order/card 상태 불일치 위험 → I-025 등록 (MEDIUM, 1~2h)
> - printSystem.ts media_material_groups DELETE+INSERT 원자성 없음 → I-026 등록 (MEDIUM, 30m)
> - shipments.ts CANCELLED 시 shipment_items 잔존 → 집계 오염 가능 → I-027 등록 (SMALL, 30m)
> - GitHub MCP 토큰 만료로 이슈 GitHub 등록 불가 — IMPROVEMENT_BACKLOG.md에만 기록
> - 자동 수정 1건 (A-009), 신규 이슈 3건 (I-025~I-027)
>
...
| I-025 | cards/lifecycle.ts syncOrderStatusFromCards 별도 호출 — order/card 상태 불일치 위험 | Area 4 | — | 1~2h |
| I-026 | printSystem.ts media_material_groups DELETE+INSERT 원자성 없음 (db.batch 미사용) | Area 4 | — | 30m |
| I-027 | shipments.ts CANCELLED 후 shipment_items 잔존 — 집계 쿼리 오염 가능 | Area 4 | — | 30m |
...
| ID | 제목 | 커밋 | 날짜 |
|----|------|------|------|
| A-009 | entity_id 인덱스 12개 일괄 추가 (0231 migration) — inventory_transactions 등 핵심 테이블 | (커밋 예정) | 2026-05-19 |
...
| ID | 제목 | 커밋/Issue | 날짜 |
|----|------|-----------|------|
| A-009 | entity_id 인덱스 12개 일괄 추가 (0231 migration) | — | 2026-05-19 |
