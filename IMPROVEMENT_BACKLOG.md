...
> - 마이그레이션 전체(0001~0207) + 핵심 라우트 파일 분석
> - 주문 하드삭제 5개 순차 DELETE → db.batch() 원자화 자동 수정 (A-009) — 중간 실패 시 부분 삭제 방지
> - 대시보드 strftime() 표현식 쿼리 인덱스 미사용 → I-025 등록 (SMALL, 1h)
> - approval_requests MANAGER 전체 조회 entity 격리 미적용 → I-026 등록 (SMALL, 1~2h)
> - 기존 인덱스(delivery_date, billing_status, entity_status 복합) 적절 확인 ✓
> - labor_contracts entity_id 포함 확인 ✓, tasks entity_id 불필요 패턴(order_id/card_id 경유) 확인 ✓
> - 자동 수정 1건 (A-009), 신규 이슈 2건 (I-025/I-026)
>
...
| I-025 | 대시보드 strftime() 월별 쿼리 인덱스 미사용 (풀스캔) | Area 4 | — | 1h |
| I-026 | approval_requests MANAGER 전체 조회 — entity 격리 미적용 | Area 4 | — | 1~2h |
...
| ID | 제목 | 커밋 | 날짜 |
|----|------|------|------|
| A-009 | 주문 하드삭제 5개 순차 DELETE → db.batch() 원자화 (중간 실패 부분삭제 방지) | 15f19c9 | 2026-05-17 |
...
| ID | 제목 | 커밋/Issue | 날짜 |
|----|------|-----------|------|
| A-009 | 주문 하드삭제 db.batch() 원자화 (orders/core.ts 5개 순차 DELETE) | 15f19c9 | 2026-05-17 |
