...
> - 세금계산서 취소 시 orders.billing_status 미초기화 → 매출 중복 계상 → #59 등록 (HIGH)
> - quotations.entity_id NOT NULL 제약 누락 → 멀티사업자 격리 위반 가능 → #60 등록 (MEDIUM)
> - purchase_requests.supplier_id 인덱스 누락 → A-009 자동 수정 (0206 migration)
> - 거짓 양성 제거: purchase_request_items ON DELETE CASCADE 이미 존재, quotations DELETE는 soft-delete(취소), shipment_items 인덱스는 0094 migration에 존재
> - 자동 수정 1건 (A-009), 신규 이슈 2건 (#59/#60)
>
...
| I-025 | 세금계산서 취소 시 billing_status 미초기화 — 매출 중복 계상 | Area 4 | #59 | 1~2h |
| I-026 | quotations.entity_id NOT NULL 누락 — 멀티사업자 격리 위반 가능 | Area 4 | #60 | 1h |
...
| ID | 제목 | 커밋 | 날짜 |
|----|------|------|------|
| A-009 | purchase_requests.supplier_id 인덱스 추가 (0206 migration) | — | 2026-05-15 |
