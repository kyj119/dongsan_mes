...
> - 마이그레이션 0001~0230 전수 분석: FK 관계, entity_id 커버리지, 인덱스 효율 점검
> - entity_id 인덱스 6개 누락 발견: fixed_assets/returns/waste_records/journal_entries/purchase_invoices/budgets → A-009 자동 수정 (0231 migration)
> - orders 하드삭제 배치에서 `tax_invoice_orders` 미정리 버그 발견 → A-010 자동 수정 (취소된 세금계산서의 junction 레코드 고아화 방지)
> - depreciation_records entity_id 누락 — 교차 사업자 감가상각 조회 가능 → #131 등록 (SMALL, 30m)
> - clients 소프트삭제 전용(is_active), purchase_orders.supplier_id 고아 위험 실질적 없음 확인
> - 자동 수정 2건 (A-009/A-010), 신규 이슈 1건 (#131)
>
...
| I-025 | depreciation_records entity_id 누락 — 교차 사업자 감가상각 조회 | Area 4 | #131 | 30분 |
...
| ID | 제목 | 커밋 | 날짜 |
|----|------|------|------|
| A-010 | orders 하드삭제 배치 tax_invoice_orders 고아 레코드 버그 수정 | f1be813 | 2026-05-19 |
| A-009 | entity_id 인덱스 6개 추가 (fixed_assets/returns/waste_records/journal_entries/purchase_invoices/budgets) | f1be813 | 2026-05-19 |
