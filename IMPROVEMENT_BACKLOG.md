...
> - 신규 테이블 6개 entity_id 인덱스 누락 → A-009 자동 수정 (0227 migration): returns/purchase_invoices/waste_records/fixed_assets/budgets/journal_entries
> - returns.ts RESOLVED+RESTOCK 경로 balance_after NOT NULL 누락 → 런타임 실패 → #111 등록 (BUG, SMALL)
> - journal_entries 결제 자동분개 중복 방지 누락 → 동일 payment_id 이중분개 가능 → #112 등록 (BUG, SMALL)
> - 자동 수정 1건 (A-009), 신규 이슈 2건 (#111/#112)
>
...
| I-025 | returns.ts RESOLVED+RESTOCK balance_after NOT NULL 누락 → 런타임 실패 | Area 4 | #111 | 1~2h |
| I-026 | journal_entries 결제 자동분개 중복 방지 누락 (payment_id 이중분개) | Area 4 | #112 | 30분 |
...
| ID | 제목 | 커밋 | 날짜 |
|----|------|------|------|
| A-009 | entity_id 인덱스 누락 6테이블 (returns/purchase_invoices/waste_records/fixed_assets/budgets/journal_entries) | d9decc9 | 2026-05-18 |
...
| ID | 제목 | 커밋/Issue | 날짜 |
|----|------|-----------|------|
| A-009 | entity_id 인덱스 6테이블 일괄 추가 (0227 migration) | d9decc9 | 2026-05-18 |
