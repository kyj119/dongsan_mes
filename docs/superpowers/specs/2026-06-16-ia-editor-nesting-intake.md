...
- **상태**: 🔴 **방향 재정의 (2026-06-17, 용준님)** — 구현된 Konva 검출보정 에디터가 의도와 어긋나 폐기. **객체 기반 자유 대지 편집기 + 2개 입력경로(대지편집/직접연결), 직접연결 우선 착수**. 상세 = **§14 (정본)**. 아래 §0~§13은 이전 설계(부분 무효 — §14 우선)
...
## 14. 방향 전면 재정의 (2026-06-17, 용준님 브레인스토밍) — **정본**

> 구현된 Konva 검수 에디터(전체렌더 배경 + 그룹박스 드래그/리사이즈 = 검출영역 보정)가 용준님 의도와 어긋남.
> 원하시는 것 = "검출 보정 ❌, **일러스트레이터식 자유 대지에서 추출 그룹을 객체로 다루며 출력 처리(마감/후가공/여백/돔보)를 핫키로 적용하고 결과를 눈으로 보는** 편집기".

### 14.1 핵심 재정의
| 항목 | 재정의 |
|------|--------|
| 편집기 형태 | **검출보정 폐기** → 객체 기반 **자유 대지 + 시트 오버레이** (여러 파일 그룹 통합) |
| 그룹 | 추출 그대로 객체로. **검출 영역 재조정 기능 제거** |
| 대지 작업 | 배치 · 크기/비율 · 마감 · 후가공 · 여백 · 돔보 (품목 미지정) |
| 미리보기 | **웹 벡터 근사만**(즉시·핫키). 실제 IA 렌더는 주문 생성 시점에만 |
| 돔보 | **단일 그룹에도 추가**(ProcessOrderItem.jsx 신규 — 현재는 시트 둘레만, 단일 출력엔 없음) |
| 품목(PM-xxx) | 대지 미지정 → **주문 추가 단계**에서 지정 |
| 라인 | 그룹별 **개별 라인** 기본 / 네스팅 시트 = 1라인(공통품목·수량=조각수) |
| 네스팅 | 대지 내부 시트 오버레이 **우선**, 어려우면 탭 분리→대지로 가져오기 |
| 출력/저장 | **주문 생성 시점**에 기존 주문확정→`AI_PROCESS`→`Z:\DESIGN\…\[주문번호]\` 통합 저장. 별도 개별저장 ❌. **독립 P5 렌더잡 폐기·흡수** |
| 조작 | **핫키 중심**(숫자=마감프리셋, T=돔보, R=회전, D=복제, N=시트담기, Ctrl+S=저장, Ctrl+Enter=라인추가 — 구현 시 확정) |

### 14.2 2개 입력 경로 (종착점 동일 = `order_items` + 주문폴더 EPS)
```
경로 1 (대지 편집기)            경로 2 (직접 연결, ia-editor 우회) ⭐우선
고객 AI 업로드                  완성 EPS/AI를 주문서에 바로 첨부
  ↓ ExtractGroups 자동분석         ↓ (그룹추출/대지 생략)
  ↓ 자유 대지: 그룹=객체           ↓
  ↓ 배치·크기·마감·돔보(웹근사)    ↓
  ↓ 네스팅(대지내/탭)             ↓
  └──→ 주문 추가(품목지정) ←──────┘
            ↓  order_items → AI_PROCESS → 주문폴더 EPS (+썸네일)
```
- **경로 2 우선 착수 결정** (작은 작업·백엔드 대부분 존재·즉시 효용)
- D-a 완성 EPS 그대로 → 가공 없이 주문폴더 복사 / D-b AI 파일 + 수동라인 → 전체문서(-1) 가공
- **공통: 썸네일 추출 → 주문관리/주문카드 활용** (직접연결도 썸네일 필수 — 그룹추출 생략 시 `cards.thumbnail_url` 소스 공백 방지)

### 14.3 직접 연결(D) — 코드 조사 결과 (2026-06-17)
- 백엔드/에이전트 **대부분 존재**. 빠진 것 = ① 프론트 UI ② 단일 완성EPS passthrough ③ 썸네일 연결.
  - `ai_group_index=-1`(전체문서) 가공 = `ProcessOrderItem` 첫 아트보드 (Program.cs:2314~) **동작**
  - 완성 EPS 복사 = 현재 `ai_group_index=-2`+`layout_id`(`ai_layout_requests` 묶음 결과) 전용 → **단일 완성 EPS는 신규 passthrough 분기 필요**(`CopyLayoutFilesToZDriveAsync`는 layout 결과만 복사)
  - 업로드 시 분석 강제 트리거(`/api/ai-analysis/upload`) → 직접연결은 `/api/files/upload`(분석 없음) 사용
  - 파일 경로: 업로드 `r2://sources/{id}/{name}`, 에이전트 Program.cs:910~ 로컬/r2/청크 3단 fallback
- **작업**: ① 주문폼 "직접 연결" 모드 UI(파일첨부 분석우회 + 수동 라인) + API ② 에이전트 passthrough 복사 + EPS/AI→PNG 썸네일 렌더 ③ 썸네일을 `cards.thumbnail_url`/주문 연결 콜백
- **분담**: Web(주문폼 UI+API+썸네일연결) = 에이전트 무관 즉시 / C#(passthrough+썸네일렌더) = 에이전트 재빌드·배포 필요(현재 PID 34840 실행 중 → 중지 후 교체)

### 14.4 Phase 재계획 (대형 — 세션 분리)
| Phase | 내용 | 비고 |
|-------|------|------|
| **D** (우선) | 직접 연결 — 완성EPS 복사 + AI/수동라인 + 썸네일 | Web 먼저, C# 배포 |
| N1 | 자유 대지 캔버스(그룹=객체, 실제크기 비율, 드래그/리사이즈/회전, 핫키 골격) | 신규 핵심 |
| N2 | 처리설정 인스펙터 + 마감·여백·돔보 **벡터 근사** + 핫키 | |
| N3 | 네스팅(대지내 시트 오버레이 우선/탭 폴백, `shelfBinPack` 재활용, 동일품목 가드) | |
| N4 | 주문 연결(품목 지정 모달, 개별/네스팅 라인, 새·기존 주문, 면적단가) | |
| N5 | 단일그룹 **돔보**(`ProcessOrderItem.jsx` + 웹근사) | IA 배포 |

### 14.5 폐기 / 유지
- **폐기**: `iaEditor.js:290-446`(Konva 검출보정 캔버스 + crop/보정 localStorage) · P5 독립 렌더잡(주문흐름 흡수, 폴링 메커니즘만 참고)
- **유지·재활용**: 추출 데이터층(groups_json·canvas_json·좌표) · `MARGIN_RULES`/finishing_methods/presets · 주문확정→AI_PROCESS→주문폴더(완전 동작) · `shelfBinPack` · order_items 스키마(컬럼 충분, finishing2/3 마이그만 확인)
