...
- **작성일**: 2026-06-11
- **상태**: ✅ **검토 완료·옵션 A PoC 승인 (2026-06-11 용준님)** — PoC 범위 = §6(비임계 파일 1개 env.ASSETS 실측, 무중단). 우선순위 낮음(기능 작업 뒤). 성공 시 단계적 외부화(shell.js는 P4 폴백 확보 후 최종), 실패 시 옵션 B 확정 + dead 파일 정리
- **대체**: `static-assets-migration.md`(2026-06-10, P0 파일럿 → prod 2회 다운 → 롤백, 문서 삭제됨 — git 히스토리 `docs/archive/static-assets-migration.md` 참조). 그 문서의 P0~P3 접근은 **무효**.
