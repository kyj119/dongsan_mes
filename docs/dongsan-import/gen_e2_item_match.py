...
ALIAS_PFX = {
    ...
    # ── 상반기 미연결에서 추가 (2026-08-08). 7월엔 안 나왔던 표기다.
    '솔벤현수막TPM': 'SVB',                    # 솔벤 현수막 원단(TPM 은 브랜드) — 폭은 R1 이 붙인다
    '솔벤현수막TPMS150': 'SVB',
    'PVC켈그레이30M호홍': 'KELG',               # 켈그레이 30M 호홍
    'PVC켈그레이30M잉크테크': 'KELG-IT',         # ★잉크테크는 브랜드가 코드에 있다(호홍과 구분)
    '무광코팅지자동엠보61M120G': 'HJ-MGAUTOB-P',  # MES 는 「보급형」이 품명에 붙어 있다(61m/120g 일치)
    '무광코팅지자동용45M120G호홍': 'MCP120',
    '깃발천S': 'GITBAL',                       # 깃발천 폭별
    '미러천깃발천': 'MIRROR',                   # 미러천 = 깃발천 계열 변종
    'LM5400보급형': 'LM5400',                  # 조명시트 LM5400
    'LM54000보급형': 'LM5400',                 # ★자릿수 오타(54000)도 같은 물건 — 원본 표기를 그대로 받는다
    '솔벤용코팅지50M': 'SVCOAT',
    '솔벤텐트천300D': 'SVT',                    # 300D 는 원사 굵기 표기 — 폭이 코드를 가른다
    '비조명후렉스': 'FLEXN',
    '조명후렉스': 'FLEXL',
    '울트라칼라시트내부용': 'ULTRA-CAL',
    '울트라보조시트': 'ULTRA-BOJO',
    '합성지프라임하이코': 'PJ-HAP-P',            # 하이코=프라임 브랜드. 코드가 `-P<폭>` 형식이다
    '합성지그레이프라임하이코': 'PJ-HAPGR-P',
    '현수막2코팅J': 'AQ2',                      # -J 도 2코팅. 접미는 공급사 표기일 뿐 폭이 코드를 가른다
    '현수막1코팅J': 'AQ1',
    '양면코팅지50M': 'HJ-YMSIDE50-P',           # 기존 `HJ-YMSIDE-P<폭>` 는 30m — 길이가 단가를 가른다
}
# 폭이 없는 품목 — 이름 → 코드 직접
ALIAS_EXACT = {
    ...
    '자이언트폴SET대': 'ACC-023',               # 자이언트배너 폴세트
    '자이언트폴SET': 'ACC-023',
    '자이언트배너출력': 'TRGI-PO',               # 전사 자이언트배너(폰지 기본)
    'LED투광기검정색백색': 'SGM-LEDP-BW',
    '판형삼각접착고리24호': 'ACC-050-P12',        # 규격 12Ø = 12파이
    '배너양면용깃대': 'ACC-034',                 # 깃대(기타)
    '자동몰드': 'ACC-MOLD-9',
    '하도매자동몰드': 'ACC-MOLD-24',
    '원터치배너그레이': 'GDS-OTB-GR',
