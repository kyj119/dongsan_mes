...
WHERE item_code IN (
  ...
  -- 목창꽂이(ACC-054·ACC-054-NS)는 전 라인이 수량×단가와 일치해 실단가가 남는다 → 제외 안 함.
  ...
  -- 남은 껍데기 21곳 정리분 (2026-08-10)
  'ACC-042',        -- 국기봉/막봉 — ★영광 40개×5,000원 과 조양산업 40,000개×130원 이 **같은 품목에 섞였다**.
                    --   가중평균이 135원으로 눌려 5,000원짜리를 완전히 왜곡한다. 규격 분리 전까지 0.
  'BUJIK-090',      -- 부직포 90폭 — 부림엔에프 3건 전부 수량 표기 없음(금액 전용)
  'PKG-BOX-500350300', -- 포장박스 500*350*300 — 적요 수량(148)이 금액(462,000÷1,500=308)과 안 맞아 금액 전용
  -- 미연결 매입 연결분 (2026-08-24 오후, `_bak_0824_unlink15`). 라인이 전부 수량 1(구매건당 뭉침)이라
  -- SUM(amount)/SUM(quantity) 가 「건당 평균」이 된다 — 갈바 판 수·각목 다발 수 불명. 실수량 확보 전까지 0.
  -- 같은 날 연결한 SGM-SMPS·MAG-060(실단가)·ACC-014(회전고리 88,280개×66원 실수량)는 정상이라 제외 안 함.
  'SGM-GALVA',      -- 갈바(자재) — 진보스텐·케이텍 8건 전부 수량 1, 금액 2만~40만(판 수 불명)
  'WDS-01',         -- 각목 — 건설목재·영림 3건 수량 1, 금액 5.8만~60만(다발 뭉침)
  'ACC-056'         -- 태극기 끈 — 준석(AD) 1건 4,765,184 수량 표기 없음(금액 전용)
);
