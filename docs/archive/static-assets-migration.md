...
> ⛔ **무효/대체됨 (2026-06-11)**: 이 문서의 P0~P3 접근(`/static` 외부화 + `_routes.json` 제외 + `_headers`)은 prod를 2회 다운시켜 롤백됨(커밋 `24bb493c`). 근본원인 분석 + 강건 재설계는 **`docs/superpowers/specs/2026-06-11-static-assets-rootcause-redesign.md`** 참조. ⚖️ⓑ "외부화 재시도 금지" 적용 중.
>
> 목적(원안): `?raw`로 워커에 인라인되는 클라 JS(2.6MB)를 정적 에셋으로 빼서 **브라우저/엣지 캐싱·콜드스타트 개선·대역폭 절감**.
> 작성: 2026-06-10 · 상태: ⛔ 무효(2026-06-11 롤백)
