...
## prod 실데이터 대사 (2026-09-27, 읽기 전용)
오늘 고친 코드 결함이 prod 에 남긴 흔적은 **0건**: 청구액≠주문액 0 · 단가미정 청구 0 · 계산서 걸린 미청구 0 · 취소주문+계산서 0 ·
계산서≠청구그룹 0 · 중복 계산서 0 · 발주 끝전 0 · 재발주 헤더 0 · 불합격 0 · 지급↔발주 법인 0 · 통장↔입금 금액 0 · 끊긴 링크 0.
(주문서 화면에서 태어난 주문이 거의 없어 경로를 밟은 적이 없었다 — 병행 전환 후엔 매일 밟는다.)

실제 이격은 **이관·통장 데이터** 쪽이다.
| # | 이격 | 규모 | 판단 |
|---|---|---|---|
| D1 | 같은 거래처·날짜·금액 입금이 두 법인에 이중(동산 이카운트 이관 「· 선명」 ↔ 선명 통장 이관, 선명 수금→동산 이체를 동산 통장이 또 수금 처리) | 18건 1.47억 (인효 6건·오케이애드공사 등) | 미수 과소 — 건별 확인 후 한쪽 삭제 |
| D2 | 이관 주문 라인 `vat_included=0` 인데 헤더 부가세 정확히 10% | 9,419건(전부 청구) | 잠복 — 주문서에서 수정 저장하면 부가세 소실 |
| D3 | 주문 없는 결제처 입금(카드사 매출정산 8곳·대전지방조달청 등) | 28곳 578건 4.36억 | 원 거래처 미수 과대 + 결제처 선수금 과대(음수 잔액 합 −7.93억) |
| D4 | 법인 오귀속 입금(순합≈0) · 엘이디포유 선명 과지급 | 5곳 ≈760만 · AP 2,322만 | 법인별 잔액만 틀림 |
| D5 | 같은 법인 동일 입금(이관 중복 의심) | 19건 1,223만 | 건별 확인 |
| D6 | 이관 발주 수량 이상(입고수량=금액 5라인, 부분입고인데 전액 21건, 입고 0 RECEIVED 14건) | AP 금액은 정상 | 수량만 오염 |
| D7 | 출금 APPLIED 인데 지급 링크 없음(진안알미늄 5건 — 지급행은 있음) · 비씨카드 정액출금 3건이 매출정산 거래처로 분류 | 4,000만 · 700만 | 링크·분류만 |
