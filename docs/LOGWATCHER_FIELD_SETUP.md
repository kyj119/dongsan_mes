...
**의심의 여지가 없을 때만 자동 교체한다** — 셋 다 맞아야 한다:
①설정 경로가 **30일 이상** 죽었다(일시 중단과 구분) ②최근 3일 내 갱신된 후보가 **정확히 1개** ③watcher 가 1개.
하나라도 어긋나면 **보고만 하고 손대지 않는다**. 교체 때는 `equipment.json` 백업 + **립 위치 파일 삭제**가
한 세트다 — 위치가 옛 파일 기준이라 안 지우면 새 파일을 옛 오프셋부터 읽어 **과거분이 통째로 소급 전송**된다
(지우면 다음 기동이 first run = EOF 시작). 처음엔 이 위험 때문에 수동으로 두었는데, 위치를 지우면 위험 자체가
없어지므로 조건을 좁혀 자동화했다(2026-09-22 HSM-05 실물: 설정 `C:\TNSRip-X1`(2023-10-14·12KB 잔해) ↔
실제 `D:\TNSRip-X1`(당일·2.5MB). **드라이브만 다른 같은 경로**라 사람 눈에도 안 보인 채 3년이 지났다).
