...
5. journey-loop 세션 이월(정본 `/journey-loop` · 메모리 `project-journey-loop` · prod `8b49fdb6`, 09-12 새벽): ~~J1 예외 경로·J2 출고 취소~~ 완료 → **30단계 3사이클 통과**. ~~P8~~ 고침(09-14 `685ea40c`, 나안: 영업=자재 유통 → 주문서·견적서 「원자재 포함」 기본 ON·`user-prefs` 계정별 기억·shell.js 공용) · ~~P11~~ ①②③ 고침(09-14 `3e23e875`: 유통 주문서 삭제·`?type=dist`→302 · `helpers.deriveOrderType` 라인 파생(자재만=DISTRIBUTION, 마스터 없는 라인=제작, 명시값 우선) · `/api/items?with_stock=1` 재고·최근단가 → 검색 모달 열 2개) → **J7 신설·34단계 통과**. ★규격 1칸(`300*90`) 회귀 안 함(09-04·`0601`) — 입력 편의 `*` 분리는 원하면 별도. ⚠️**동시 세션이 :3000 에 dev:d1 을 띄우면 내 번들이 밀린다 → `JOURNEY_BASE_URL=http://localhost:3001 npm run journey:cycle`**(러너는 자기 포트 프로세스만 정리) · 판정 대기 **P9**(완전 출고된 주문은 보드에서 사라져 **출고 취소 화면이 없다** — 보드 「출고됨」 필터 또는 주문 상세 버튼) · **P10**(주문서에서 거래처 안 고르면 앱 문구 대신 「선불/착불」 required 말풍선이 먼저 — 기본값 두거나 앱 검증에 합류). 다음 여정 후보 J4 「발주 없이 입고」. 병행테스트 기간 `npm run journey:cycle` 을 `/loop` 로 반복, 직원 결함은 여정 단계로 승격. P6 = 보안 묶음 때. ⚠️여정용 비관리자 계정은 DESIGNER(MANAGER 는 /orders 접근권 없음).
