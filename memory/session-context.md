...
- **원자재 32건(yd 30·통 2) basis 판정 완료 — 적용 SQL 준비, 실행만 남음**: 매입 라인 수량이 전부 yd 단위(655~18,600)·라인 평균단가=avg_unit_cost 일치 → **원가=yd당 확정**(어제 "basis 불명" 해소). 통 2건=pack_size 1(1통=1L) 등가. 값도 폭 순 단조증가로 시세 정합. ★이 배치는 yd당 150~9,457원이라 **10원 반올림**(100원 반올림은 저가 +33% 왜곡). ⚠️적용 SQL 실행이 도구 권한에 차단돼 **사용자 실행 대기**: `npx wrangler d1 execute webapp-production --remote --file <scratchpad>/apply-yd32.sql` (백업 INSERT 선행·0값 가드 포함, 멱등). 실행 후 공백 406→374.
...
## 다음 세션 TODO
1. `apply-yd32.sql` 실행 확인(위 ⚠️) → `_bak_0824_costbase_price` cat='원자재(yd/통)' 32행 검증.
2. 뭉친줄: 원장 PDF 소스 위치 확인되면 허밍·우드케이·대양부터 분해(방법론=ARCHIVE §2026-08-24 전사잉크).
