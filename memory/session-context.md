...
2-1. **마이그 0501·0502 prod 적용완료** (items 937→**964**): 0501=깃대조립 SET 9종+수기대조립 SET 5종(`{PFX}-GDSET-*`·`TGK-SGSET-*`)+R120 깃대+UV-PATC·UV-TENT + **BOM 28행**(`product_materials` — `bom_items`는 은퇴 테이블이라 안 씀. 수량 컬럼 없음=구성품 1EA 해석). 0502=결정 11건 반영(간판자재 SGM 활성화 3종·신설 3종, 농협시트 LB 2종, ACC-037 유리섬유깃대, ACC-035-R-D ㄷ자꽂이, UV-PC-4.5T-M, TGK-8-1(105×70), AQ-BUJIK). **⚠️D1 함정: 행값 `IN (VALUES ...)`이 에러 없이 0행 매칭 — CTE JOIN 필수**(0501 주석).
