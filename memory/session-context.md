...
| ★바류 산정=비례 확정 | 트러스바·날개·보강대·전선·입체바·캡바 = 자투리 이월 비례(용준님·0510 notes). SMPS·형광등·옆마구리=ceil 유지. 최종 원가율 10.1%·CH 16.9% |
...
## ★실물 역추적 완료 (후속 세션 2026-08-01 밤 — 0511)

- **신규 프레임 9라인 = 전부 실제 신규 확정**(Z:\1. 광고물 도안·견적 7건 판독 — .ai가 %PDF 호환이라 PyMuPDF 렌더→Read). 원가율 FRL 67.7%·FRN 86.3%는 실제.
- **★역방향 오분류 실증**: oi 14420(8100×1650·56k/㎡) 교체 분류였으나 실물=신규 → FRL 재분류. **대형 신규 판가 34~60k/㎡=교체 가격대와 겹침** → 교체(-R 201라인) 원가율은 하한치.
- 보정: 형광등 4→**5.5/㎡**(손의손 3판 실측)·까치발 BOM 신설(ACC-029·가로 2.5m당 1개·매입 3,800). 최종 원가율 **10.5%**.
- 2차 견적 BOM 실판가 확보(신전떡볶이·손의손 견적 분해 — [[design-sign-bom]] 기록).

## ★교체 그룹 전수 보정 완료 (2026-08-02 — 0512)

- 실물 12건 추가 판독(총 19건·10거래처): **≥30만 -R 표본 전부 신규 제작** → **91라인(59.6M) 승격**(재단/천갈이 표기 3건 제외·30만 미만 107L 교체 유지). 신규 65.0M vs 교체 20.8M — P1 가설 반증.
- **★신규 발견 = FRN 111.1%**: 매장용 트러스바 BOM이 **건설현장 각파이프 프레임**(프랜즈=현장 게이트판)에 과대 적용. 각파이프 매입 실적 없음 → 명세 46곳 도착분에서 단가 확보 후 자재 모델 분리.
- ⚠️wrangler `--file` import 엔드포인트만 Auth error 10000 → `--command` 직접 실행으로 우회(query는 정상).

## 도착분 적재 시도 → 판정: 적재 보류·검증만 (2026-08-02)

- **★하우사인 = 겹업(양방향) 거래처 실측 확정**: 매출측 **565주문·청구 35.59M·수금 30.75M(미수 ~4.84M)** vs 매입측 계산서 5.51M·**지급 0(미지급 5.51M)** — 발주 메일 수십 통(솔벤현수막·배너 출력 = 동산의 도매 고객)·상계 정산 구조 가능성 큼. **적재 불가 사유**: ①1~4월 명세 부재(월 원장 발송이 5월분부터 시작) ②2025말 기초잔액 미확보 ③계산서 건별 데이터 원천 없음(hometax_invoices 0건·purchase_invoices는 무관 소액 4건뿐·명단 CSV는 요약만) ④상계 여부 미확인. **행별 검산 불가 상태의 부분 적재는 금지 원칙(#29)** → 보류. 확보분 = 5월 253,275·6월 917,950(품목 라인·`Z:\Designs\123\하우사인`)+6월 계산서 보안메일(암호화 — 사업자번호로 열람).
- **우진환경**: clients 미등록·명단 잔액 0(균형·저순위 "마감만 확인") → 6월 명세(폐섬유 20m³·1,540,000)는 보관만. 등록·적재 불요.
- 필요 시 다음 = 하우사인에 **1~4월 명세+2025-12-31 잔액+상계 정산 여부** 요청 후 적재(겹업이라 AR·AP 동시 대사 — [[design-item-role-multi-flag]] 유사 축).

## 명세 메일 확인·수취 (2026-08-02 — hanmail 검색·읽기 전용, 용준님 승인 하 저장)

- **hanmail(dongsan1902) 검색 결과 → `Z:\Designs\123` 저장 7건**: 하우사인 5·6월 원장(**매월 정기 발송 중** — 요청 없이도 옴·건별 명세도 다수) · 서울경금속 7월 원장 · 케이엠테크 거래처원장 일반+장비(★경영박사 링크형 — **발행 3개월 후 만료**라 PDF로 즉시 보존) · 우진환경 6월 명세(명단 43) · 소브린 명세(7/29 수리비 단건).
- 다음메일 검색은 **점진 스캔**이라 결과가 늦게 늘어난다 — 15초+ 대기 후 판독(초기 결과만 보면 누락). 검색 범위=발신자+제목.
- 진안알미늄·리치코엘 등 잔여 명단은 세금계산서 메일뿐 = **원장 요청 발송 필요**(각파이프 단가 목적 1순위=진안알미늄·극동잔넬). 명단 외 **우승상사**(부대기 하청) 명세·통장사본 다수 관찰 — 필요 시 동일 수취.
- **전 거래처 2차 스윕 결과(08-02 심야)**: ★**다다모아 거래처관리대장 2통 도착**(6/29·7/30 이카운트 발송) — 7/30분 PDF 저장: **이월 13,598,200+7월 1,056,000=잔액 14,654,200**(명단 추정 22.6M과 -7.9M 차 → 대사 필요). **다다모아도 겹업**(동산에 태극기·깃대 발주 다수 = 위탁 생산 고객). 재현테크 전자거래명세표 7/29 저장(+4/24분 존재·스캔 지연으로 미저장·명단 11번) · 소브린 6/4분 추가 존재 · 이카운트(주) 사용료 명세 · KT/오릭스/제네시스 정기 요금명세. **0건**=티피엠·태영기획·유진프라스틱·(진안알미늄·극동잔넬·리치코엘·에코컴퍼니=계산서만). ⚠️다음메일 검색=점진 스캔이라 20초+ 대기 필수·이카운트/경영박사 뷰어의 '확인' 버튼=수신확인 통지라 누르지 말 것(PDF 변환만).

## ★검토 대기 (용준님)

1. **비조명 대형(건설현장) 프레임 자재 모델 분리** — FRN 111%의 원인(트러스 66%). 각파이프 단가 필요.
2. 갈바 산정규칙(돌출 20L 미산정) · 캡바 사용 여부(비례는 확정) · 트러스 폭 규칙 — 확정 시 `product_materials` UPDATE 후 리포트 재실행만.
3. (별건) 재단 패널 트랙 커밋은 **해당 세션이 진행 중**(21시에도 파일 수정 관측 — shell 0.12.0 작업 중)이라 건드리지 않았다. 그 세션 종료 후 그쪽에서 커밋할 것.
