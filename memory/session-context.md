...
2-5. **🟡기타거래처 AR·계산서 제외 코드 (2026-07-31 — 커밋 완료·⚠️미배포)**: 신설 `constants/arPolicy.ts`(`excludeArExcludedClientsSql`=내부법인+기타거래처·`isCashRetailBrn`). AR 표면 10파일 22곳 교체(AP/발주·매출원장은 의도적 미변경) + 발행 가드 3곳(**all-zero BRN이 체크섬 통과라 기존 `!brn` 검사 무력**). 계산서 후보는 `invoice_method='CARD'`로 이미 제외. 검증=tsc 0·build·entity 60/60·prod 시뮬(제외 차 2,126,880=기타 잔액 정확). **push=자동배포라 명시 요청 대기** — 배포 후 확인: 미수금 목록 기타거래처 부재·회계허브 AR −2,126,880·발행 400 문구.
