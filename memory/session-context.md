...
> **다음 세션은 맨 위 세션의 "다음 세션 이월 TODO"부터 읽는다** — 그 아래 상세 핸드오프는 판단 근거가 필요할 때만.
> (⚠️ 아래쪽 "이월 TODO 통합" 절은 2026-08-06 시점이라 최근 두 세션 분이 빠져 있다. 최신은 항상 맨 위 세션 것.)

# 세션 핸드오프 — 목록 UX 사후 점검 라운드 + 열폭 전수 스윕 (2026-08-09)

> 이식이 끝난 목록 UX(감사 `docs/audits/2026-08-08-list-ux-ecount-gap.md` §10)를 **실측 위주로 다시 훑어** 9건 수정.
> 커밋 `2536d33c`(P0~P3) → `a720d37f`(현황판) → `66e16d84`·`72112df7`·`5eb43d16`(열폭) → `c981445e`(문서). 전부 push 완료.
> 배포 `0b162bbc` → `2908345d` → `41ec5bed` → `c01bf6a4`. 매 배포 smoke **110/110**.
> **마이그레이션 없음.** 로컬 dev 서버(`npm run dev:d1`)가 켜져 있을 수 있음.

## 이번에 고친 것 (전부 prod)

| 등급 | 내용 |
|---|---|
| **P0** | 주문 CSV 가 8,778건 중 **3,000건만 무고지 절단**(마지막 행 2026-05-15·5,778건 유실) → 캡 5,000 + 안내행 |
| **P1** | 주문번호 108→140·상태/회계반영 →104 · 공통 `.col-code` 132→**148** · 긴급/타법인 배지 아랫줄로 |
| **P1** | 원장 매출 프리셋이 **기간을 잃던 것**(저장은 하는데 복원 안 함) |
| **P2** | 주문 기간·정렬을 「더보기」 밖으로 · 날짜 라벨 「등록일」→**주문일**(실제 조건은 `o.order_date`) |
| **P2** | 출고 이력 정렬 셀렉트 신설 · 견적 CSV 신설 · 견적 기본정렬 SSOT 복원 |
| **P3** | 폐기 소재 DOM 조회 제거 → `check:dom` **OK 0건** · 죽은 `/api/*` 호출 0건 확인 |
| 후속 | `/hr` 사번 88px(이름 130→190) · `/bank` 계좌번호 188·잔액 134·정기지출 범위 200(개월 60·성격 68) |

## 다음 세션 이월 TODO

1. **CSV 없는 목록 5개**(지급요청·매입계산서·재고·거래처·출력실적) — ⚠️**조회조건 SSOT 빌더 추출이 선행.**
   지금 붙이면 WHERE 사본 5벌이 생겨 이번 P0 의 원인 구조(주문 CSV 법인필터 누락)를 그대로 재생산한다.
2. **`/api/reports/entity-attribution-audit` UI 없음** — 어제 P5 로 만든 법인 귀속 감사인데 화면이 없다.
   `/reports` 는 매출 분석 탭 묶음이라 데이터 품질 감사와 성격이 달라 **배치 위치가 제품 결정**. 물어볼 것.
3. **반품 2단계** — `returns`/`customer_claims` prod 0건. 등록 UI 는 `/quality` 「반품(RMA)」에 **이미 있다**
   (`POST /api/returns`). 그래서 "못 만들어서 0건"이 아니다. ★**확인 필요**: 반품이 정말 안 일어나는지,
   아니면 일어나는데 MES 에 기록을 안 하는지. 후자면 "데이터를 기다린다"는 계획 자체가 틀렸다.
4. `orders.is_voucher` 플래그 — 전표 제외가 아직 주문번호 접두(`ICM-%`·`%OPEN%`) 의존.
5. 분석 탭 명세 3표 칩 · 남은 기존 열 잘림(`/equipment` 이름·`/activity-log` 일시 로케일 표기·`/tasks`·`/storage-zones`).
6. **주문 CSV 5,000행 상한** — 안내는 뜨지만 8,778건 전체는 못 받는다. 상한 상향은 운영 판단 대기.

## 판단 기준 (다시 헤매지 않으려면)

- **"이식 끝 = 끝"이 아니다.** 이번 9건 중 3건은 이식이 만든 게 아니라 **원래 있었는데 안 보이던 것**이다.
  끝났다고 선언한 영역을 **실측으로 한 번 더** 훑는 라운드가 값을 했다.
- **형제-불완전 스윕의 원인은 게으름이 아니라 '수단 부재'였다.** 주문 CSV 만 `#372` 스윕에서 빠진 건
  주문만 `csvStreamResponse`(스트리밍)를 쓰는데 **그 함수에 `footerNote` 파라미터가 없어** 안내를 붙일
  수단이 아예 없었기 때문. → 스윕이 한 곳을 건너뛰었으면 **"왜 거기만 달랐나"를 먼저 본다.**
- **공통 CSS 를 고칠 땐 그 클래스를 쓰는 페이지를 전수 실측한다.** `.col-code` 는 20개 페이지 31곳.
  `table-layout:fixed` 에선 `min-width` 가 무시돼서 **한 열을 넓히면 잘림이 옆 flex 열로 옮겨간다.**
- **잘림 측정은 두 단계.** `scrollWidth > clientWidth` 는 *잘렸는지*만 안다. *얼마나 필요한지*는
  canvas `measureText + padding` 으로 재야 한다(안 잘린 열은 둘이 같다).
- **저장/복원 대칭은 스크립트로 전수 대조.** 13개 목록 중 비대칭 1건(원장)을 눈으로는 못 찾았다.

## 주의사항 (사고 2건 — 둘 다 내 오판)

- ⚠️⚠️ **`git push … | tail && deploy` 는 게이트가 아니다.** 파이프라인 종료코드는 마지막 명령(`tail`) 것이라
  **push 가 거부돼도 `&&` 가 통과해 배포가 나간다**(오늘 실제 발생). 이번엔 선행분이 전부 `.md` 라 무사했지만
  소스였으면 회귀였다. **게이트 명령은 파이프에 넣지 말 것** → [[feedback-multi-session-deploy]].
- ⚠️ **SPA 페이지 응답은 JSON 이다.** `X-SPA-Request` 로 받으면 HTML 이 `pageContent` 에 **이스케이프되어**
  들어와 `id="x"` 마커가 전부 빗나간다. 오늘 마커 11개 중 7개를 "미반영"으로 오판했다(실제론 정상).
  `JSON.parse(t).pageContent` 를 꺼내 검사할 것 → [[reference-spa-auth-page-fetch]].
- ⚠️ **배포 후 브라우저가 구버전을 보여준다.** 서버 번들엔 새 값이 있는데 화면은 옛 값이었다.
  "미반영" 판정 전에 **서버 번들 마커부터** 확인하고, 브라우저는 캐시버스터로 재로드.
- ⚠️ **도구모음은 마운트 시점에 프리셋 목록을 1회 읽는다.** API 로 만든 프리셋은 **새로고침 전엔 셀렉트에 없어**
  `sel.value=id` 가 무효 → "프리셋 기능이 고장났다"로 오판하기 쉽다(오늘 겪음).

## 검증 명령 (PowerShell)

```powershell
npm run verify                      # tsc + build
npm run smoke                       # 로컬 110/110 (dev:d1 기동 필요)
$env:SMOKE_URL='https://webapp-9i0.pages.dev'; npm run smoke   # prod
node scripts/sort-audit.cjs ; node scripts/entity-audit.mjs ; node scripts/check-dom-refs.cjs
```

---

# 세션 핸드오프 — 전사 8색 LogWatcher 현장 설치 (2026-08-09)

> **2대 모두 온라인·신빌드·경로 실측 확인.** `TRANS-8C-01`(DESKTOP-5C9D04J) · `TRANS-8C-02`(PC-202605141926).
> 서버는 08-05 배포분 그대로(마이그 `0518`·`0519`, 스모크 107/107). 이번 세션은 **에이전트 쪽만** 손댔다.
> 커밋 = `006cb236`(설치기) → `bea160b5` → `397a685d`(롤오버) → `18dc496a`(시각형식) → `eb3f747e` →
> `28cb0751`(생성기) → `dde8bb02` → `4d242c87`(문서). 전부 push 완료.

## 다음 세션 이월 TODO (전사 축)

1. **실출력 1건 자연검증** — 아직 양쪽 이벤트 **0건**(설치일이 일요일). 첫 건이 뜨는지, 소요시간이 0분이 아닌지 확인.
2. **첫 건은 카드 미연결이 정상** — 전사는 파일명에 주문번호가 없다(145건 전수). `/production` 「출력파일 연결」 1회 → `print_file_map` 학습 → 이후 자동. **이걸 모르면 "매칭이 고장났다"로 오진한다.**
3. 잉크 소모(`KDots` 8색×3드롭) 미착수 — **중단분은 실제 출력 안 됨 → 완료율 보정 필요**.
4. `emit_rip_only_after_hours`(기본 0=끔) — 리핑만 하고 출력 안 한 잡을 RIP 이벤트로 낼지 운영 판단.
5. `Z:` 패키지 4개 중 `LogWatcher`·`LogWatcher-v2`·`LogWatcher-transfer-8c` 는 설치기 정본 반영 완료(구파일 `.bak-20260806` 보존). **`LogWatcher-nest-v2` 는 install-service.bat 자체가 없다**(다른 세션 소관) — 그 패키지로 설치할 땐 설치 수단을 먼저 확인할 것.

## 판단 기준 (다시 헤매지 않으려면)

- **"같은 기종 2대"를 같다고 가정하지 말 것.** 같은 PrintExp 인데 로그 위치·**줄 시각 형식**이 갈렸다. 중국어 마커는 동일해서 **블록 인식은 양쪽 다 된다** → 실패가 성공처럼 보인다. **`--test` 소요시간이 전부 0분이면 시각 파싱 실패**다.
- **하트비트가 오면 경로는 맞는 것이다.** `WatcherManager` 가 `IsAccessible()`(두 폴더 모두 존재) 실패 시 **하트비트도 안 보낸다**. 경로 의심 전에 이걸 먼저 본다.
- **`equipment_id` 가 하트비트에 실려 있으면 파서 생성도 성공한 것**이다(실패하면 `[INIT] FAILED` 후 목록에서 빠진다).
- **`print_log_path` 가 서버에 채워졌는가 = 신빌드 판별기.** 구빌드는 폴더감시형 파서의 키를 몰라 항상 빈칸.
- **이벤트 0건 ≠ 고장.** 이 파서는 켠 시점부터 기록한다(`backfill_days:0`, PrintExp 는 EOF 시작). `agent_heartbeats.created_at` 으로 **언제 켰는지** 먼저 확인할 것 — 3일 전 설치인 줄 알았는데 4분 전이었다.
- **현장 증상은 도구 탓일 때가 많다.** "중국어가 나왔다"=UTF-8 bat + 다른 코드페이지, "경로를 못 찾는다"=관리자 창의 UAC 별도 세션, "30초 대기"=robocopy 기본 재시도 100만 회.

## 주의사항

- ⚠️ **repo 에 안 돌아온 현장 수정은 다음 배포에 되살아난다.** `install-service.bat` 정본이 `LogWatcher-v2` 패키지에만 있었다. 현장 파일이 repo 보다 새로우면 **repo 로 되가져올 것**.
- ⚠️ **JSON 경로 손편집 금지** — `"C:\PrintExp…"` 의 `\P` 는 `'P' is an invalid escapable character`. `make-equipment.ps1 -Unit N` 이 `/` 로 써준다(윈도우 정상 동작, 파서 검증 완료).
- ⚠️ **PS5.1 은 BOM 없는 `.ps1` 을 ANSI 로 읽는다** → 스크립트 안 한글이 깨져 설정에 그대로 박힌다. BOM 필수.
- ⚠️ 에이전트 갱신은 **창 2개** — 일반 창에서 Z:→C: 로 받고, 관리자 창에서 로컬끼리 복사(`/XF` 불필요, 원본에 설정이 없음).
- ⚠️ `Z:\Designs\LogWatcher-nest-v2` 는 **다른 세션 Flexi 패키지**. 건드리지 말 것.

---

# 세션 핸드오프 — 재단 패널: 맞붙임·칼선 정확도·구멍·DXF 경로 (2026-08-07~08)

> **4축 배포완료** — host `CUT-CEP-0.20.0` · shell `0.55.0` · `audit:ia-jsx` **드리프트 0** · 백업 `_backup\20260808-180221`.
> 게이트 = `cut:smoke` **371** · `panel:smoke` 111 · `cut:bench` · `cut:butt` 42 · `cut:nest` · `cut:bleed`.
> ⚠️ **각 디자이너 PC 는 `install-a0-panel.bat` 미실행** — 축2(호스트)는 Z: 1파일이라 이미 전 PC 반영이지만 껍데기는 PC별이다.

## 다음 세션 이월 TODO (재단 축)

1. **플로터/CNC 실기 투입** — 만든 DXF 를 **실제 장비에 걸어본 적이 없다**(spec §9-8ⓑ). 이게 안 되면 아래 전부가 가정이다. **용준님만 할 수 있음.**
2. **`MIN_HOLE_MM` 2mm 물리 근거 확정** — 칼날·비트 지름으로. 지금 값은 노이즈를 걸렀을 뿐(§9-10).
3. **라운드/이형 조각의 공유 변** — 직각 사각만 맞붙임이 된다. 라벨 맵 방식(판 전체를 조각 번호로 굽고 조각↔조각 경계를 한 번만 추출) = 특수 케이스 없이 라운드까지 커버. 직각 사각은 지금의 mm 산수가 더 정확하니 **둘을 병행**.
4. 반컷/완컷 구분 근거(§9-2) · 재단 이력 DB 기록(§9-5) · `rasterizeItem` 임시문서 재사용(성능)
5. **판짜기 경로에 여전히 없는 것** = 없음(구멍까지 이번에 이식). 단품↔판짜기 격차 해소됨.

## 판단 기준 (다시 헤매지 않으려면)

- **맞붙임이 안 켜지면 화면이 이유를 말한다** — 조건 5개(butt.js 유무·호스트 0.18+·여백/간격 0·조각 크기·본체 직사각). 조용히 폴백하면 "기능이 고장났다"로 읽힌다. 실제로 그렇게 한 세션을 날렸다.
- **성능을 의심하기 전에 스레드가 살아있는지 본다** — CDP `Runtime.evaluate` 가 **즉시 응답하면 죽은 것**이다(느린 게 아니다). → [[feedback-cep-callback-silent-death]]
- **`anyOverlap` 류 자기검사는 축척 오류를 못 잡는다** — 같은 공간 안에서만 보기 때문. **다른 출처와 대조**해야 한다(배치 크기 ↔ 굽은 마스크 bbox).
- **굽기 캔버스 테두리에 닿은 덩어리는 아티팩트다** — pad ≥1mm 라 진짜 아트는 못 닿는다. 남의 파일 탓으로 단정하지 말 것(오진 전례).
- **길이만으로 직선과 완만한 호를 못 가른다** — 호는 새그 ≈ L²/(8R). 편차를 직접 재야 한다.
- **evalScript: 인자는 ASCII, 반환은 한글 OK.** 경로를 받아서 되넘기면 그때 죽는다 → **왕복을 없앤다.**
- **MCP Illustrator `run` 은 문서 열거·저장에서 계속 hang** 한다. 안정적인 것은 **패널 CDP(8888)** 뿐. `view` 는 **앞에 있는 창**을 찍으므로 일러를 앞으로 올려야 한다(`BridgeTalk.bringToFront`).
- **축2 배포는 `--yes` 로 안 넘어간다**(비대화 실행이면 거부) — 실제 터미널에서 사람이 y 를 눌러야 한다. 설계 의도이므로 우회 금지.

상세 = durable 메모리 [[design-irregular-nesting]] · [[feedback-cep-callback-silent-death]]
...
| 순위 | 이슈 | 요지 |
|---|---|---|
| 🔴 1 | #597 | **재단 칼선(DXF) 붙은 주문이 PUT 수정 시 order_ai_files RESTRICT FK로 500** — 편집 자체 차단, 기능 버그 최우선 |
| 🔴 2 | #598 | 전사 8색(0518/0519) rip.ts 장비별 실적 통계에 event_kind='PRINT' 필터 누락 — RIP+PRINT **이중집계** |
| 🔴 3 | #592+#593 묶음 | 간판 BOM item_code 오참조 3종 silent no-op + 마이그 0509↔0512 상충 재분류(item 7282) — 원가 정합, 같은 도메인 한 세션 |
| 4 | #590 | 주문 수정 시 기존 행 에누리(line_discount) 화면 미복원 → 저장 즉시 소멸 — 금액 데이터 손실 |
| 5 | #589 | 취소주문 2단계 하드삭제가 합배송 `consolidate_with_order_id` 정리 누락 — 죽은 링크 잔존 |
| 6 | #591 | 공군사관학교(3763) BRN에 내부코드 '00017' 저장 — 세금계산서 거부 위험, 데이터 정정 1건 |
| 7 | #585+#586+#587 묶음 | 광고문자 3건(실패 수신자 식별 #574 미반영·수정 시 미리보기 게이트 무효화 안 됨·수신거부 검색창 부재) — 같은 도메인 |
| 8 | #596 | **fixed-in-tree** — `e27cd7f` 반복지출 패널이 이미 요구 충족(백로그 Area6 판정). 확인 후 close만 |
| — | #453 | infra: 스케줄 auto-scan의 prod 도메인 egress 차단 — 환경 제약, 코드 아님, 별도 판단 |

**B. 작업지시서 실주문 자연검증** — 신규 주문 → 카드 체크리스트 파생 → 현장 체크 실사용 관찰 (prod 기존 카드는 체크리스트 없음=섹션 숨김이 정상, 신규 생성분부터 파생)

**C. 재단 세션(#42) 이월분** — ①실주문에서 주문폴더 EPS+DXF 쌍 자연검증 ②디자이너 PC 4대 `install-a0-panel.ps1` 설치 ③이 PC 일러 재시작(shell 0.16.0 로드)

**D. 후속(운영 피드백 후)** — category별 체크리스트 settings 템플릿 · MANUAL 스텝 추가 UI · `POST /generate/:orderId` 레거시 경로 generateCardsForOrder 통합 검토

~~prod 배포~~ → **완료(2026-08-06)**: 0522 prod → deploy `c10031a7` → 리뷰 후속 `a74d9628` → #600 `37555b17`, prod smoke 108/108
...
> 정본 문서 = `docs/dongsan-import/SMARTA_FIXED_ASSETS.md` · `SMARTA_LOANS.md` · `SMARTA_FIXED_EXPENSES.md`
> 커밋 `e27cd7f3` 까지 push 완료 · prod 배포 `adef1e90` · 마이그 `0517` · smoke 107/107

## 결과

| 트랙 | 결과 |
|---|---|
| ① 감가상각 엔진 검증 | 정률 26건 원 단위 일치 · 결함 A·B·C 처리 · 비품 32건 이관 |
| ② 차입금 대사 | 20.2억 누락 → 17건 등록 · **세무장부와 원 단위 일치** |
| ③ 고정비 누락 탐지 | 22건 월 23,143,760 · 안분 결함 수정 · 탐지 API+화면 |

**월 규모** 감가상각 20,374,349 · 고정비 23,143,760 · 차입금 19,913,036
**차입금 잔액** 2,067,615,858 = 세무장부 293(1,849,163,285) + 253 금융성(218,452,573) ✓

## 판단 기준

**세무장부는 화면을 골라야 한다.** 감가상각 검증은 고정자산관리대장(SAAS0107)이 아니라
**미상각분감가상각계산(SAAS0102)** — 상각대상금액이 있어 정액/정률을 구분할 수 있다.
부채는 합계잔액시산표(SACL0104)→거래처원장(SABK0109)→계정별원장(SABK0107) 순으로 좁힌다.
매입 실적 원본은 **매입매출장(SABK0116)**.

**계정과목명·적요를 믿지 말 것.** 「비품」(212)에 생산장비 46건이 들어 있어 연 6,267만이 빠져 있었고,
`Extreme-UV1804 1 x 26,000,000` 은 실제 공급가액이 37,000,000 이었다(부가세가 10% 로 맞아 기장은 정상).

**정기지출은 통장에서 먼저 찾고 세무장부 계정으로 확정한다.** 계정을 못 찾으면 등록하지 않는다 —
농협생명 908,000/월이 실은 저축성보험이었다.

## 이번에 내가 틀린 것 (반복 방지)

1. **AP 2.56억 과대 → 오진.** `purchase_orders` 에 `SMP-` 접두의 **선명(e2) 매입이 섞여 있는데**
   합산치를 동산기획만의 세무장부 251 과 비교했다. 법인별로 나누면 **+2,018만(3.1%)**.
   → 세무장부 대조는 **entity 를 먼저 맞출 것**.
2. **스타리아 "MES DISPOSED vs 세무장부 상각중 불일치" → 틀림.** 208 대변합 13,980,000 이 처분 기록.
3. **현대캐피탈 "통장엔 있고 253 잔액엔 없다" → 착시.** 내가 잔액 0 행을 필터로 뺐다.
4. **구축물 재계산 중 전기말 누계를 날렸다.** **이관 자산은 전년말 앵커 기록이 필수**.
5. **PowerShell `-match` 마커 확인 → 대용량 문자열에서 오탐.** `.Contains()` 를 쓸 것.

## 주의사항

- **`generate-schedule` 은 만기 미확인 차입금에 쓰면 안 된다.** `maturity_confirmed=0`(마이그 `0517`)이
  막지만 **화면 버튼은 안 막혀 있다.**
- **73142 는 회전형** — 원금 스케줄 없음. **잔액을 세무장부 293 에서 주기 갱신**해야 한다.
- **적재 SQL 전부 비멱등**(`loans.loan_number`·`fixed_expenses.name` UNIQUE 없음).
- **`docs/dongsan-import/load/` 는 gitignore** — 커밋은 추적 경로만.
- 세션 내내 **다른 세션이 같은 체크아웃에서 커밋·브랜치 전환**을 했다. `deploy:prod` 는 워킹트리
  전체빌드라 남의 dirty 파일이 배포에 실린다.

## 이월 TODO

**바로 가능** — AP 잔여 +2,018만(매입 계상 시차 확인) · 보류 7건 74,338,253 처리 ·
현대상사/대진국기사/NTIC 거래처 확정(유사명 다수, 추측 금지) · 운산직물 은행연동 40,000,000 대응 정리

**자료 필요** — 여신거래약정서 8건(12.6억) · 오릭스 상환예정표 · 케이엠테크 거래명세서(UV1804 11,000,000)

**미착수** — 장비 전수조사 `docs/EQUIPMENT_SURVEY.md` · 회차형 상대명(`CMS 미래01018`→`02019`) 정규화
...
npm run build
npm run deploy:prod
$env:SMOKE_URL="https://webapp-9i0.pages.dev"; npm run smoke   # 기본값은 localhost
```

---

# 세션 핸드오프 — 재단 완성 시트 주문 연결 + 재작업 규칙 (2026-08-05 #42)

> durable 메모리 = [[design-rework-rules]] · [[design-irregular-nesting]]
> 커밋 `826baef8`(기능) · `309f0ad9`(현황판) — **push 완료** · prod 배포 `e8db0863` · 마이그 `0516` · 4축 드리프트 0

## 이월 TODO 통합

1. **실주문 자연검증** — 재단 시트 등록 → 주문 라인 → 출력 후 **주문폴더에 EPS+DXF 쌍**이 생기는지,
   주문 상세에 `✂DXF` 배지가 뜨는지. 헤드리스로 못 덮는 구간이라 실사용으로만 확인된다.
2. **디자이너 PC 4대 패널 설치** — `Z:\DESIGNS\IA-등록\_scripts\install-a0-panel.ps1` + 일러 재시작.
   미설치 PC 는 [품목]이 keyword 로 안 실린다(고장 아님). ※이전 세션부터 이월된 항목.
3. **이 PC 일러 재시작** — 패널 `shell 0.16.0` 로드용(디스크는 최신, 실행 중이라 구버전 페이지를 물고 있음).
4. (조건부) **경로 C 빈도가 늘면** 주문 복사에 파일 연결 승계 추가 → C 가 A 처럼 전부 자동.
   지금은 **의도적으로 안 넣었다**(재작업이 새 파일을 만드는 경우 승계가 방해된다).

## 결과

출발 = "주문서를 현재 MES 편집기로 못 만드는 **묶음 품목·재단이 들어있는 파일**을 완성품으로 라인에 연결하는 방안 점검".
갭 6건 중 배관 결손 2 + 오묶음 1 + 식별정보 1 처리, 재작업은 **기능화하지 않고 운영 규칙으로 확정**.

| # | 내용 | 위치 |
|---|---|---|
| 1 | passthrough(-3)가 EPS 옆 동명 `.dxf` 동반 복사 + 라인 등록 | `Program.cs:3041` · `RegisterLineFileAsync` |
| 2 | 라인 부가 파일 1:N (`order_ai_files` + `order_item_id`·`kind`) | 마이그 `0516` · `orders/operations.ts` · `core.ts` |
| 3 | 주문 상세 `✂DXF` 배지(클릭=경로 복사) | `orders.js` `copyLineFilePath` |
| 4 | 재단 시트를 트레이 자동 묶음에서 제외 | `orderForm/intake.js` `ofTrayBundleKey` |
| 5 | 패널 [품목] → manifest `keyword` (shell `0.16.0`) | `cut-main.js` · `index.html` |
| 6 | 재작업 절차서 + CLAUDE.md 참조 | `docs/REWORK_RULES.md` · `CLAUDE.md:116` |

## 결정 + 이유

- **청구 단위 = 시트 전체 단가**(용준님). 조각별 청구를 안 하므로 시트 1장 = 라인 1개 유지, **묶음 품목으로 펴지 않는다**.
  이유 = 자식은 *각자 자기 파일*을 갖는 모델이라 시트 1파일을 자식 N개에 물리면 같은 EPS 를 N번 복사한다.
  **수용한 한계** = 조각별 청구 불가 · 클레임 시 조각 지목 불가.
- **DXF 는 경로 기록까지만**(웹 다운로드 X). 실물은 Z:·주문폴더에 있고 현장 PC 가 직접 연다. R2 는 작업량 2~3배라 미채택.
- **재작업 = 개발하지 않는다.** 판단은 질문 2개 — ①파일을 다시 만드나 → **경로 A**(패널 재등록, 전부 자동)
  ②출고 전+무상인가 → **경로 B**(기존 주문 유지) / 그 외 → **경로 C**(복사+수동).

## 판단 기준 (같은 자리에서 헷갈릴 것)

- **DB 에 dxf 경로를 안 실은 이유** — 패널이 EPS 와 **같은 폴더·같은 이름**으로 DXF 를 만들어
  `Path.ChangeExtension` 으로 충분했다. "intake 에 dxf 컬럼 추가"는 불필요한 확장이었다.
- **`print_file_map` 을 쓰지 않은 이유** — RIP 출력완료 매칭 테이블이고 `UNIQUE(order_number, file_seq)` 라
  DXF 를 끼우면 seq 가 밀려 **카드 매칭이 오염**된다. 라인↔파일 정본은 `order_ai_files`.
- **경로 B/C 의 파일명 규칙이 정반대인 이유** — 매칭이 파일명 꼬리로 `print_file_map` 을 찾는데 그 행은
  **에이전트가 파일을 만들 때만** 생긴다. 새 주문(C)은 꼬리를 새 주문번호로 바꿔야 원 주문 오염이 없고,
  기존 주문(B)은 행이 그대로라 **원본 파일명이 정답**이다.

## 주의사항 (실측으로 드러남)

- ⚠️**Windows 경로를 인라인 `onclick` 인자로 넘기지 말 것** — 백슬래시가 JS 이스케이프로 먹힌다
  (`Z:\DESIGNS\2026` → `Z:DESIGNS2026`). `data-` 속성으로. 이번에 만들고 실측으로 잡았다.
- ⚠️**passthrough 파일 경로가 Z: 인 것은 조건부다** — 라인 해소 순서가 order aiFilePath → `item.ai_file_path`
  → **라인 `ai_analysis_id` R2 다운로드**(`Program.cs:2590`)인데, 재단 시트는 `file_path` 가 `Z:/…eps` 라
  `/download` 가 400 으로 떨어져(`aiAnalysis.ts:406`) Z: 경로가 유지된다.
  **R2 로 내려받는 경로였다면 임시폴더에 EPS 만 있어 DXF 동반 복사가 무력**했다. 유사 기능 추가 시 먼저 확인.
- ⚠️**주문 복사는 `ai_analysis_id`·`ai_file_id` 를 승계하지 않고**(`orders/operations.ts:192-221`)
  **금액은 원본 그대로 승계**한다. 무상 재작업인데 단가를 안 고치면 매출 이중계상.
- ⚠️**일러 실행 중이면 패널은 구버전 페이지를 물고 있다**(디스크는 최신). 버전 불일치를 보면 재설치부터 하지 말고 디스크 확인.
- ⚠️로컬 D1 은 비어 있다(orders 0건). `seed_data` 는 `item_categories` UNIQUE 충돌로 중단. `clients` 에는 **`entity_id` 컬럼이 없다**.
- ⚠️`npm run smoke` 기본값이 `localhost:3000` → **prod 검증 시 `$env:SMOKE_URL` 필수**.

## 워킹트리

`src/scripts/iaEditor.js` — **세션 시작 전부터 있던 dirty(주석 2줄, 내가 안 건드림)**. 회전 패스스루 구현 완료를
설명하는 주석이라 동작 영향 없음. 커밋 여부는 용준님 판단. 그 외 클린 · `main` = `309f0ad9` · origin 동기.

---

# 세션 핸드오프 — SmartA(WEHAGO) 매입장부 전수 대사 + 7곳 적재 (2026-08-03~04)

> 정본 문서 = `docs/dongsan-import/SMARTA_AP_RECONCILE.md` · 원천 = `load/smarta_ap_*.json`
> durable 메모리 = [[reference-smarta-wehago-ledger]] · [[feedback-ap-estimate-blindspots]]
> 커밋 ac6df476 · 5c26d351 · 43883982 · e709300e · 5db49141 · e3e43189 (**전부 push 대기**)

## 결과

- **세무 기장 정본 = SmartA, WEHAGO 웹에서 직접 조회 가능**(경리 PC·파일 수취 불필요). 로컬 DuzonBizon 설치본은 2025-04 방치·무용
- 기적재 6곳 중 **5곳이 세무장부와 원단위 일치** — 이관 방법론 자체가 검증됨
- **신규 7곳 적재 완료** = 티피엠 41,081,943 · 텍스젯 8,488,261 · 리치코엘 6,657,800 · 에코컴퍼니 5,653,810 · 재현테크 4,246,000 · 유진프라스틱 3,008,300(clients 신규) · 대승 600. 합 69,136,714, 전부 원단위 일치
- **품목 연결 23행 / 99,760,900원**. 신규 품목 2종 등록(GDS-FB1-CASE·GDS-FB1-FIN)

## 핵심 판단 (반복 금지)

- **VAT 역산**: SmartA 대변은 VAT 포함 총액 → 공급가 `/1.1`. 전 건 정확히 나뉨. 생성기가 안 나뉘면 예외로 중단하게 해뒀다
- **★리치코엘은 도금 임가공 업체**(품목 공급 아님). 가로봉=가로기용 막봉 도금→황금봉, 수기봉=애기봉=수기용 막봉 도금. 전부 ACC-041. 처음 ACC-032로 붙였다가 정정 — **품명만 보고 품목을 정하지 말 것**
- **아일렛 호수 역산**: 5호 21원·9호 27원. 전표 적요는 `수량 X 단가`이고 대표 1행만 실림 → 적요분 빼고 반대 호수 단가로 나누면 나머지 0. 같은 수법이 다른 거래처에도 통할 수 있다
- **수성잉크(C)-20L = RM-I-TPM-C-20L**. 공급사명(TPM)이 품목코드에 이미 박혀 있었다 — 품목 검색 시 **공급사명 접두도 같이 볼 것**
- **단가표 ≠ 매입 증빙**: 티피엠 수입미디어 단가표(메일 5/6)를 보고 솔벤매쉬 매입으로 추정했으나 용준님이 "안 산다" 확인 → 폐기. 현수막소재 정체는 여전히 미상
- **설비·용역은 "기타 분류"가 아니라 item_id NULL일 뿐**. `purchase_order_items`에 품목 외 지출 분류 컬럼이 없다

## 이월 TODO

1. **미지급금(253) 전수 확인 미완** — 매입처 명단 사업자번호로만 필터했다. 명단 밖 거래처·미지급비용(262)·장기미지급금(297) 미확인. **WEHAGO 재로그인 필요**(세션 만료로 중단)
2. **정운교역 지급 3종 20,952,440 보정** — 어음 9,000,000 + 상계 6,952,440 + 외상매출금 어음회수 5,000,000. 넣으면 AP 310,636,189 → 289,683,749
3. **잔여 적재 4,097,221** — 사인앤그래픽 3,960,000(253) · 와우프레스 113,520(253) · 삼영섬유 23,701(251·이월 단수)
4. **거래명세서 대기** — 티피엠(현수막소재 정체)·텍스젯(실사원단)·진안알미늄/극동잔넬(각파이프 단가, 간판 BOM FRN 111% 해결용)
5. **자산등록 기능 = API만 있고 UI 0** — `routes/fixedAssets.ts`(등록·감가상각·처분 완비), 화면·사이드바·권한 전무, 데이터 0건. 티피엠 OPTIMUM-EP3208 53,096,000이 첫 대상
6. **ACC-041 황금봉 원가 주의** — 막봉 매입 + 도금비 합산이고 용도(가로기/가정용/수기대)가 한 품목에 묶여 있다. avg_unit_cost 산정 시 별도 판단
7. 하반기 증분 트랙(7월 이후) — 기준일 6/30로 통일해 적재했다
...
- **✅다다모아 매입 적재 완료(08-02 — prod·6번째 거래처)**: `gen_dada_ap.py`→`load/dada_ap.sql`. OPEN 198,000(1~2월·2/28) + 일별 PO 5건(3/24 발색기 CF2000 8M·3/30 잉크·4/6·4/21 XP1000 대량·5/11 — 품목 라인 18·잉크 XP1000/ST1000N 미등록이라 품명 보존) + 지급 2건(3/5 기타 198,000·**3/24 상계 8,800,000** — AR측 이관 수금 '채권채무상계처리'와 1:1 대칭). **실측 AP 13,598,200 원단위 정확**·기존 5곳 불변·smoke 104/104. 발생 합=명단 계산서합 22,596,200 정합. 7월 1,056,000은 하반기 증분 트랙(기준일 6/30 통일). 롤백=notes 마커 '다다모아 매입 이관 2026-08-02'.
- **★다다모아 대장 1~6월 추적 완료(08-02)**: 발송 이력 = 2/26·4/29·5/28·6/29·7/30 매월말 5통(1·3월 발송 없음). **2/26분 = 이카운트 링크 만료**("메일 확인 기간 경과" — 이카운트도 만료 있음, 경영박사 3개월과 동일 함정 → **대장류 메일은 도착 즉시 PDF 보존이 정책**). **5/28분(기간 3/1~5/28) 확보** → 7/30분과 함께 **3/1~7/30 전 구간 품목 라인 + 잔액 체인 완성**: 이월 198,000(3/1) → 3월 발색기 CF2000 8.8M 판매+**"외상매입금 외상매출금 상계처리" 8.8M(=겹업 상계 실증!)**+잉크 → 4월 11.9M(XP1000 잉크 대량) → 5월 0.92M → 기말 13,598,200 = 7/30 이월 ✓ → 7월 1,056,000 → **최종 14,654,200**. **명단 대사 완전 정합**: 계산서합 22,596,200 = 이월 198,000 + 3~5월 발생 22,398,200 / 잔액차 = 상계 8.8M+수금 198,000−7월 1.06M. 1~2월 상세만 유실(이월 198,000으로 수렴 — 대사 무지장). → **다다모아는 적재 가능 상태**(품목 라인·이월·상계·검산 전부 확보). 부수 발견 = (주)탑미디어 대장(3/30·만료 추정).
