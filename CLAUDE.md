...
- **`ia:deploy`**(`ia-deploy.cjs` `GATES`): **audit:empty-catch** · **audit:jsx-ternary** · **audit:jsx-syntax** · cut:bleed · cut:nest · **cut:rectfast**(사각 빠른 경로 — 띠 도련=전체 PNG 픽셀 동일·배선) · cut:butt · cut:placement · cut:plate · cut:plate:baseline · cut:frame · cut:review · **cut:trorder**(주문→전사 입력 매핑) · cut:smoke · **cut:shellsync** · panel:smoke · cut:e2e + ia-jsx 드리프트 (⚠️`test:outcopy` 는 2026-09-15 **하루 만에 은퇴** — 지키던 코드가 에이전트로 넘어갔다. **없어진 코드를 지키는 게이트는 초록불이 아무 뜻도 없다** → 성질은 `panel:smoke` §13 으로 옮겨 실었다)
