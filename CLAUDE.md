...
- **셸로 코드를 주입할 때 백틱은 명령 치환이 된다** — Git Bash 에서 `node -e "…\`foo\`…"` 의 백틱이 실행돼 **그 자리만 조용히 비고 나머지는 멀쩡히 저장된다**(이날 버전 주석·현황판 배너가 그렇게 잘렸다). 백틱이 든 한글 문장은 `node -e` 로 넣지 말고 **파일에 써서 읽힌다**(§heredoc 백슬래시 붕괴와 같은 축). **같은 축의 셋이 더 있다**(2026-09-18 실측): ①**MSYS 경로변환**이 `/` 로 시작하는 **인자**를 `C:/Program Files/Git/...` 로 바꾼다 — `node probe.cjs "/api/x"` 가 `ENOTFOUND webapp-9i0.pages.dev**c**` 로 죽는 게 그 증상이다(URL 뒤에 `C:/...` 가 붙어 호스트가 잘못 파싱된 것). `MSYS_NO_PATHCONV=1` + **스크립트 경로는 Windows 형식**으로 주거나, 애초에 대상 목록을 스크립트 안 배열로 박는다. ②**quoted heredoc(`<<'EOF'`) 도 백슬래시를 삼킨다** — 백슬래시 2개짜리 정규식 `replace(/\\/g,'/')` 가 1개짜리 `replace(/\/g,'/')` 로 저장돼 SyntaxError 가 난다(정규식·경로가 든 코드는 heredoc 말고 **파일로 쓴다** — 이 문장 자체도 처음엔 heredoc 으로 넣다가 같은 자리에서 당했다). ③scratchpad 스크립트는 프로젝트 `node_modules` 를 못 찾는다 — 절대경로로 require.
...
### 느린 게 아니라 크다 — 페이로드는 응답시간에 안 나타난다 (`npm run audit:query-cost` 의 `maxKB`)
**200 이고 빠른데 낭비인 응답이 있다.** 2026-09-18 전수 감사(prod GET 430건 + 페이지 33개 실측): 전환·응답시간은 이미 건강했고(SPA 62~102ms · API 중앙값 ~60ms · 500 **0건** · D1 통계 508행 정상 · GET 경로 N+1 실질 3건), 낭비는 **화면이 쓰지도 않는 바이트**였다. 재고 화면은 **로스율 숫자 하나** 때문에 실사 상세 **357KB** 를 받아 브라우저에서 합산했고(`inventory.js`), 발주 공급처 필터는 활성 거래처 **전량 2,890곳 218KB** 를 받았는데 고를 수 있는 건 발주 이력이 있는 **123곳**뿐이었다. 둘 다 **141ms·124ms** 라 `audit:query-cost` 의 `budgetMs` 에도 안 걸린다 — **`maxKB` 만이 이걸 잡는 눈금**이다. 새 목록 API 를 만들면 「화면이 이 필드를 다 쓰는가」를 먼저 묻고, `TARGETS` 에 `maxKB` 와 함께 줄을 추가한다. 값을 올릴 때도 같은 질문을 먼저 한다.
- **집계는 서버에서 접는다** — 화면이 비율·합계 하나만 그리는데 원본 배열을 보내고 있으면 그건 전건 반환이다. 목록 응답에 합계를 실으면 **요청 자체가 사라지는** 경우가 많다(실사 상세 호출이 그랬다).
- **필터 드롭다운의 모집단은 「전체」가 아니라 「고를 수 있는 것」이다** — 목록을 거르는 용도라면 그 축에 실제로 존재하는 값만 있으면 된다.

### 같은 화면인데 어떤 클릭은 2배 느리다 — SPA 를 타는 동선과 아닌 동선 (2026-09-18)
`spaNavigate` 는 **사이드바 링크만** 가로챘다(`shell.js` `.sidebar .nav-item[href]`). 그래서 사이드바로 가면 62~102ms, 화면 안에서 눌러 가면 165~279ms 인 **같은 화면**이었다 — 사용자에게는 「어떤 건 빠르고 어떤 건 느리다」로 보인다. 코드 이동의 정본 = **`navigateTo(url)`**(`location.href=` 를 새로 쓰지 않는다).
- ★**`pushState` 는 페이지 스크립트 실행 *전*이어야 한다** — 종전엔 뒤였고, 그래서 스크립트가 `location.search` 로 자기 인자를 읽으면 **이전 페이지의 주소**를 봤다. 사이드바 링크엔 쿼리가 없어 한 번도 드러나지 않았다. 쿼리로 화면을 여는 경로(`/orders?view=`·`/order-form?edit=`)를 SPA 로 보내려면 이 순서가 선행 조건이다.
- **SPA 로 보내면 안 되는 셋**: ①외부/비내부 주소 ②**같은 pathname 재진입**(`?raw` 전역 스코프에서 스크립트가 두 번 돌면 재선언이 충돌한다) ③**해시가 붙은 주소**(`pushState` 는 `hashchange` 를 발화시키지 않아 hash 로 탭을 여는 페이지가 기본 탭에 머문다 — #622 와 같은 축). 인증·권한 리다이렉트(`/login`·`/no-permission`)와 단독 레이아웃(portal·purchaseInvoice)도 그대로 둔다.
- ⚠️**전환 성능을 잴 때 Playwright `page.click()` 을 쓰면 안 된다** — actionability 대기가 섞여 「SPA 가 2~3배 느리다」는 **정반대 결과**가 나온다. `el.click()` 으로 직접 누르고, 판정은 「데이터가 화면에 보일 때까지」로 통일한다.
- ⚠️**Tailwind Play CDN 은 최초 방문에만 725ms 다**(재방문은 캐시 적중 0ms). 빌드 전환은 `grid-cols-${n}` 같은 **런타임 생성 클래스를 조용히 깨뜨린다** — 상세·판단 근거 = `docs/audits/2026-09-18-performance-audit.md`.
...
- **게이트 = `npm run test:calc`** — **37항목 체인, 목록 정본=`package.json`**(개수는 `grep -o "npm run" | wc -l` 로 센다 — 손으로 적은 수는 낡는다)(청구면적 `test:orderline`·마감표기 `test:finishing-label`·파일규격 `test:file-dims`·과다출력 분모 `test:declared-qty`(파일명 **두 축** — 패널 `-N장)`·에이전트 `-NEA-`. 2026-09-22 실기: 에이전트 축을 안 읽어 1EA 주문 2매 출력이 과다로 안 떴다)·여신 `test:credit`·품목중복 `audit:items:selftest`·4대보험 기간요율 `test:insurance-period`·공제 오버라이드 `test:payroll-override`·간이세액표/자녀공제 `test:income-tax` 외 24개). **`deploy.yml`(CI) · `ship:gate`(/ship) · `/deploy-verify` Phase 1 세 경로 전부가 배포 전에 돌린다**(2026-08-25 CI 신설 → 2026-09-10 나머지 둘 편입. 그전엔 로컬 `deploy:prod` 로 내보내면 이 게이트가 배포를 못 막고 **이미 나간 뒤** CI 실패로만 드러났다).
...
- ★**`GradientColor.angle` 은 거짓말한다 — 방향의 정본은 `.matrix` 다**(2026-09-22 실측, 일러 30.7). 세로 그라디언트가 `.angle=0` 으로 읽히고 `.matrix=[0,-1,1,0]`(90° 회전)이 진짜였다. 색 객체에 `angle`·`matrix` 를 **넣는 것은 둘 다 무시된다**— 방향을 주는 유일한 길은 **개체에 칠을 붙인 뒤 `rotate(deg, false, false, true, false)` 로 칠만 돌리는 것**이다. 각도는 `atan2(matrix.mValueB, matrix.mValueA)` 로 뽑는다(V3 왕복 확인). 전사 도련 `extend` 가 첫 실기에서 세로 바탕을 **가로로** 깔았던 자리다(`mesTr_bleedExtend`). ★**비활성 문서의 개체 속성 읽기는 한 번에 수십 ms 다 — 수천 번이면 수십 분**(2026-09-22 실측, 일러 30.7). `documents.add` 로 판 문서가 활성이 된 **뒤에** 원본(비활성)의 잉크 상자를 다시 재는 루프가 **38~41분**이었고, 같은 루프가 원본 활성일 때(pick·bake)는 몇 초였다. CPU 는 거의 0 · Not Responding · 예외 없음이라 **교착처럼 보인다**(강제 종료 2회). 처음엔 별색 그라디언트를 의심해 두 번 고쳤는데 둘 다 헛짚었다 — 단계별 시각을 파일에 흘려 쓰는 훅(`mesTr_trace`)을 넣고서야 「첫 줄에서 다음 줄로 못 넘어감」으로 잡혔다. **원인을 모른 채 코드를 두 번 바꾸지 말고 시각을 흘려 쓴다.** 규칙 = 다른 문서를 만들기 **전에** 원본에서 읽을 것을 전부 읽어 숫자로 넘긴다(`mesTr_backdropInfo`). ⚠️멈춘 일러에 COM(`GetActiveObject`)을 부르면 **그 호출도 같이 멈춘다** — 상태는 `Get-Process` 로만 본다. ★프로브 JSX 는 **ASCII 만** — 한글 주석 한 줄이 `Error 44`(인코딩 변환 실패)로 파일 전체 로드를 막는다(세 번 걸렸다).
- ★**`visibleBounds` 는 「보이는 것」이 아니다 — 클립이 잘라 낸 부분까지 합쳐서 답한다.** 실측(일러 30.7, 2026-09-21): 클립 패스 60x180mm 인 그룹이 안의 배치이미지(214.74x163.93) 때문에 **214.74x242.83mm** 로 잡혔다. 예외도 경고도 없이 **크기만 3.58배**다. 클립은 디자인 파일에 흔하므로 **개체 크기를 재는 코드는 전부 이 함정 위에 있다.** 정본 = A0 `mesA0_itemBounds` · 재단 `mesCut_inkBounds` · 전사 `mesTr_inkBounds`(전부 **클립 ∩ 콘텐츠**). 겸해서 **채움도 획도 없는 패스는 세지 않는다** — 실기 문서에 5,779mm 짜리 빈 패스가 있어 눈에 안 보이면서 모든 군집을 한 덩어리로 붙였다. `position` 도 같은 상자를 보므로 배치는 잉크를 재서 `translate` 한다.
- ★**그래서 새 틀·새 호스트가 걸릴 자리는 「무엇을 재는가」가 아니라 「크기를 어디서 얻는가」다.** 윈드배너가 오늘 안전한 이유는 틀을 잘 만들어서가 아니라 **개체를 아예 안 재기 때문**이다 — 크기가 EPS 헤더 `%%HiResBoundingBox` 에서 오고(`cut:frame --verify` 가 Z: 실파일과 대조), 호스트는 파일을 열기만 한다(`mesTr_openFrame`). **틀에 「앉히기」나 「자동 분석」을 붙이는 순간** 개체를 재게 되고 그때 이 함정에 들어간다(F형은 실루엣 틀이라 더 그렇다). 게이트 = `panel:smoke` §17e — **`mes-*-host.jsx` 를 열거해서** 개체를 재는 호스트마다 클립 존중 입구(`mes<축>_inkBounds` 규약)를 요구한다. 손으로 셋을 적어 두면 **다음 호스트는 안 걸린다**(이 저장소가 손목록으로 같은 사고를 세 번 냈다). 생짜로 재는 것이 맞는 자리(진단용·임시 캔버스는 크게)는 **옆에 사유를 적는다** — A0 `:886`·`:946` 이 그 본보기다.
...
- 안전으로 세는 다섯: ①인라인 `slice(i, i + N)` ②그 slice 로 선언된 식별자 ③`chunks.push(slice)` 후 for-of ④**명시적 길이 가드** ⑤공용 `chunk80()`(`src/utils/chunk.ts`) 이 만든 배열의 원소(for-of·`.map` 인자). N 은 상수 식별자도 읽는다(`EMP_CHUNK`).
- ★**「길이 가드가 있다」를 글자로 세면 안 된다 — `if (ids.length > 0)` 이 상한 가드로 둔갑한다.** 2026-09-21 실기: `>`·`>=` 이기만 하면 가드로 쳐서 **30곳이 기준선에 이름조차 없이** 숨어 있었다(아침에 「32건으로 줄였다」고 적은 수가 그래서 나왔다). 상한 가드의 성질은 둘이고 **둘 다** 봐야 한다 — ①넘치면 **빠져나간다**(then 가지가 return/throw/continue/break. `&&` 로 묶이면 안 센다, `||` 는 센다) ②**N 이 실제 상한이다**(`0`·`1` 은 빈 배열·단건 분기다 → `N >= 2`). 허용형(`if (a.length > 0 && a.length <= 90) { …IN… }`)은 **then 가지 안에서 쓰일 때만** 인정한다.
- ★**게이트가 철자를 못박으면 개선을 막는다** — 탐지기가 `slice(i, i + N)` 이라는 **표현**만 알아서 공용 `chunk80` 헬퍼를 잡았다(§조용한 격하의 「구현을 못박으면」과 같은 축). 이름으로 인정하되 **이름만 믿지 않는다**: 감사가 `src/utils/chunk.ts` 를 파싱해 **실제 청크 폭을 읽고** 한도를 넘으면 exit 1 한다(80→500 발화 확인).
- **길이를 정적으로 알 수 없다 ≠ 결함**이다 — 자연 bounded(단일 주문 라인·고정 enum)면 기준선(`scripts/bind-limit-baseline.json`, 2026-09-21 기준 37건)에 **사유와 함께** 넣는다. 기준선에 없는 새 자리만 exit 1. 고쳐서 없앴으면 `--update` 로 **줄인다**(안 줄이면 되돌아가도 안 잡힌다).
- ★**「100 이하라 안전」은 근거가 아니다 — 배열 옆에 상수 바인드가 하나만 붙어도 101 이 된다.** 2026-09-21 실기: `items.ts` `with_stock=1` 이 품목 **100개 + entity 1개 = 101** 로 prod 500 이었다(경계 실측 `limit=99`→200 · **`100`→500**). 화면이 `limit=50` 만 써서 사람 눈에 안 보였는데 `safeLimit` 은 **500 까지** 허용하고 있었다 — **화면이 안 쓴다고 경로가 없는 게 아니다.** 더 아픈 건 그 자리가 **이 감사의 기준선에 「safeLimit 종속(기본 100 이하)」로 안전하다고 적혀 있던 자리**라는 점이다. 기준선의 `reason` 은 **「검토했다」는 뜻이지 「안전하다」는 뜻이 아니다.**
- 그래서 risk 는 **수치로** 적는다 — `low`(한 건의 하위 행·고정 상수) · `bound`(상한을 수치로 확인·여유 있음, 그 수를 적는다) · **`edge`(상한이 100 에 닿아 있다 — 상수 하나만 늘어도 터진다)** · `input`(호출자가 개수를 정하는데 가드 없음). 2026-09-21 재분류에서 `edge` 2건(`costs.ts` 정확히 100 · `aiAnalysis.ts` 200=#655)과 **미청크 실결함 1건**(`cards/queries.ts` — 카드 `safeLimit` 200 이라 analysis id 가 200까지 간다. **#409 가 난 바로 그 파일인데 이 자리만 청크가 빠져 있었다**)이 나왔다.
- ⚠️**`--update` 가 사유를 지우면 안 된다** — 해소 1건을 반영하려고 부른 갱신이 **나머지 전부의 사유를 조용히 날렸다**(2026-09-21, 39건). 지금은 보존하고 「사유 보존 N건」을 찍는다. 기준선의 값어치는 목록이 아니라 **「왜 안전한가」**에 있다.
- ★**가드로 막을지 청크할지는 「그 수를 세어」 정한다** — #657(세금계산서 묶음) 은 여섯 자리에 `order_ids` 가 흐르고 auto_issue 면 국세청 전송까지 가므로 길이 가드가 단순해 보였는데, prod 를 세니 **한 거래처 한 달 주문 최대 118건**(90 초과 4건)이었다. 가드 90 은 **실제 월말 청구를 막는다** → 청크. ⚠️**원자성이 필요한 쓰기는 문장만 나누고 `db.batch` 는 하나로 둔다**(청구확정·계산서 취소·합포장 — 청크마다 커밋하면 「절반만 확정된 계산서」가 생긴다). 개수 대조는 **누적 후**에 한다.
...
- **패널 축**(`거래처-키워드(규격)-후가공-N EA` · 주문번호 **없음**) = 등록 행을 **주문서 저장 후 흡수할 때** 쓴다(`workbench.ts`).
  ...
  ⚠️같은 날 재등록하면 픽업 사본(`_출력\<날짜>\`)이 **같은 이름이라 덮어써진다**. → **막지 않고 경고한다**(2026-09-18): 호스트가 등록 때 픽업 폴더와 **오늘의 다른 등록 폴더**를 봐서(`mesA0_pickupClash`/`mesCut_pickupClash`) 가공은 warn `P`, 재단은 `pickdup=` 로 패널에 띄운다. **이름에 접미사를 안 붙이는 이유** = 이 파일명이 그대로 RIP 에 나가는 이름이고 그 규약이 08-27 에 출력완료 매칭 0% 를 되살린 근거다 — 같은 건을 다시 낸 것인지 다른 건인지는 **사람만 안다**. 게이트 = `panel:smoke` §14(양방향 자가시험).
...
- **`ia:deploy`**(`ia-deploy.cjs` `GATES`): **audit:empty-catch** · **audit:jsx-ternary** · **audit:jsx-syntax** · cut:bleed · cut:nest · **cut:rectfast**(사각 빠른 경로 — 띠 도련=전체 PNG 픽셀 동일·배선) · **cut:hardengrid**(굳히기 격자 = 확대 후 캔버스 안) · cut:butt · cut:placement · cut:plate · cut:plate:baseline · cut:frame · cut:review · **cut:trorder**(주문→전사 입력 매핑) · cut:smoke · **cut:shellsync** · panel:smoke · cut:e2e + ia-jsx 드리프트 (⚠️`test:outcopy` 는 2026-09-15 **하루 만에 은퇴** — 지키던 코드가 에이전트로 넘어갔다. **없어진 코드를 지키는 게이트는 초록불이 아무 뜻도 없다** → 성질은 `panel:smoke` §13 으로 옮겨 실었다)
...
### 게이트가 **구현**을 못박으면 개선을 막는다 (2026-09-17 실기)
**게이트는 「그 자리에 그 글자가 있는가」가 아니라 「그 성질이 지켜지는가」를 물어야 한다.** `cut:smoke` 의 폰트 어서션은 주석에 성질을 정확히 적어 놓고(「살아 있는 텍스트가 있으면 폰트가 반드시 실린다」) **검사는 `embedAllFonts = true` 의 개수가 2인지**를 봤다. 그래서 같은 성질을 더 잘 지키는 구현(조건부 판정)이 **FAIL 로 막혔다**.
- **막힌 게 게이트 덕인지 탓인지부터 가른다** — 이날 게이트가 잡은 것 자체는 옳았다(재단은 A0 와 달리 아웃라인을 보장하지 않으므로 `true` 에 근거가 있었다). 잘못은 **그 근거를 개수로 표현한 것**이다. 게이트가 막으면 ①성질이 깨졌나 ②표현이 낡았나를 먼저 갈라야 하고, **②일 때만 게이트를 고친다**(그리고 고쳤으면 자가시험으로 **양방향** 확인 — 되돌리면 FAIL, 원복하면 PASS).
- **호스트가 둘이면 같은 결정도 둘이다** — A0 가 0.9.0 에서 고친 `embedAllFonts` 줄이 **재단에는 한 달 넘게 안 왔다**(work.ai 폐지도 같은 형태였다). 두 호스트를 **대조하는** 어서션은 이미 있다(`cut:smoke` 3l 「EPS 형식 옵션이 A0 와 동일」) — **공유 결정이 새로 생기면 거기에 줄을 추가한다.** §형제 스윕의 호스트판이다.
- **「이제 되네」로 닫지 않는다** — 같은 날 「대기함에서 거래처가 안 들어오더니 이제 들어온다」가 있었는데, 후보가 셋(설계된 가드·프리필 경쟁조건 수정·JWT 수정)이라 **무엇이 고쳤는지 특정하지 못했다.** 원인을 모른 채 닫으면 **안 돌아온다는 근거도 없다** — 재현 조건을 적어 두고 다음에 같은 증상이 나오면 그 목록부터 지운다.
