...
### 마이그레이션 번호는 유일하지 않다
`migrations/` 에 같은 4자리 번호가 반복해서 생긴다(2026-09-10 실측 21개 번호 — 그중 `0576`은 3중복). 병렬 worktree 세션이 각자 다음 번호를 딴 결과라 계속 늘어난다 — **하드코딩된 목록은 매 사이클 바로 낡으므로 여기 나열하지 않는다.** 현재 목록은 `ls migrations | sed -E 's/^([0-9]+)_.*/\1/' | sort | uniq -c | awk '$1>1'` 로 직접 확인(대상 테이블·컬럼이 겹치는지도 같이 확인 — 지금까지는 전부 우연히 비겹침). 추적 이슈 = #639.
...
- 게이트 없음(미구현) — 새 마이그레이션을 만들 때 위 명령으로 직접 확인.
