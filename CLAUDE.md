...
- ★**`visibleBounds` 는 「보이는 것」이 아니다 — 클립이 잘라 낸 부분까지 합쳐서 답한다.** 실측(일러 30.7, 2026-09-21): 클립 패스 60x180mm 인 그룹이 안의 배치이미지(214.74x163.93) 때문에 **214.74x242.83mm** 로 잡혔다. 예외도 경고도 없이 **크기만 3.58배**다. 클립은 디자인 파일에 흔하므로 **개체 크기를 재는 코드는 전부 이 함정 위에 있다.** 정본 = A0 `mesA0_itemBounds` · 재단 `mesCut_inkBounds` · 전사 `mesTr_inkBounds`(전부 **클립 ∩ 콘텐츠**) — 새 호스트를 만들면 이 셋을 대조하는 `panel:smoke` §17e 에 줄을 추가한다(전사가 한 달 늦게 따라온 자리다). 겸해서 **채움도 획도 없는 패스는 세지 않는다** — 실기 문서에 5,779mm 짜리 빈 패스가 있어 눈에 안 보이면서 모든 군집을 한 덩어리로 붙였다. `position` 도 같은 상자를 보므로 배치는 잉크를 재서 `translate` 한다.
