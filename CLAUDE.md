...
- ★**`GradientColor.angle` 은 거짓말한다 — 방향의 정본은 `.matrix` 다**(2026-09-22 실측, 일러 30.7). 세로 그라디언트가 `.angle=0` 으로 읽히고 `.matrix=[0,-1,1,0]`(90° 회전)이 진짜였다. 색 객체에 `angle`·`matrix` 를 **넣는 것은 둘 다 무시된다**— 방향을 주는 유일한 길은 **개체에 칠을 붙인 뒤 `rotate(deg, false, false, true, false)` 로 칠만 돌리는 것**이다. 각도는 `atan2(matrix.mValueB, matrix.mValueA)` 로 뽑는다(V3 왕복 확인). 전사 도련 `extend` 가 첫 실기에서 세로 바탕을 **가로로** 깔았던 자리다(`mesTr_bleedExtend`). ★**다른 문서의 색 객체(Gradient·SpotColor)를 새 문서 개체에 붙이면 일러가 CPU 0 으로 멈춘다**(실기 3회, 2026-09-22 — 원본 그라디언트 패스 복제 1회 · `gc.gradient = 원본.gradient` 할당 1회, 둘 다 PANTONE 별색 스탑 · 예외도 대화상자도 없이 COM 만 끊긴다 · CMYK 스탑 합성 문서는 같은 코드가 8초). 같은 원본을 `duplicate` 로 통째 복제하는 굽기는 10초에 끝나니 「복제」가 아니라 **색 객체의 문서 간 참조**가 함정이다 → 판 문서 안에 `gradients.add()` 로 새로 만들고 스탑마다 **새 `CMYKColor`** 로 옮긴다(`mesTr_toCmyk`). ⚠️멈춘 일러에 COM(`GetActiveObject`)을 부르면 **그 호출도 같이 멈춘다** — 상태는 `Get-Process` 로만 본다. ★프로브 JSX 는 **ASCII 만** — 한글 주석 한 줄이 `Error 44`(인코딩 변환 실패)로 파일 전체 로드를 막는다(세 번 걸렸다).
