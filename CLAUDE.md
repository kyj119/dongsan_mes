...
- ★**`GradientColor.angle` 은 거짓말한다 — 방향의 정본은 `.matrix` 다**(2026-09-22 실측, 일러 30.7). 세로 그라디언트가 `.angle=0` 으로 읽히고 `.matrix=[0,-1,1,0]`(90° 회전)이 진짜였다. 색 객체에 `angle`·`matrix` 를 **넣는 것은 둘 다 무시된다**— 방향을 주는 유일한 길은 **개체에 칠을 붙인 뒤 `rotate(deg, false, false, true, false)` 로 칠만 돌리는 것**이다. 각도는 `atan2(matrix.mValueB, matrix.mValueA)` 로 뽑는다(V3 왕복 확인). 전사 도련 `extend` 가 첫 실기에서 세로 바탕을 **가로로** 깔았던 자리다(`mesTr_bleedExtend`). ⚠️`duplicate`+`resize` 는 매트릭스를 보존하지만, 다른 문서의 **클립 그룹 안** 개체를 문서 밖으로 복제하는 경로는 4.5GB 메모리 상태의 실기에서 25분 무응답 뒤 COM 이 끊긴 이력이 있다(깨끗한 일러에선 186ms — 원인 미확정) → 바탕은 복제하지 않고 **새 사각에 칠만 옮긴다**. ★프로브 JSX 는 **ASCII 만** — 한글 주석 한 줄이 `Error 44`(인코딩 변환 실패)로 파일 전체 로드를 막는다(세 번 걸렸다).
