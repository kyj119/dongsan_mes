...
// 어느 원단에도 통으로 안 들어갈 때만 폴백 — **분할 수는 최소**로 못 박고, 그 안에서 **폭을 균등 분배**.
// 3500×4000, 최대폭 3200 → 두 방향 다 2분할. 3500 을 반으로 나눈 1,750 을 1800폭이 덮는다.
// (종전엔 패널을 전부 3200폭으로 계산해 41,455원이었다 — 지금은 9,134원)
placement('두 변 다 최대폭 초과 — 균등 분할한 폭에 맞춘다', AQ, 3500, 4000, 1,
  { qty: (4000 / 914.4) * 2, width_mm: 1800, splits: 2, fitted: false })

// ★자투리 회귀 (2026-09-10) — 짧은 변이 최대폭을 **조금** 넘길 때가 가장 심했다.
//   `AQ-WDB 4400×330` = 3,300mm 가 3,200 을 100mm 넘겨 **전폭 2장**으로 계산됐다(456,009원/장).
//   균등 분할이면 1,650mm 짜리 두 장 → 1800폭 두 장 = 100,477원. prod 942라인 3,265만원 과다.
placement('자투리가 작아도 전폭 2장이 되지 않는다', AQ, 44000, 3300, 1,
  { qty: (44000 / 914.4) * 2, width_mm: 1800, splits: 2, fitted: false })
{
  const got = selectRollPlacement(AQ, 44000, 3300, 1)
  if (!got || got.mat.width_mm === 3200) {
    fails.push('자투리 100mm 가 전폭 한 장 값을 받으면 안 된다 · 실제 폭=' + (got && got.mat.width_mm))
  } else pass++
}

// 분할 수는 **늘리지 않는다** — 균등 분배는 폭만 고르는 것이지 분할 폭주를 여는 게 아니다.
// 24000×42000(장폭 현수막 240×420cm 급): 8분할이 최소이고 균등 폭 3,000 을 덮는 건 3200 뿐 → 종전과 같다.
placement('균등 분배가 최대폭을 이기지 못하면 종전 그대로', AQ, 42000, 24000, 1,
  { qty: (42000 / 914.4) * 8, width_mm: 3200, splits: 8, fitted: false })
