...
}
{
  ...
  // A0: realW = 파일 × N · 실물 mm 상수는 ÷N · 파일명 `_1-N`. 재단도 같아야 한다.
  {
    ...
    // ★목록 달린 칸은 조합 중 목록을 뗀다 — **대비책이지 확인된 수정이 아니다**(2026-08-31).
    //   "거래처에 한글이 안 들어간다" 보고로 넣었지만, 이후 실측이 가설을 뒷받침하지 않았다:
    //   CDP 조합 주입은 목록이 붙은 채로도 정상이었고, **실제 타이핑 기록엔 composition 이벤트가
    //   하나도 없었다**(CEF 가 자모를 곧장 beforeinput/input 으로 준다) — 즉 이 처리는 발동하지 않는다.
    //   증상은 패널을 다시 여니 사라졌다 → 실제 원인은 OS 키보드/IME 포커스 유실 쪽으로 본다.
