...
// ⚠️ 2026-08-24 재정렬: 구 계약(현재 초점/블로커/다음 액션/핸드오프/멀티세션 배너 + 🔴 현재 진행 중 등)은
//   2026-08-10 문서 다이어트 이후 실재하지 않아 **모든 트림이 검증 실패→자가 복구**만 하고 있었다
//   (배너 13건>임계 12 인데 트림 불가). 계약은 문서의 현행 정본 구조를 따른다 — 문서를 계약에 맞추지 않는다.
//   이모지 변형(VS16)에 흔들리지 않게 절 이름 텍스트로만 잡는다.
const CONTRACT = [
  ['제목', /^# PROJECT_STATUS\.md/m],
  ['운영 규칙 블록', /^> \*\*운영 규칙/m],
  ['최근 완료 인덱스 절', /^## .*최근 완료/m],
  ['진행 중 절', /^## .*진행 중/m],
  ['결정·확인 대기 절', /^## .*결정·확인 대기/m],
  ['설계 확정 절', /^## .*설계 확정/m],
  ['보류함 절', /^## .*보류함/m],
  ['편집 중 절', /^## .*편집 중/m],
  ['잠복·블로커 절', /^## .*잠복·블로커/m],
  ['기존 에러 절', /^## .*기존 에러/m],
  ['ARCHIVE 포인터', /PROJECT_STATUS_ARCHIVE\.md/],
]
...
function analyze(lines) {
  ...
  // '진행 중' 섹션 경계 (구명 "현재 진행 중" → 현행 "🔴 진행 중" — 둘 다 잡는다)
  let secStart = -1
  let secEnd = -1
  lines.forEach((l, i) => {
    if (/^## .*진행 중/.test(l) && !/편집 중/.test(l) && secStart < 0) secStart = i
