...
| `max_event_age_days` | ★ 파싱 직후 이보다 오래된 이벤트는 버린다 (0=끄기) | `7` |

> **★ `max_event_age_days` 는 소급 오염 차단기다 (2026-08-31 신설).**
> 위치 파일이 어긋나 있으면 append-only 로그를 옛 지점부터 다시 읽는다 — RIPLOG 는 60MB+ · **2~3년치**라
> 한 번에 수천 건이 소급 전송된다. 서버 멱등키(`file_path`+`print_completed_at`)는 **이미 적재된 것만** 막으므로
> **한 번도 적재된 적 없는 장비**(실측: HYB-3200-01·SOLV-3200-01 은 prod 이벤트 0건)에는 아무 방어가 없고,
> 이벤트마다 `applyEventToCard` 가 **옛 카드를 출력완료로 뒤집는다 — 되돌리기 어렵다.**
> 실측(재생): 위치를 0 으로 두면 20,217건이 나오는데 그중 **20,184건이 7일 초과**(최고령 2019-11-21)였다.
> 정상 운영에서 7일을 넘는 이벤트는 나오지 않는다(폴백 창은 시간 단위). PC 가 며칠 꺼져 있었어도 그대로 복구된다.
> ★ 재시도 큐에는 적용하지 않는다 — 큐에 든 건 이미 파서를 통과한 정상 이벤트다(오프라인 며칠이면 당연히 오래된다).
> ★ 폐기는 조용히 하지 않는다 — `⚠ N일보다 오래된 이벤트 N건 폐기 (최고령 …)` 를 남긴다.

> **★ 2축 파서는 둘째 축이 없어도 멈추지 않는다 (2026-08-31 수정).**
> 종전에는 `IsAccessible()` 이 두 축을 AND 로 묶어, D: 가 늦게 붙거나 PrintExp 를 새 폴더로 재설치하면
> **그 장비의 실적이 통째로 0** 이 됐다 — 잡으려던 맹점보다 나쁜 결과다.
> 이제 신원 축(`Print.log`·RIPLOG)만 있으면 계속 돌고, 둘째 축이 사라지면 상태가 바뀔 때 한 번
> `⚠ … 없음 — 2축 조인 중단, 립 기준으로만 송출합니다` 를 남긴 뒤 `rip_fallback_hours` 로 흘려보낸다.
> 축이 돌아오면 `… 복구 — 2축 조인 재개` 를 남기고 다시 조인한다.
