...
#### `tns_flora` — TopazRip + Flora 2축 조인 (평판) ★
...
| `record_size` | | 레코드 크기 (기본 2376). 파일 길이가 배수가 아니면 Flora 축을 건너뛴다 | `2376` |
...
>
> **레코드 구조** (2,376바이트 고정 append. 실측 20,137/20,137 파싱 성공):
> | 오프셋 | 내용 |
> |---|---|
> | `0x0000` | 매직 `5C A3 F0 F0` — 형식이 다르면 여기서 걸러진다 |
> | `0x0008` | uint16 레코드 ID (단조 증가) |
> | `0x0024` | `.prt` 전체 경로 (OS ANSI = 한국어 Windows 면 cp949, NUL 종료) |
> | `0x0910` / `0x0920` | SYSTEMTIME 시작 / 종료 |
> | **`0x0933`** | **중단 플래그 0=완주 / 1=취소** — 판정 정본 |
> | `0x0938` | pass 수(진행량). 판정에 쓰지 않는다 — 방향성 확인용(완주 중앙값 403초 vs 취소 79초) |
>
> **조인 = 파일명.** print_rec 는 RIP 산출물(`<잡명>NNNN_M.prt`), Print.log 는 원본(.eps/.jpg) 이라
> 꼬리 `NNNN_M.prt` 만 떼면 basename 이 같다. 실데이터 재생 조인율 **최근 200건 98.5% · 1,000건 94.2%**.
