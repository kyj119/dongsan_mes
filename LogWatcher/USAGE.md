...
LogWatcher는 각 RIP PC에 설치되어 TopazRip(TNSRip-X11)의 `Print.log`를 실시간으로 감시하고, 출력 이벤트를 MES 서버에 자동 보고하는 에이전트 프로그램입니다.
...
LogWatcher는 마지막으로 읽은 위치를 `last_position.txt`에 기억합니다.
전체 로그를 다시 읽으려면 이 파일을 삭제하세요.
