...
// 가산형 열람 가드 — 라우터 레벨 read 게이트(`.use('/*', requireRole(...))`) 대체용(회귀 0).
//   ADMIN·legacyRoles 는 종전 그대로, 그 외 역할은 해당 페이지 can_access=1 이면 통과.
//   재무 라우터처럼 라우터 전체가 requireRole 로 막혀 신규 역할이 읽기조차 못 하는 곳에 사용.
export function requireAccessOrRole(pageKey: string, ...legacyRoles: string[]): MiddlewareHandler<HonoEnv> {
  return async (c, next) => {
    const user = c.get('user') as any
    if (!user?.role) {
      return c.json({ success: false, error: '인증이 필요합니다' }, 401)
