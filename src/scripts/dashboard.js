        ...
        if (statsResponse.data.success) {
            ...
            if (kpiArea && kpiArea.querySelector('.ds-skeleton')) {
              kpiArea.innerHTML =
                ...
                + '<div class="ds-card ds-card-compact"><div class="flex items-center justify-between mb-1"><div class="text-sm" style="color:var(--c-text-secondary)">수금률</div><i class="fas fa-hand-holding-usd text-xs" style="color:var(--c-success);opacity:0.6"></i></div><div class="text-3xl font-bold tabular-nums" style="color:var(--c-text)" id="statCollectionRate">-</div><div class="text-xs mt-1 tabular-nums" style="color:var(--c-text-muted)" id="statCollectionDetail">이번 달</div></div>'
      ...
      if (shipments.length === 0) {
        ...
        shipmentsEl.innerHTML = shipments.map(function(s) {
          ...
          return '<div class="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 cursor-pointer" onclick="location.href=\'/shipments\'">'
            ...
            + '<div class="text-[10px] text-green-600">출고완료</div>'
            + '</div></div>';
        }).join('');
      }
    }
