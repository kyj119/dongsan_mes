  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      html += '<tr class="hover:bg-gray-50">'
        + '<td class="px-3 py-2 text-left font-medium" title="' + escapeHtml(r.employee_name || '-') + '">' + escapeHtml(r.employee_name || '-') + '<div class="text-[10px] text-gray-400">' + escapeHtml(r.employee_code || '') + '</div></td>'
        + '<td class="px-3 py-2 text-left text-xs" title="' + escapeHtml(deptName) + '">' + deptName + '</td>'
        + '<td class="px-3 py-2 text-left text-xs" title="' + escapeHtml(r.entity_name || '-') + '">' + (r.entity_name || '-') + '</td>'
...
window.lcSave = async function() {
  ...
  var payload = {
    ...
    base_salary: parseInt(document.getElementById('lcBaseSalary').value) || 0,
...
function lcSelectEmployee(empId) {
  ...
  if (preview) {
    ...
    var ent = emp.entity_id && window.entityName ? window.entityName(emp.entity_id) : '-';
