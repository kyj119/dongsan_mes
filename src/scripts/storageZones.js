  ...
  try {
    ...
    var rows = items.map(function(it) {
      ...
      return '<tr class="border-b border-gray-50"><td class="py-1" title="' + escapeAttr(it.item_name || '') + '">' + escapeAttr(it.item_name || '') + '</td>'
        ...
        // 2026-09-17 두 구멍을 다 메웠다 — 라우트가 base_unit·pack_size·stock_mode 를 싣고(routes/storageZones.ts
        //    `/:id/stock`), UOM_JS 가 이 페이지에도 주입된다(pages/storageZones.ts). 가드는 남겨 둔다:
        //    헬퍼가 없으면 종전 표기로 폴백한다(= 회귀 없음).
