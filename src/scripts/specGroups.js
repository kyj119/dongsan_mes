...
function sgpRenderDetail() {
  ...
  var rows = values.map(function(v) {
    ...
    return '<tr class="' + (inactive ? 'opacity-40' : '') + '">'
      ...
      + '<td class="px-3 py-1.5 text-left" title="' + sgpEsc(v.label) + '">' + sgpEsc(v.label) + '</td>'
  ...
  box.innerHTML =
    ...
    + '</div></div>'
    + '<div class="overflow-x-auto"><table class="w-full text-sm ds-table ds-table-striped ds-table-fixed">'
    + '<thead class="bg-gray-50 text-xs text-gray-600"><tr>'
    + '<th class="col-code px-3 py-2 text-left">코드</th>'
    + '<th class="col-name px-3 py-2 text-left">라벨</th>'
    + '<th class="col-qty px-3 py-2 text-center">정렬</th>'
...
function sgpRenderBases() {
  ...
  return '<div class="ds-card overflow-hidden mt-4">'
    ...
    + '<th class="col-qty px-3 py-2 text-center">변종수</th>'
    + '<th class="col-action px-3 py-2 text-center">액션</th>'
    + '</tr></thead><tbody>'
