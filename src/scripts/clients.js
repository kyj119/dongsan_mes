...
function displayClients(clients, pagination) {
    ...
    var rows = clients.map(function(c) {
        ...
        return '<tr class="border-b border-gray-100 hover:bg-blue-50/30 group ' + rowClass + '">'
            ...
            + '<td class="px-3 py-2.5 text-sm font-medium" style="color:#212529;" title="' + escapeHtml(c.client_name) + '">'
                + '<a href="/clients/' + c.id + '" class="hover:text-blue-600">' + escapeHtml(c.client_name) + '</a>'
            + '</td>'
            + '<td class="px-3 py-2.5">' + typeBadge + '</td>'
            + '<td class="px-3 py-2.5 text-xs text-gray-500" title="' + escapeHtml(c.representative || '') + '">' + escapeHtml(c.representative || '-') + '</td>'
            + '<td class="px-3 py-2.5 text-xs text-gray-500 tabular-nums" title="' + escapeHtml(c.phone || c.mobile || '') + '">' + escapeHtml(c.phone || c.mobile || '-') + '</td>'
