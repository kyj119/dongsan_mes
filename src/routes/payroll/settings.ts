...
settingsRouter.get('/tax-table/:year', async (c) => {
  ...
  const rows = await c.env.DB.prepare(
    `SELECT id, year, monthly_pay_min, monthly_pay_max, dependents_1, dependents_2, dependents_3, dependents_4, dependents_5, dependents_6, dependents_7, dependents_8, dependents_9, dependents_10, dependents_11 FROM income_tax_table WHERE year = ? ORDER BY monthly_pay_min, id LIMIT ? OFFSET ?`
