...
itemsRouter.get('/units-flag', async (c) => {
  try {
    ...
    const on = !!row && ['1', 'true', 'on', 'ON', 'TRUE'].includes(String(row.setting_value ?? '').trim())
    return c.json({ success: true, data: { forms: on } })
  } catch {
    return c.json({ success: true, data: { forms: false } })
  }
})
