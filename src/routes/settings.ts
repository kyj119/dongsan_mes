...
settingsRouter.get('/credit-policy', requireRole('ADMIN'), async (c) => {
  try {
    ...
    // 판정식·합성 전부 credit-helpers 가 정본 — 여기서 SQL 을 조립하지 않는다.
    //   (조립을 호출부에 두면 바인딩 위험 주석과 코드가 떨어져 2026-08-25 사고가 재발한다. 게이트=`npm run test:credit`)
    const impact = await queryCreditImpact(c, sim)
