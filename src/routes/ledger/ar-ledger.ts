/**
 * ledger/ar-ledger.ts — 거래처 원장 + 재무 요약 라우트 (accounts-receivable.ts에서 분리, 2026-06-12 대형파일 분할 5/5)
 *
 * 거래처 원장(client/:clientId·export/csv) · 정산(settlement) ·
 * 월별/마감/손익 요약(monthly-summary·closing-summary·profit-summary). 배럴에서 마운트.
 * ⚠️ 이동만, 로직 수정 0.
 */
...
arLedgerRouter.get('/client/:clientId/export/csv', async (c) => {
  try {
    const clientId = c.req.param('clientId')
...
arLedgerRouter.get('/settlement', async (c) => {
  try {
    const { startDate, endDate } = c.req.query()
...
arLedgerRouter.get('/profit-summary', async (c) => {
  try {
