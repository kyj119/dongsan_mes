 ...
 *
 * ⚠️★**감싸는 쪽은 이 서브쿼리보다 앞에 `?` 를 두면 안 된다.** SQLite 는 `?` 를 SQL **텍스트 순서**로 채우므로,
 *    바깥 SELECT 목록에 `?` 를 하나라도 넣으면 여기 params 가 통째로 한 칸씩 밀린다.
 *    2026-08-25 실측 사고: 정책 시뮬레이션이 바깥에 경고비율 `?` 를 두는 바람에 entity 필터 자리에 months(6)가
 *    들어가 `a.entity_id = 6` → adjustments 전량 누락 → 초과 37곳이 **108곳**, 3.55억이 5.52억으로 표시됐다.
 *    타입체크·빌드·스모크 전부 통과한다(SQL 은 문법상 멀쩡하다). **검증은 숫자 대조로만 잡힌다.**
 *    → 바깥에서 쓸 상수는 **인라인**하거나, 최소한 서브쿼리 뒤에 오도록 배치할 것.
 *
