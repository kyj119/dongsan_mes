/**
 * taxInvoices/issue.ts — 세금계산서 발행 라이프사이클 (taxInvoices.ts에서 분리, 2026-06-11 대형파일 분할 3/5)
 *
 * 직접발행(direct) / 주문기반 생성(POST /) / 발행(:id/issue) / 수정발행(:id/modify) / 취소(:id/cancel).
 * 바로빌 발행·세금계산서 write 경로. 배럴(taxInvoices.ts)에서 마운트. ⚠️ 이동만, 로직 수정 0.
 */
...
taxInvoicesIssueRouter.post('/:id/cancel', requireRole('ADMIN'), async (c) => {
  try {
    const id = parseInt(c.req.param('id'))
    const user = c.get('user')
...
export default taxInvoicesIssueRouter
