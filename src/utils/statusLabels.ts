...
// IDLE=gray 확정 (2026-07-22): 대기=중립. amber는 HOLD/QUOTATION 등 '주의' 계열 전용
export const EQUIP_STATUS_TONES: Record<string, string> = {
  RUNNING: 'green',
  IDLE: 'gray',
  MAINTENANCE: 'amber',
  BROKEN: 'red',
...
export const EQUIP_STATUS_ICONS: Record<string, string> = {
  RUNNING: 'fas fa-play',
  IDLE: 'far fa-clock',
...
window.MES_STATUS = (function(){
  ...
  var toneDot = { blue: 'text-blue-500', green: 'text-green-500', amber: 'text-amber-500', red: 'text-red-500', gray: 'text-gray-400' };
  var toneDotBg = { blue: 'bg-blue-500', green: 'bg-green-500', amber: 'bg-amber-500', red: 'bg-red-500', gray: 'bg-gray-400' };
  function pick(kind) {
    if (kind === 'card') return { l: card, t: cardTones, i: cardIcons };
    if (kind === 'equip') return { l: equip, t: equipTones, i: equipIcons };
    return { l: order, t: orderTones, i: orderIcons };
  }
  var api = {
    cardLabels: card,
    orderLabels: order,
    ...
    equipLabel: function(s){ return equip[s] || s; },
