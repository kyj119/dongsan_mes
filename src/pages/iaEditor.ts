...
//   P1b 원본 아카이브 · P3 시트 네스팅 · 모아찍기(임포지션) · 시안 검수(구 /workbench)
// Phase 7b-2(2026-07-28): '파일 처리' 뷰 제거 — 업로드·NAS 분석·처리설정 인스펙터·근사 미리보기.
//   소스 공급은 일러 MES 패널이 전담하고(등록분은 status='done'), 배치에 쓰이는 마감·돔보는
//   네스팅/모아찍기 패널이 자체 입력을 가진다(인스펙터 설정은 배치가 읽지 않았다).
//   대기함·에이전트 배지·세션 파일 탭은 뷰 밖 공통 소스바로 이동.
export function iaEditorPage(c: Context<HonoEnv>) {
  return renderPage(c, {
    ...
    pageContent: `
      <div class="mb-4">
        <p class="text-sm text-gray-500">일러스트레이터 <b>MES 패널</b>에서 등록한 대기물을 세션에 담아 → 시트 네스팅 · 여러 파일 모아찍기(임포지션) → EPS 출력 · 주문. <span class="text-gray-400">가공(단건 EPS 추출)은 패널이 담당합니다.</span></p>
      </div>

      <!-- 뷰 토글 -->
      <div class="flex gap-2 mb-4">
        <button id="iaeViewCanvas" class="px-4 py-2 rounded-lg text-sm font-medium border border-blue-500 bg-blue-50 text-blue-700"><i class="fas fa-layer-group mr-1"></i>네스팅/모아찍기</button>
      ...
      <!-- 공통 소스바 (뷰 밖): 대기함 · 에이전트 상태 · 세션 파일 탭 · 파일 상태줄.
           네스팅/모아찍기의 소스를 공급하는 영역이라 검수 뷰에서는 iaeSetView가 숨긴다. -->
      <div id="iaeSourceBar" class="mb-4">
        <div class="mb-2">
          <button id="iaeIntakeBtn" class="text-xs px-3 py-1.5 rounded-md border border-amber-300 text-amber-700 hover:bg-amber-50"><i class="fas fa-inbox mr-1"></i>모아찍기 대기함 <span class="text-amber-400">(일러 MES 패널 등록분)</span></button>
        ...
        <div class="ds-card flex flex-col">
          ...
          <!-- 파일 상태줄: 분석이 끝나지 않은 파일 안내 + 분석 취소 + 새로고침.
               팔레트는 status='done' 만 노출하므로 이 줄이 없으면 "왜 안 뜨지"가 된다. -->
          <div id="iaeFileStatus" class="px-3 py-2"></div>
        </div>
      </div>

      <!-- 네스팅 뷰 (동일 품목 시트 자동 배치 → 정적 미리보기 → EPS/주문). 자유드래그 없음. -->
      <div id="iaeCanvasView">
