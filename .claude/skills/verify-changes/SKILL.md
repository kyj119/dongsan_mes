---
name: verify-changes
description: 방금 고친 기능을 Playwright MCP로 실제 브라우저 인터랙션까지 검증. 스크린샷만 보고 "잘 되네요"가 아니라, 버튼 클릭·폼 입력·API 응답·에러 콘솔까지 체계적으로 확인. 범위 지정 없는 "검증해줘"·"확인해줘"·"테스트해줘"·"verify"의 기본 스킬. 변경과 무관한 전반 훑기는 smoke-test.
---
