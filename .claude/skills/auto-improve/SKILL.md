...
4. 발견 사항 분류:
   - 🔧 자동 수정 가능 → **새 worktree(`.\scripts\new-session.ps1 auto-improve-<날짜>`)에서** 수정 + build + 검증 → 그 브랜치에 커밋(메인 체크아웃에 dirty WIP 금지 — 다른 세션 `deploy:prod` 에 휩쓸린다)
...
- 문서 동기화 (sync-docs)
- escapeHtml 누락 추가
  (※ 인덱스 추가 마이그레이션은 2026-09-26 부터 **제안만** — 마이그는 prod 적용·번호 충돌·되돌리기가 사람 판단이다)

**자동 수정 금지 (반드시 제안으로)**:
- 새 기능 추가
- UI/UX 변경
- DB 스키마 변경 (인덱스 포함 — 모든 마이그레이션)
- 라우트 추가/삭제
- 비즈니스 로직 변경
- 기존 API 응답 형식 변경
