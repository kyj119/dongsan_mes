...
- **✅ 08-26 품목 명칭·계열 점검 + 분류 정비** — ★**TPM↔잉크테크 `item_group` 뒤바뀜**(껍데기가 TPM 그룹 차지·삭제) — 검색어 오염은 증상, 원인은 그룹. ★모달이 **규격을 안 그렸다**(230품목 동명) → 규격 열+제품/자재/상품 배지. ★**H2 49건 대부분 형제와 일관** — 소수 이탈 7건만 정정. 게이트 **G1·H4a**=형제 판정. **포맥스 36→MATERIAL** · **Extreme 수성 8→코스테크 통합·삭제**. 경위·롤백=`docs/analysis/2026-08-26-item-type-minority-fix.sql`·`-ink-unit-unification.sql`. 남은=**장비 1.59억·외주 1.90억(자산/용역 축)**
...
- **가공·재단 패널 흐름 재설계(설계 확정·구현 대기)**: ★1순위=**배포 축**(축2는 Z: 1곳=전PC 즉시인데 축3/4 셸만 PC별 수동) → 축2가 축3/4 자동갱신+**일러 재시작 안내**로 5축→2축. ★**출력완료 매칭이 0%다** — 8월 print_events 5,554건 중 카드매칭 0·작업파일 962건 중 주문코드 0. ★`rip_filename` 카드순번 **2자리** vs `resolveCard` **3자리** 불일치(`helpers.ts:303`↔`printEvents.ts:116`, 정규식 실행 확인) → **넓히는 쪽은 resolveCard**. ★cards 테이블 **0건**이라 중복은 아직 없음 → **카드가 파일명을 만든다(단일소스→파생)**, 패널 독립채번 금지. ★판짜기=20분/10조각(현수막 후가공 10배)=최우선 · 후가공 유지 · 네스팅 강등 철회. 남은=하루 판짜기 건수·카드축 개시 시점. spec=`2026-08-26-panel-flow-restructure`
