...
- **✅ [2026-07-02] 급여대장 단일 뷰 + 공제 가시성 (prod `8af1d04d`, 커밋 `75cbecc4`)**: compact 간단표·토글 폐기(용준님: "급여대장 아닌 폼 불필요") — 체크박스·상태·액션을 대장 descriptor(kind check/status/act, ui:true=CSV·인쇄 제외)로 통합. 공제 안 보이는 원인=표폭 1822px(0원 지급컬럼들이 공제 그룹을 화면 밖으로 밀어냄, prod 공제 상세데이터는 정상) → **전원 0 컬럼 자동 숨김**(`prLedgerActiveCols`, bold/always 제외)으로 ~1564px, 공제 6종 always 항상표시. 인쇄=화면컬럼 재생성·CSV=전체컬럼. Playwright 검증(선택3명·탭전환·공제계 뷰포트內)+apex 마커. +급여·4대보험 자금계획 (N명) 합산 표기(`f76dcba7`, prod `4f036e58`).
