...
> **✅ prod 데이터 2026-08-24 — 현수막 실사 6주치 소급 적재(5/22~6/29) + 전사잉크 8주치 + ★롤당 야드 정정** — ①수기 시트 6장 판독 적재(회차 #13·#14·#15 신설, #5·#6·#7 보강) → 실사 계열이 **5/22~8/07 12주**로 늘었다. tally 범례=`−`1 `T`2 `干`3 `⺊F`4 `正`5(5/29 시트가 「140 正−(6)」로 숫자 병기 → 확정). ②★**AQ2-070·090 롤당 130→250 정정**(5/22·6/12·6/29 시트 명시 + 매입 산술 13,796÷55=250.8·4,870÷19=256) — 기존 적재분 12행 소급, 동산 재고평가 3,443만→**3,847만**. ③전사출력실(z3) 회차 8개 신설, KM 열전사잉크 4색 적재(승인 안 함 — 재고 보정 목적 아님). ④소모량 API를 **품목별 「그 품목을 실제 센 회차」** 기준으로 변경 — 현수막 전용 회차가 시트 소모 구간을 끊던 문제. 검증=드리프트 없음·prod smoke **116/116**. 백업=`_bak_0824_aq_rollsize`. 남은=**AQ2-090 6/18 중복 라인(13,796yd×2, 676만) 원장 대조** · KM 잉크 `is_active=0` 여부 · 코스테크 전사잉크 단가(RM-I0067~70, 현재 0)
...
- **가공·재단 패널 UI 다이어트(피드백 9번)**: 구현 완료·게이트 통과(panel:smoke 111·cut:smoke 371·세로나열 0@260px) — **미커밋·미배포**. 배포=`ia:deploy`+각 PC 패널 설치(축3→4)
- **도련 회색 오염(피드백 10번)**: 원인 하네스 재현 완료 — 회색 공급원=ink 굽기 AA(mes-cut-host.jsx:2073)+도련 축소 스무딩(cut-main.js:1085). 수정은 **회색 난 실파일(또는 결과창 도련 방식 문구) 확보 후** — 용준님 선택
