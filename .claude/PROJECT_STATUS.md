...
- **✅ 08-26 품목 명칭·계열 점검 — 선택기 규격 열 + `audit:items` G·H축** — ★**TPM ↔ 잉크테크 `item_group` 이 뒤바뀌어** 있었다(빈 껍데기가 TPM 그룹 차지·하드삭제) — 검색어 오염은 증상, 원인은 그룹. ★모달이 **규격을 안 그렸다**(230품목 동명) → 규격 열+**제품/자재/상품 배지**. 품명 불변(이카운트 대사). ★**H2 49건 대부분은 형제와 일관** — 계열 소수 이탈 **7건만** 정정. 게이트 G1·H4a=형제 판정(142→8·49→7). 경위=`docs/analysis/2026-08-26-item-type-minority-fix.sql`. 남은=**장비 1.59억·외주 1.90억(자산/용역 축)**
...
- **가공·재단 패널 흐름 재설계(설계만·승인 대기)**: ★1순위=**배포 축**(축2는 Z: 1곳=전PC 즉시인데 축3/4 셸만 PC별 수동 → 개선이 도달 못 함) → 축2가 축3/4를 자동 갱신 = 5축→2축. 그 밖 결함=오프셋·도련 칸 2벌 중복·배율 미기록·두 탭 폼 상이. 배율 자동판정=규격÷실측, **여백 빼고 재고 W·H 비율 어긋나면 정지**. ★후가공은 파일 기하를 바꾸므로 유지(대체 불가) · ★네스팅 강등 철회(빈도 4.3%지만 **시간 최다**). 미결 4건(CEP 재시작 요부·파일명 자동화·소요시간·폴더 자동화 범위). spec=`2026-08-26-panel-flow-restructure`
