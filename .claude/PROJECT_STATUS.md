...
### [GitHub 이슈 백로그]
- **✅ 처리완료·배포·close(2026-06-05)**: #355·356·335·351·352·357·349 + **#343·#345·#353·#354·#346** — dead-filter(생산보드 품목군/원가 자동차감/메시지 날짜/활동로그 사용자/매입 매칭상태)·CSV(cashSchedule·검수)·검수목록 공급업체 드롭다운+페이지네이션·연차 부서필터·휴가 날짜범위·불량률→검수 드릴다운 +**미사용수당 응답 정합버그 수정**(d.employees/total). 커밋 `0c04fad`, prod 배포(`webapp-9i0.pages.dev`), 9페이지 실브라우저 스모크 통과(콘솔에러 0, 신규 API 4종 200)
- **개선(미착수)**: #344(포털 셀프서비스 3건)
- **N+1**: #350 payroll batch-create **loop-invariant hoist만 선반영**(무위험·결과값 불변·커밋 `0c04fad`). **잔여(#341·#350 집계 재작성/prefetch/batch)=실서버 정합 검증세션 필요** — 샌드박스 런타임 검증 불가(D1·egress 차단)
