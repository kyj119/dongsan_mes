...
- **▶ [2026-06-30] 품목 미등록 갭 감사 + 판재 등록 (worktree `session/master-banner`)**:
  ...
  - **✅ 자재/원단·상품 18**(0418·0419): 자재/원단 MATERIAL(열전사원단·타포린·그라스화이바·원형나무·각목 + 200g코팅지 3(판매용·NONE) + 3M인쇄비닐 + SPM011M(무광·재고1롤 소진용)) + 상품 GOODS(좌우보필형 액자·A3자석액자·국기함3등급·청사초롱·탁상용국기·지퍼가방). **스킵**: 솔벤캔버스(미취급)·저밀도원단(기등록 동일)·LD59THG(LD59HTG 오타).
  - **★남은 = 간판 + 단가만**: ①**간판 구성요소**(채널/프레임/돌출/LED·SMPS/갈바/각관 ~3.19억 = **BOM·조립견적 설계 선행**, brainstorming 필요·마스터에 `간판` 분류 없음) ②**단가**(전 품목 base_price=0, 전역 과제). 참고(보류): 가로기 깃발(커스텀 추정)·태극기/근조 본염·코팅지 외 선명 명칭매핑.
