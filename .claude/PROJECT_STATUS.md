...
> **✅ 커밋 2026-08-24 · `ff40ac77`(main 미push·미배포) — 분석·대시보드 수치 감사 ‹가› 범위 수정** — ★기초채권 E1-OPEN/ICM 189건 5.32억(2025-12-31)이 매출 집계에 섞임: 대시보드 TOP거래처·월별매출 12개월 창·period-comparison 전부 `voucherOrderSql()` 제외. ★마진 매출기준 `unit_price×qty`→`oi.amount`(AREA 라인 −8.83억/−21.8% 과소 해소). 잔액 열=파생잔액 교체. 검증=로컬 smoke 116/116. 남은=마진탭(total_cost 전량 0)·디자이너탭(전량 관리자)·손익계산서탭 구조 결정 + 죽은 엔드포인트 7개·UTC 정리
