...
- **품목 단가 배선 3종 — 코드 완료·미배포·마이그 미적용(2026-08-11)**: prod 실측으로 전제가 두 번 뒤집힌 건. ①MES 실사용 0(8월 주문 0·견적 0·카드 0, 활동 12건/1명 — 살아있는 건 자동수집 4종뿐) ②단가 공백이 원인(판매품목 880 중 773이 0원) ③구조는 완비·데이터만 빔(단가표 11테이블 전부 0건, `journal_entries` 는 코드 참조 0). 완료=최소 청구 규격 배선(서버+클라+견적 4곳 통합·자체테스트 22건 `npm run test:orderline`)·특약단가 조회 배선(주문서만 빠져 있었음)·AREA 환산 근본수정(`prices.ts`). **결정 대기 = 마이그 0529 적용(155품목)·배포**. 잔여 = C등급 56 + N 165 + X 387(용준님 판단) · ⚠️**이관 주문 수정 시 에누리 오염 잠복**(미해결) → [[design-order-line-billing-rules]] · [[feedback-imported-unit-price-semantics]]
