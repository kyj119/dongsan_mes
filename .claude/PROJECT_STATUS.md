...
- **08-24 키트 [1] 에 취소 실측 3라운드 추가 (`--probe --cancel-test`) — 배포 대기** — R1 정상→R2 전송중 취소(PC)→R3 전송후 취소(프린터), 라운드별 시각 앵커+직전 스냅샷 diff 교차분석(공용 로그 / 취소 전용 매체 / 취소 무기록 판정). ★**스냅샷 확장자 화이트리스트를 노이즈 블랙리스트로 반전** — Flora `print_rec.dat` 가 안 보이던 08-19 UV 탐지 실패의 직접 원인(`MarkerProbe.cs:335`). 통합테스트 7/7 PASS. 남은=**`make-kit.ps1` 로 Z: 키트 갱신**(수동 배포축) → [1] 대상 4대(HYB-3200·SOLV-3200·출력실1·2)
