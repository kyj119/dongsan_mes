...
> **✅ prod 데이터 2026-08-24(2) — 뭉친 매입 전표를 거래처원장으로 분해**(케이엠·서울경금속·다다모아·명성연사) — ★정체는 **원장 총계로 확정**(금액이 열쇠)·일자별 전표 재분할(합계 불변). ★KM 열전사 「7월 매입 중단」은 `JOIN items` 집계 오류. ★LED4U 원장 밖 매입 1,825만 실재(잔액 대사). 볼로프=3개들이 포장가 정정. 검증=smoke 116/116. 경위=ARCHIVE §2026-08-24 전사잉크. 남은=미연결 **328라인 2.5억**(오후 실측 정정·확실 15건 연결 `_bak_0824_unlink15`, 목록=`docs/analysis/2026-08-24-unlinked-purchase-lines.csv`)·KM 8색 비활성 시점·준석·에코(질문 제출)
