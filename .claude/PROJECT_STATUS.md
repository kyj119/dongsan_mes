...
### [GitHub 이슈 백로그 (2026-06-05 세션 후속)]
- **✅ 처리완료·배포(2026-06-05)**: #355(여신 CHECK 마이그0300 prod적용)·#356(멀티테넌시 격리 6모듈)·#335(저장형 XSS)·#351(hr orphan 크래시)·#352(현금영수증 필터)·#357(업로드 검증) + #343/#345 부분 → **이슈 close 대기**
- **개선(미착수)**: #354(검수목록 UX·필터/CSV), #353(dead-filter 6건), #346(연차 부서필터·불량률 드릴다운), #344(포털 셀프서비스 3건)
- **N+1(검증세션 필요)**: #341·#350 — 재무/HR 핫패스, 실서버 출력값 정합 검증 필수
