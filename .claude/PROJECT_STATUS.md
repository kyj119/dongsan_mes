...
- **⛔ [2026-07-03] 출고관리 고도화 v2 P1~P4 — 구현·로컬검증 완료, prod 배포 대기 (spec `docs/superpowers/specs/2026-07-03-shipping-verification-consolidation-v2.md`, worktree `session/shipping-verify` 커밋 `cd492fd6`+머지 `487db433`)**: 용준님 확정 = 범위 나(P1~P4)·**부분출고 전면 불가(전량 출고만)**. ①검수: `shipment_checks`(0439)+체크리스트 API 2종+PC 검수 모달(칩 n/m)+**하드 게이트**(bulk-ship 미완성카드 사전차단·silent 부분출고 경로 제거·POST /shipments 전량검증)+소프트 게이트(미검수 경고) ②명세서: 검수 체크지(QR→/pack)·납품명세서(복수 주문=통합·소계) ③합배송: 후보=같은법인 복수주문+미출고 전체(waiting 칩)·merge 납품일 완화+0438 예약 동기·'합배송 대기 →MM/DD' 배지 ④/pack 모바일 검수(QR 스캔·탭 즉시저장, 권한 0439). 검증=tsc·build·node --check·로컬 D1 E2E 11/11+게이트/머지 스위트 전부 통과. **⛔블로커: prod D1 0439 적용이 권한 게이트에서 차단 → push·배포 보류**(0438 사고 패턴 방지: /daily가 shipment_checks 참조라 마이그 없이 코드 도달 시 /shipments 500). **배포 런북=spec §배포 절차** (①0439 --remote ②push ③deploy:prod ④apex 마커). 브랜치는 main 3커밋(storageZones·production 필터·spec docs) 머지 완료=superset.
