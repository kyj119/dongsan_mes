...
- **🟡 카드/주문 상태모델 단일화 (#1·2·3) — 코드완료·로컬E2E검증·prod 미배포**: 이중 상태모델(PRINT_PENDING vs PRINTING+rip_status) 충돌이 #1·2·3 공통원인. 4 Phase 구현(P1 보드버킷 status기준→#2 / P2 PRINTING=LogWatcher단일화→#3 / P3 유통 즉시SHIPPED→#1 / P4 레거시정리 마이그0298). 로컬 dev환경 복구(0089→0298 클린적용)+실라우트 E2E PASS(#1 bulk-ship→SHIPPED, #2 보드 PRINT_PENDING 노출). **⚠️ 최대미결: prod cards.status CHECK에 PRINT_PENDING 있는지(0284 적용?) 미확인 → 미적용 시 #2 진짜원인=INSERT 실패라 마이그 prod적용이 실제 fix.** ▶ 배포플랜: prod CHECK 확인→마이그0298(+미적용분) 적용→deploy:prod→#1·2·3 검증. 설계 → `docs/superpowers/specs/2026-06-05-status-model-unification.md`, 메모리 `session-context`.
