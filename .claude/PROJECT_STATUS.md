...
- **✅ 08-26 품목 명칭·계열 점검 — 선택기 규격 열 + `audit:items` G·H축 (배포 `cbf5737a`)** — ★**TPM ↔ 잉크테크 `item_group` 이 뒤바뀌어** 있었다(빈 껍데기가 TPM 그룹 차지) — 검색어 오염은 증상이고 원인은 그룹. ★품목검색 모달이 **규격을 안 그리고 있었다**(230품목이 형제와 동명·포맥스 24는 코드로만 구분) → 규격 열 + **제품/자재/상품 배지**. 품명은 안 고침(이카운트 대사=품명 매칭). G1 게이트=형제그룹 판정(142→87→**8건 오탐 0**). 껍데기 `RM-I0011~14` 하드삭제(38테이블 스캔=inventory 4행·수량 0). 남은=H2 49건
