...
- **▶ [2026-06-30] 품목 미등록 갭 감사 + 판재 등록 (worktree `session/master-banner`)**:
  ...
  - **✅ 판재 dual 등록**(0412, 58 SKU): 포맥스·아크릴·폼보드·스카시·광확산PC·알마이트 × 실주문 두께 × **색상별 SKU**. **보드 MATERIAL 29**(원자재·BOARD·장·dual·sheet_spec 4x8) + **UV 평판 출력 PRODUCT 29**(AREA, 자작나무 패턴) + 1:1 링크. prod 검증 29/29/29. 단가 보류.
  - **남은 미등록(우선순위)**: ①**게양/부속**(깃대·까치발·로프·수기대·자수삼발이=GOODS, 배너부속처럼) ②**간판 구성요소**(채널/프레임/돌출/전기=BOM·조립견적 설계 선행, 큰 프로젝트·brainstorming) ③자재/원단(열전사원단·솔벤캔버스(원단미파악)·족자봉·엠보) ④상품/액자(좌우보필형·국기함). 단가=전역 보류.
