...
### [기성품/유통 즉시출고] — Phase 1+2+3 완료·배포됨, 데이터 지정만 남음
- **남은 작업(데이터)**: 태극기 호수별 등 **기성 PRODUCT를 품목 관리 UI '기성품' 토글로 지정** 필요. GOODS/MATERIAL은 자동(production_required=0). 미지정 시 해당 PRODUCT는 기존대로 제작(카드 생성).
- 코드/마이그(0285·0286) 전부 prod 반영. 기성/유통 = 카드 미생성·즉시 출고가능·출고 시 재고차감(음수 허용)·주문서 재고부족 경고.
