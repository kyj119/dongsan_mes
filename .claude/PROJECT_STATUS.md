...
- **▶ [2026-06-29] 품목 마스터 배너류 등록 (worktree `session/master-banner`)**:
  - **출력 배너 6종(7품목) prod 적용·검증·push 완료** (마이그 **0398~0403**, dep 불요=데이터 only): 윈드(TRW)·워킹(TRWK)=전사·니트 / 실내(AQ-INB)·미니(AQ-MNB)=수성·패트 / 자이언트(TRGI-PO/ME)=전사·폰지/매쉬 2제품 / 솔벤패트(SV-PATB)=솔벤·패트. **전부 AREA·is_active=1·매입X**. prod D1 0→7 검증(필드 정확, 0356/0360 패턴 동일).
  - 결정: 배너=출력 PRODUCT + 거치대(폴/물통/스파이크)=부속 GOODS 주문라인 / 형(S/F/H)=주문옵션 / 규격=AREA 통일 / 가로등=규격확대 / 실외·철제=물통SET GOODS.
  - **⚠️ 공통 후속(보류)**: ①원단 폭+product_materials 링크(니트 미등록·폰지/매쉬/패트 기등록·솔벤급 패트 미등록) ②형 옵션 주문서 코드 ③단가 전부 0.
  - **남은 등록**: 태극기배너(전사인쇄)·외국기배너(별도제품)·**부속품 GOODS 26종**(물통SET/폴세트/배너대/삼발이/브랏켓)·가로등(0360) 표준규격 확대. 원천분석=에이전트 보고(배너 1,795라인).
