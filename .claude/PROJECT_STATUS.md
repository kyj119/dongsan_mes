...
- **08-11 Z: 작업파일→주문서·카드 자동생성 타당성 백테스트 = 성립**(정본 `docs/order-file-matching/`) — 이카운트 **주문서현황**(판매현황 아님·접수시점)을 정답지로 오프라인 채점(실시간 연결·API 불요): **그룹 배정 90.7%·모호 2.0%·규격 80.8%·수량 79.0%**, 미청구(재출력·수정·샘플) 7.3%. ★**조인 단위=파일이 아니라 「주문 그룹」**(파일 단위면 45.8%·모호 30% — 파일 N개=1라인이라 grain 불일치) · 순번 결정적 조인 **기각**(3.5%, 파일=출력순/이카운트=전표입력순) · 규격 복원=**BoundingBox×배율**(제품유형별 이산값, 스냅 100%) · 거래처 별칭 59종 자동학습. 남은: **스캔 축 확대**(☆간판·★시트CUT·메인2~4 — 주문 커버리지 58.8%의 주원인) · 초안 생성 PoC(`designer_intakes` 적재, 하류는 이미 prod) · `벌` 단위 확정
