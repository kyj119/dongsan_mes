...
- **▶ [2026-06-30] 품목 미등록 갭 감사 + 판재 등록 (worktree `session/master-banner`)**:
  ...
  - **남은 미등록**: ①**간판 구성요소**(채널/프레임/돌출/LED·SMPS/갈바/각관=BOM·조립견적 설계 선행, 최대 3.19억·brainstorming) ②자재/원단(열전사원단·솔벤캔버스(원단미파악)·엠보) ③상품/액자(좌우보필형 액자·국기함). 단가=전역 보류. 참고(보류): 가로기 깃발(커스텀 추정)·태극기/근조 본염(데이터 확인 후).
