...
- **✅ [2026-07-02] 급여대장 단일 뷰 + 2단(지급/공제) 구조 (prod `cb489613`, 커밋 `75cbecc4`→`7aacd7b1`)**: ①compact 간단표·토글 폐기(용준님: "급여대장 아닌 폼 불필요") — 체크박스·상태·액션·근태마크를 대장에 통합. ②공제 안 보이는 원인=표폭 1822px(prod 공제 상세데이터는 정상). 1차 0컬럼 자동숨김 → 용준님 "숨기지 말고 다 보이게, 2~3단 구조로 상하 스크롤만" 피드백으로 **2단 밴드 구조 확정**(`prBandHtml`): 슬롯당 위=지급(기본급~지급계)/아래=공제(국민연금~공제계), 직원당 2행, 신원·실지급·상태·액션 rowspan=2, 0원 흐림(`.z`). 폭 1534px=가로스크롤 제거. 소계/합계도 2행 쌍. 인쇄=2단(체크/상태/액션 제외)·CSV=평면 전체컬럼(`LEDGER_MAIN_COLS`). Playwright 검증(전항목 헤더·noHorizScroll·선택3명·탭전환)+apex 마커. +급여·4대보험 자금계획 (N명) 합산 표기(`f76dcba7`, prod `4f036e58`).
