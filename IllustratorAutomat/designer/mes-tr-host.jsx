...
function mesTr_parse(payload) {
  ...
  for (i = 0; i < recs.length; i++) {
    ...
    // 선(접는선·끈고리) `L:idx,x,y,len,color[,kind,corePt,haloPt]` · 하도매 `H:idx,cx,cy,r,color[,haloPt]`
    //   색 k(검정)|w(백)|x(검정 심 + 백 테두리). 0.8.0: 두께는 패널(정본 plate-rules.marks)이 준다 — 없으면 종전 4pt 심 + 1pt 테두리.
    if (k === 'L') { out.loops.push({ i: parseInt(n[0], 10), x: parseFloat(n[1]), y: parseFloat(n[2]), len: parseFloat(n[3]), color: n[4] || 'x', kind: n[5] || '', core: parseFloat(n[6]), halo: parseFloat(n[7]) }); continue; }
    if (k === 'H') { out.holes.push({ i: parseInt(n[0], 10), cx: parseFloat(n[1]), cy: parseFloat(n[2]), r: parseFloat(n[3]), color: n[4] || 'x', halo: parseFloat(n[5]) }); continue; }
...
function mesTr_makePlate(payload) {
  ...
  try {
    ...
    for (i = 0; i < p.loops.length; i++) {
      ...
      // ★두께는 패널이 준다(정본 plate-rules.marks.line — 0.8.0). 안 주면 종전 4pt 심 + 1pt 테두리.
      var core = (lp.core > 0) ? lp.core : 4;
      var halo = (lp.halo > 0) ? lp.halo : 1;   // 삼항 둘을 한 문장에 두지 않는다(audit:jsx-ternary — 이 엔진은 왼쪽 결합)
      if (lp.color === 'x') {
        var under = lyArt.pathItems.add();
        under.setEntirePath([[x0, y0], [x1, y0]]);
        under.filled = false; under.stroked = true; under.strokeColor = cmyk(0, 0, 0, 0); under.strokeWidth = core + halo * 2;
        mkKey++;
      }
      var ln = lyArt.pathItems.add();
      ln.setEntirePath([[x0, y0], [x1, y0]]);
      ln.filled = false; ln.stroked = true; ln.strokeColor = (lp.color === 'x') ? cmyk(0, 0, 0, 100) : markCol(lp.color); ln.strokeWidth = core;
      mkLoop++;
    }
    for (i = 0; i < p.holes.length; i++) {
      ...
      if (hl.color === 'x') { el.stroked = true; el.strokeColor = cmyk(0, 0, 0, 0); el.strokeWidth = (hl.halo > 0) ? hl.halo : 1.5; mkKey++; }
