...
var MESCUT_VERSION = 'CUT-CEP-0.59.0';  // 0.59.0 = ★등록 후 수신 확인(2026-09-27) — nestRegister 가 등록 폴더 이름(names=)을 돌려주고 mesCut_ingestState 가 에이전트 표식(.ingested/.rejected)을 센다. 잃는 것 없음(반환에 키 추가 · 조회 함수 추가).
 ...
 */
/**
 * ★굳히기 PDF 에 **마크를 넣지 않는다**고 명시한다 (0.52.0 · 2026-09-23 실기).
 *   증상 = 판 위 조각 모서리마다 검정 「집게」 모양. 그건 일러 재단 표시(트림 마크)의 모양 그대로다.
 *   `PDFSaveOptions` 는 명시하지 않은 값을 **그 PC 일러의 마지막 PDF 프리셋**에서 물려받는다
 *   (0.45.0 의 `viewAfterSaving` 과 같은 축). 디자이너 PC 는 인쇄용 프리셋(재단 표시·등록 마크 ON)을
 *   쓰기 마련이라, 굳힌 PDF 페이지에 마크가 들어가고 그 PDF 가 조각으로 임베드돼 **판에 그대로 찍힌다.**
 *   개발기 프리셋은 마크 OFF 라 **재현되지 않는다** — PC 별 상태 함정([[feedback-log-encoding-per-pc]] 형태).
 *   도련 여백(bleed)도 같은 이유로 0 으로 못박는다 — 프리셋 도련이 붙으면 페이지가 커져 조각 경계가 밀린다.
 *   ⚠️ 굳히기 PDF 저장이 **두 곳**이다(격자·단건). 한 곳만 고치면 그 경로에서만 마크가 사라진다 → 공통 함수.
 */
function mesCut_pdfNoMarks(po) {
    try { po.trimMarks = false; } catch (e1) { /* ignore: 일러 버전에 없는 속성 — 있는 것만 끈다 */ }
    try { po.registrationMarks = false; } catch (e2) { /* ignore: 위와 같음 */ }
    try { po.colorBars = false; } catch (e3) { /* ignore: 위와 같음 */ }
    try { po.pageInformation = false; } catch (e4) { /* ignore: 위와 같음 */ }
    try { po.bleedLink = false; } catch (e5) { /* ignore: 위와 같음 */ }
    try { po.bleedOffsetRect = [0, 0, 0, 0]; } catch (e6) { /* ignore: 위와 같음 */ }
    return po;
}
 ...
 */
/**
 * ★「클립을 넓혔다」와 「도련이 생겼다」는 다른 질문이다 (2026-09-17 실기).
 *
 * 이 함수는 넓힌 **클립 사각의 개수**를 돌려주는데, 호출부는 그 값이 0보다 크면 도련이
 * 된 것으로 보고 픽셀·단색 경로를 건너뛴다. 그런데 클립 확장이 실효가 있으려면
 * **클립 밖에 감춰진 그림이 실제로 있어야** 한다 — 배치 이미지가 클립에 딱 맞게 잘려 있으면
 * (보통의 경우다) 클립만 커지고 드러날 것이 없어 **도련이 0인 채로 성공으로 세어진다**.
 * 실기 2026-09-17: 조각 6개 중 5개가 이 경로로 가서 「클립 확장 5개(무손실)」로 보고됐는데
 * 실물에는 반 이상 도련이 없었다. §조용한 격하 — 격하를 성공으로 세고 있었다.
 * `OffsetPath v22`(0.39.0)에서 「먹었는지 검산한다」로 고친 것과 같은 형태다.
 *
 * → 넓히기 **전에** 자식 아트가 사방으로 b 만큼 뻗어 있는지 재고, 아니면 **넓히지 않는다**.
 *   넓히지 않으면 n 이 0이라 호출부가 픽셀 도련으로 내려간다(원래 정본 경로).
 * ⚠️사방 전부를 요구한다 — 한 변이라도 모자라면 그 변은 도련이 없는데, 클립을 넓혀 버리면
 *   그 사실이 가려진 채 픽셀 경로도 못 타게 된다. 부분 성공은 여기서 가장 나쁜 결과다.
 * 비용 = 조각당 자식 경계 읽기 몇 번(µs). 느려지는 부분은 검산이 아니라, 여태 건너뛰던
 *   조각들이 이제 **실제로 픽셀 도련을 하기 때문**이다 — 그게 원래 했어야 하는 일이다.
 * @returns 실제로 넓힌 클립 수 · `skipped` 는 전역(`MESCUT_CLIPSKIP`)에 누적해 결과에 싣는다
 */
var MESCUT_CLIPSKIP = 0;   // 「넓혀도 나올 그림이 없어서」 건너뛴 클립 수(판마다 초기화)
/**
 * EPS 저장 시 **폰트를 임베드해야 하나** (2026-09-17 · A0 0.9.0 정정의 형제 스윕).
 *
 * ★왜 — 두 EPS 저장 지점 모두 `embedAllFonts = true` 가 **하드코딩**돼 있었다. 그런데 판(sheet)에
 *   올라가는 조각은 PDF 로 굳혀 배치한 것이고 칼선·돔보는 패스라, **임베드할 폰트가 대개 없다**.
 *   true 는 그럴 때 일러에게 「문서 폰트를 전부 열어라」를 시키는 순수 낭비다 — 폰트가 **2,159개**
 *   깔린 실기 PC 에서 저장마다 그 값을 치르고 있었다(A0 에서 같은 줄을 고쳐 자원 고갈이 잦아들었다).
 *   실기 2026-09-17 「재단 주문등록이 시간이 좀 걸린다」의 유력 원인이다.
 * ⚠️**재단은 A0 와 달리 아웃라인을 보장하지 않는다** — A0 는 저장 전에 텍스트를 전부 아웃라인하지만
 *   여기는 판에 얹힌 아트를 그대로 저장한다. 그래서 「안 보이면 없다」로 단정하면 안 된다:
 *   ①살아 있는 `textFrames` 가 하나라도 있으면 임베드 ②**링크/미임베드 아트(`placedItems`)가 있으면
 *   그 속은 들여다볼 수 없으므로** 임베드 ③세다가 실패해도 임베드.
 *   굳히기 경로(0.32.0)는 PDF 를 배치한 뒤 **임베드**하므로 정상 판에서는 둘 다 0 이 되어 건너뛴다.
 *   빠뜨리면 RIP 에서 폰트가 대체돼 **글자가 바뀐다** — 느린 것과 틀린 것 중에는 느린 쪽이 낫다.
 */
function mesCut_needFontEmbed(doc) {
    try {
        if (doc.textFrames.length > 0) return true;
        if (doc.placedItems && doc.placedItems.length > 0) return true;  // 속을 못 보는 아트 = 임베드
        return false;
    } catch (e) { return true; }   // 셀 수 없으면 안전한 쪽(임베드)으로
}

function mesCut_vecGrowClips(items, bleedMm) {
    var n = 0, PT = MESCUT_PT_PER_MM, b = bleedMm * PT;
    // 클립 그룹 안에서 **클립 패스가 아닌** 자식들의 합집합 경계 = 실제 그림이 뻗은 범위
    function artReach(grp, clipPath) {
        var u = null;
        for (var i = 0; i < grp.pageItems.length; i++) {
            var ch = grp.pageItems[i];
            if (ch === clipPath) continue;
            var gb;
            try { gb = ch.geometricBounds; } catch (eA) { continue; } // 경계를 못 읽는 자식은 없는 셈 친다
            if (!u) u = [gb[0], gb[1], gb[2], gb[3]];
            else {
                if (gb[0] < u[0]) u[0] = gb[0];
                if (gb[1] > u[1]) u[1] = gb[1];
                if (gb[2] > u[2]) u[2] = gb[2];
                if (gb[3] < u[3]) u[3] = gb[3];
            }
        }
        return u;
    }
        ...
        if (clipped) {
            for (var c = 0; c < it.pageItems.length; c++) {
                ...
                try {
                    ...
                    // ★검산 — 사방으로 b 만큼 감춰진 그림이 있나. 없으면 넓혀도 빈 자리만 는다.
                    var a = artReach(it, ch);
                    if (!a || a[0] > g[0] - b || a[1] < g[1] + b || a[2] < g[2] + b || a[3] > g[3] - b) {
                        MESCUT_CLIPSKIP++;
                        continue;
                    }
...
/**
 * ★사각 빠른 경로 ③ 띠 도련 배치 (0.54.0) — 패널이 만든 띠 4장(`mes_cut_bleeds_<idx>_<t|b|l|r>.png`)을
 *   조각 둘레에 액자처럼 두르고 아트 **뒤로** 보낸다. 모양·색은 전체 PNG 도련(`mesCut_bleedPlaceItem`)과 같다
 *   (패널 하네스 `cut:rectfast` 가 픽셀 동일을 대조한다). 다른 것은 **면적이 아니라 둘레만큼의 픽셀**이라는 것뿐이다.
 *   띠끼리·띠와 아트는 잉크 쪽 d 만큼 겹친다 — 이음매가 안 생기게. 겹친 칸은 같은 값이고 아트에 가려진다.
 *   기준은 bleedPlaceItem 과 같다: 회전 **전** 틀에서 조립 → 회전 → 잉크 경계 **중심**에 맞춘다.
 * @param sp {W,H,g,d} mm (잉크 W×H · 도련 g · 띠 깊이 d — 패널이 픽셀에서 환산한 값을 그대로 쓴다)
 */
function mesCut_bleedPlaceStrips(doc, layer, copy, idx, sp, rotDeg) {
    if (!copy || !sp || !(sp.W > 0) || !(sp.H > 0) || !(sp.g > 0)) return false;
    var PT = MESCUT_PT_PER_MM, W = sp.W, H = sp.H, g = sp.g, d = sp.d || 0;
    var dir = Folder.temp.fsName.replace(/\\/g, '/') + '/mes_cut_bleeds_' + idx + '_';
    // 회전 전 틀(mm · y 아래로 · 원점 = 잉크 왼쪽 위)
    var R = { t: [-g, -g, W + 2 * g, g + d], b: [-g, H - d, W + 2 * g, g + d], l: [-g, 0, g + d, H], r: [W - d, 0, g + d, H] };
    var sides = ['t', 'b', 'l', 'r'], k;
    for (k = 0; k < 4; k++) { if (!(new File(dir + sides[k] + '.png')).exists) return false; }
    var bb = mesCut_inkBounds(copy);
    if (!bb) { try { bb = copy.visibleBounds; } catch (eB) { return false; } }
    var grp = null;
    try {
        ...
        for (k = 0; k < 4; k++) {
            var rc = R[sides[k]];
            var pi = grp.placedItems.add();
            pi.file = new File(dir + sides[k] + '.png');
            pi.width = rc[2] * PT; pi.height = rc[3] * PT;
            pi.position = [rc[0] * PT, -rc[1] * PT];      // position = 왼쪽 위(y 위로)
        }
        if (rotDeg) grp.rotate(-rotDeg);                   // Konva CW → 일러 CCW (조각과 같은 규칙) · 중심 기준
        var gb = grp.visibleBounds;
        grp.translate(((bb[0] + bb[2]) / 2) - ((gb[0] + gb[2]) / 2),
                      ((bb[1] + bb[3]) / 2) - ((gb[1] + gb[3]) / 2));
        grp.zOrder(ZOrderMethod.SENDTOBACK);               // 도련은 원본에 가려야 한다
    } catch (e3) { try { if (grp) grp.remove(); } catch (e4) { /* ignore: 임시 개체·파일 정리 — 이미 지워졌거나 참조 무효 */ } return false; }
    // ★링크를 끊는다 — temp 의 PNG 는 지워진다. embed 는 참조를 무효화하므로 **여기가 마지막**이다(bleedPlaceItem 과 같다).
    for (k = grp.placedItems.length - 1; k >= 0; k--) {
        try { grp.placedItems[k].embed(); } catch (e5) { /* ignore: 임베드 실패해도 배치는 성공 — EPS 저장이 링크를 품는 경로가 있다(2026-08 실측) */ }
    }
    return true;
}
...
/**
 * ★조각별 잉크 경계(mm, 문서 좌표 · y 아래로 증가) — 조각 번호용 (0.51.0). **nestBegin 다음에** 부른다.
 *   `mesCut_nestSizes` 와 같은 순서·같은 측정(`mesCut_inkBounds` = 클립 ∩ 콘텐츠)이라 인덱스가 1:1 이다.
 *   크기만으로는 줄(위→아래)·순번(왼→오른)을 못 매기므로 위치를 따로 준다 — nestSizes 의 반환 형식은
 *   패널 파서가 `x` 로 가르므로 건드리지 않고 함수를 따로 둔다(구 패널과 호환).
 * 반환 = 'ok;<x>:<y>:<w>:<h>,...' (소수 1자리 mm)
 */
function mesCut_nestBounds() {
    if (!MESCUT_NEST_ITEMS || !MESCUT_NEST_ITEMS.length) return 'ERROR 대상 없음 (nestBegin 먼저)';
    var out = [], PT = MESCUT_PT_PER_MM;
    for (var i = 0; i < MESCUT_NEST_ITEMS.length; i++) {
        var b = null;
        try { b = mesCut_inkBounds(MESCUT_NEST_ITEMS[i]); } catch (eB) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
        if (!b) { out.push('0:0:0:0'); continue; }
        out.push((Math.round(b[0] / PT * 10) / 10) + ':' + (Math.round(-b[1] / PT * 10) / 10)
            + ':' + (Math.round((b[2] - b[0]) / PT * 10) / 10) + ':' + (Math.round((b[1] - b[3]) / PT * 10) / 10));
    }
    return 'ok;' + out.join(',');
}

/**
 * ★사각 빠른 경로 ① 벡터 판정 (0.54.0) — 조각이 **정확히 축 정렬 사각**인가. 패널 `rect-fast.js` 가 3겹 중 첫 겹으로 쓴다.
 *   여기서는 **모양**만 본다(모서리·변). 투명·구멍은 벡터로 알 수 없어 패널의 저해상도 굽기가 본다.
 *   ⚠️애매하면 사각이 아니라고 답한다 — 틀리게 「사각」이라 하면 칼선이 사각으로 나가 **제품이 바뀐다**.
 *     틀리게 「아님」이라 하면 종전(느린) 경로로 갈 뿐이다.
 * 반환 = 'ok;' + 조각마다 'r:<w>:<h>' 또는 'n:<사유>' (쉼표 구분 · w,h = 잉크 mm 소수 3자리 · nestSizes 와 같은 순서)
 */
function mesCut_rectProbe() {
    if (!MESCUT_NEST_ITEMS || !MESCUT_NEST_ITEMS.length) return 'ERROR 대상 없음 (nestBegin 먼저)';
    var out = [], PT = MESCUT_PT_PER_MM;
    for (var i = 0; i < MESCUT_NEST_ITEMS.length; i++) {
        var it = MESCUT_NEST_ITEMS[i], bb = null, why = '';
        try { bb = mesCut_inkBounds(it); } catch (eB) { bb = null; }
        if (!bb) { out.push('n:nobounds'); continue; }
        try { why = mesCut_rectWhyNot(it, bb, 0); } catch (eR) { why = 'err'; }
        if (why) { out.push('n:' + why); continue; }
        var r3 = function (v) { return Math.round(v * 1000) / 1000; };
        out.push('r:' + r3((bb[2] - bb[0]) / PT) + ':' + r3((bb[1] - bb[3]) / PT));
    }
    return 'ok;' + out.join(',');
}

/**
 * ★조각별 사진(래스터·배치 이미지) 포함 여부 (0.57.0 · 도련 색 「자동」용).
 *   사진 가장자리는 부드러워(소프트 에지) 도련 색을 한두 픽셀 안쪽에서 뽑아야 하고,
 *   벡터는 흐림 끔으로 구우므로 가장자리 픽셀이 곧 아트 색이다 — 그 둘을 조각마다 가른다.
 *   숨긴 개체는 안 센다. 깊이 12 까지(그 이상은 사진으로 본다 — 안쪽 색을 뽑는 쪽이 종전 동작이다).
 * 반환 = 'ok;' + 조각마다 '1'(사진 있음) / '0' (쉼표 구분 · nestSizes 와 같은 순서)
 */
function mesCut_photoFlags() {
    if (!MESCUT_NEST_ITEMS || !MESCUT_NEST_ITEMS.length) return 'ERROR 대상 없음 (nestBegin 먼저)';
    function has(it, d) {
        if (d > 12) return true;
        var t = '';
        try { if (it.hidden) return false; t = it.typename; } catch (e) { return false; }
        if (t === 'RasterItem' || t === 'PlacedItem') return true;
        if (t !== 'GroupItem') return false;
        try {
            for (var k = 0; k < it.pageItems.length; k++) { if (has(it.pageItems[k], d + 1)) return true; }
        } catch (eK) { /* ignore: 자식 열거 실패 — 사진 없음으로 둔다(가장자리 그대로 = 벡터 기본 동작) */ }
        return false;
    }
    var out = [];
    for (var i = 0; i < MESCUT_NEST_ITEMS.length; i++) {
        var f = false;
        try { f = has(MESCUT_NEST_ITEMS[i], 0); } catch (eF) { f = true; }
        out.push(f ? '1' : '0');
    }
    return 'ok;' + out.join(',');
}

/** 경계 둘이 같은가(0.05mm) — 사각 판정은 「이 개체가 조각 전체를 덮는가」를 경계로 잰다 */
function mesCut_sameBox(a, b) {
    var t = 0.05 * MESCUT_PT_PER_MM;
    return !!(a && b) && Math.abs(a[0] - b[0]) <= t && Math.abs(a[1] - b[1]) <= t
        && Math.abs(a[2] - b[2]) <= t && Math.abs(a[3] - b[3]) <= t;
}

/** 패스가 **곡선 없는 축 정렬 사각**인가 — 앵커 4개(닫힘 중복 5개 허용)·핸들 = 앵커·x 두 값·y 두 값 */
function mesCut_isAxisRectPath(p) {
    var pts = p.pathPoints, n = pts.length, t = 0.01 * MESCUT_PT_PER_MM, xs = [], ys = [], k;
    if (!p.closed) return false;
    if (n === 5) {
        var a0 = pts[0].anchor, a4 = pts[4].anchor;
        if (Math.abs(a0[0] - a4[0]) > t || Math.abs(a0[1] - a4[1]) > t) return false;
        n = 4;
    }
    if (n !== 4) return false;
    var addUniq = function (arr, v) { for (var q = 0; q < arr.length; q++) if (Math.abs(arr[q] - v) <= t) return; arr.push(v); };
    for (k = 0; k < 4; k++) {
        var pp = pts[k], an = pp.anchor, l = pp.leftDirection, r = pp.rightDirection;
        if (Math.abs(l[0] - an[0]) > t || Math.abs(l[1] - an[1]) > t) return false;   // 곡선 핸들 = 둥근 모서리
        if (Math.abs(r[0] - an[0]) > t || Math.abs(r[1] - an[1]) > t) return false;
        addUniq(xs, an[0]); addUniq(ys, an[1]);
    }
    return xs.length === 2 && ys.length === 2;
}

/** 회전·기울임 없는 배치/래스터인가(행렬의 b·c 성분 = 0) */
function mesCut_axisMatrix(it) {
    try { var m = it.matrix; return Math.abs(m.mValueB) < 1e-6 && Math.abs(m.mValueC) < 1e-6; } catch (e) { return false; }
}

/**
 * 사각이 아닌 이유 — 사각이면 ''. `bb` = 조각 전체의 잉크 경계(이 개체가 그것을 **정확히** 덮어야 사각이다).
 *   · 패스 = 축 정렬 사각 + 채움 + **획 없음**(획은 조인 모양에 따라 모서리가 둥글어진다)
 *   · 배치·래스터 = 회전 없음(투명은 패널 굽기가 본다)
 *   · 클립 그룹 = 클립 패스가 축 정렬 사각 + 잉크 경계 = 클립 경계(콘텐츠가 클립을 다 채우는지는 굽기가 본다)
 *   · 일반 그룹 = 자식 하나가 위 규칙으로 조각 전체를 덮는다(나머지는 그 안에 있다 — 경계가 합집합이므로)
 */
function mesCut_rectWhyNot(it, bb, depth) {
    if (depth > 8) return 'deep';
    var t = it.typename;
    try { if (it.hidden) return 'hidden'; } catch (eH) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
    if (t === 'PathItem') {
        if (!mesCut_isAxisRectPath(it)) return 'path-shape';
        if (it.stroked) return 'stroke';
        if (!it.filled) return 'nofill';
        return mesCut_sameBox(it.geometricBounds, bb) ? '' : 'path-cover';
    }
    if (t === 'CompoundPathItem') {
        if (it.pathItems.length !== 1) return 'compound';
        var cp = it.pathItems[0];
        if (!mesCut_isAxisRectPath(cp) || cp.stroked) return 'compound-shape';
        return mesCut_sameBox(cp.geometricBounds, bb) ? '' : 'compound-cover';
    }
    if (t === 'PlacedItem' || t === 'RasterItem') {
        if (!mesCut_axisMatrix(it)) return 'rotated-image';
        return mesCut_sameBox(it.visibleBounds, bb) ? '' : 'image-cover';
    }
    if (t === 'GroupItem') {
        var clipped = false;
        try { clipped = !!it.clipped; } catch (eC) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
        if (clipped) {
            var cpth = null;
            for (var j = 0; j < it.pageItems.length; j++) {
                try { if (it.pageItems[j].clipping) { cpth = it.pageItems[j]; break; } } catch (eJ) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
            }
            if (!cpth) return 'clip-nested';
            if (cpth.typename === 'CompoundPathItem') {
                if (cpth.pathItems.length !== 1 || !mesCut_isAxisRectPath(cpth.pathItems[0])) return 'clip-shape';
            } else if (cpth.typename !== 'PathItem' || !mesCut_isAxisRectPath(cpth)) return 'clip-shape';
            return mesCut_sameBox(cpth.geometricBounds, bb) ? '' : 'clip-partial';
        }
        for (var k = 0; k < it.pageItems.length; k++) {
            var c = it.pageItems[k], cb = null;
            try { cb = mesCut_inkBounds(c); } catch (eK) { cb = null; }
            if (!mesCut_sameBox(cb, bb)) continue;
            var w = '';
            try { w = mesCut_rectWhyNot(c, bb, depth + 1); } catch (eW) { w = 'err'; }
            if (!w) return '';
        }
        return 'group-nocover';
    }
    return 'kind-' + String(t).replace(/[^A-Za-z]/g, '');
}

/**
 * ★원본 아트보드를 PNG 한 장으로 — 조각 번호 오버레이의 바탕 (0.51.0).
 *   **원본이 활성일 때** 부른다(nestBegin 직후 · 판 문서를 만들기 전). 원본 문서는 손대지 않는다.
 *   패널이 이 PNG 위에 번호를 얹어 고객용 이미지(`overview.png`)와 판별 강조본(`thumb_hi.png`)을 만든다.
 *   반환에 아트보드 mm 원점·크기를 실어 패널이 「원본 mm → PNG px」를 계산한다(`mmToPng`).
 * @param maxPx 긴 변 상한(px) — 기본 1600. 100% 는 72ppi 라 pt = px.
 * 반환 = 'ok;path=<png>;w=;h=;abl=;abt=;abw=;abh=' (mm 는 소수 2자리)
 */
function mesCut_exportOverview(maxPx) {
    if (app.documents.length === 0) return 'ERROR 문서 없음';
    var doc = app.activeDocument;
    var mp = parseInt(maxPx, 10); if (!(mp > 0)) mp = 1600;
    var path = Folder.temp.fsName.replace(/\\/g, '/') + '/mes_cut_overview_src.png';
    try {
        var ab = doc.artboards[doc.artboards.getActiveArtboardIndex()].artboardRect;
        var wPt = ab[2] - ab[0], hPt = ab[1] - ab[3];
        var maxPt = Math.max(wPt, hPt);
        var pct = maxPt > 0 ? Math.min(100, (mp / maxPt) * 100) : 100;
        var f = new File(path);
        if (f.exists) { try { f.remove(); } catch (eR) { /* ignore: 이전 실행 잔해 — 못 지우면 exportFile 이 덮어쓴다 */ } }
        var po = new ExportOptionsPNG24();
        po.artBoardClipping = true; po.transparency = false; po.antiAliasing = true;
        po.horizontalScale = pct; po.verticalScale = pct;
        doc.exportFile(f, ExportType.PNG24, po);
        if (!f.exists) return 'ERROR overview 미생성';
        var PT = MESCUT_PT_PER_MM;
        return 'ok;path=' + path + ';w=' + Math.round(wPt * pct / 100) + ';h=' + Math.round(hPt * pct / 100)
            + ';abl=' + (ab[0] / PT).toFixed(2) + ';abt=' + (-ab[1] / PT).toFixed(2)
            + ';abw=' + (wPt / PT).toFixed(2) + ';abh=' + (hPt / PT).toFixed(2);
    } catch (e) { return 'ERROR overview: ' + e; }
}
...
function mesCut_efsRound(doc, rects, names, mmPerPx, aaOn, tag, idx) {
    ...
    // ★굽기 폴더를 **실행마다 다른 이름**으로 (2026-09-18 실기 「p0.png 를 내보낼 수 없음」).
    //   여태 이름이 `mes_cut_efs_<tag>` 로 고정이라 파일명도 매번 `p0.png` 였다. 이전 실행이 못 지운
    //   p0.png 가 잠겨 있으면 일러는 덮어쓰기를 거절하며 **모달**을 띄우는데, 그건 예외가 아니라
    //   `no-output` 으로만 내려온다(:2422 주석의 그 모양). 종전 코드는 `purge-left` 를 사유에 적고도
    //   **그대로 진행**해서 실패가 예정돼 있었고, 패널은 사람에게 「%TEMP%\mes_cut_efs_* 를 지우세요」를
    //   안내하고 있었다(cut-main.js:444) — 원인을 알면서 사람 손에 맡긴 상태였다.
    //   A0 호스트가 0.15.0 에서 프로브 파일명에 실행 접미사를 붙여 고친 것과 **같은 형태**다(형제 스윕 미완).
    var stamp = (new Date()).getTime().toString(36);
    MESCUT_EFS_SEQ = (MESCUT_EFS_SEQ || 0) + 1;
    mesCut_efsSweepOld(tmpDir);   // 오래된 잔해는 치워 둔다(못 지워도 무해 — 이번 실행은 다른 이름을 쓴다)
    var fold = new Folder(tmpDir + '/mes_cut_efs_' + tag + '_' + stamp + '_' + MESCUT_EFS_SEQ);
    try { if (!fold.create()) { MESCUT_EFS_WHY = 'mkdir-fail'; return null; } } catch (eC) { MESCUT_EFS_WHY = 'mkdir ' + eC; return null; }
    var o, it;
    try {
    ...
    // 폴더 이름이 실행마다 다르므로 **다음 실행이 안 치운다** → 쓰고 나서 여기서 지운다.
    //   못 지워도 무해하다(다음 실행은 또 다른 이름을 쓰고, 위 스윕이 나중에 걷는다).
    try { mesCut_efsPurge(fold); fold.remove(); } catch (eX) { /* ignore: 임시 폴더 정리 — 남아도 다음 실행의 스윕이 걷는다 */ }
    return out;
}

/**
 * ★`%TEMP%` 에 쌓인 지난 굽기 폴더를 걷는다 — 이름이 실행마다 달라졌으므로 누가 치우긴 해야 한다.
 *
 * 잠겨서 못 지우는 것이 있어도 **이번 실행에는 영향이 없다**(다른 이름을 쓴다). 그게 요점이다:
 * 종전에는 같은 이름을 다시 쓰느라 잠긴 파일 하나가 굽기 전체를 막았다.
 * @returns 지운 폴더 수
 */
function mesCut_efsSweepOld(tmpDir) {
    var n = 0;
    try {
        var t = new Folder(tmpDir);
        var fs = t.getFiles(function (x) { return (x instanceof Folder) && /^mes_cut_efs_/.test(x.name); });
        for (var i = 0; i < fs.length; i++) {
            mesCut_efsPurge(fs[i]);
            try { if (fs[i].remove()) n++; } catch (e1) { /* ignore: 잠긴 잔해는 남겨 둔다 — 이번 실행은 다른 이름을 쓴다 */ }
        }
    } catch (e) { /* ignore: 스윕 실패는 이번 실행에 영향이 없다 — 새 폴더는 따로 만든다 */ }
    return n;
}
...
function mesCut_bakeScreens(doc, boxes, srcBB, padPt, mmPerPx, tag, bleedTag, lines, f) {
    if (MESCUT_STRIP_PT > 0) return mesCut_bakeStrips(doc, boxes, mmPerPx, tag, lines);
...
/**
 * ★사각 빠른 경로 ③ 띠 굽기 (0.54.0) — 조각마다 **네 변의 잉크 쪽 띠**만 내보낸다(깊이 = MESCUT_STRIP_PT).
 *   도련은 가장자리 한 줄을 늘리는 일이라 사각 조각의 나머지 픽셀은 쓰이지 않는다 — 비용이 면적이 아니라 둘레에 비례한다.
 *   파일 = `mes_cut_<tag>_<i><t|b|l|r>.png` · 줄 = `S <i> <side> <wPx> <hPx> <path>`.
 *   AA 는 끈다(도련 원색 'ink' 굽기와 같은 조건 — 공급원이 블렌드 색이 되지 않게).
 *   ⚠️옛 경로(조각별 exportFile)가 없다 — 실패하면 통째로 실패시키고 패널이 **종전 전체 경로**로 간다.
 */
var MESCUT_STRIP_PT = 0;
function mesCut_bakeStrips(doc, boxes, mmPerPx, tag, lines) {
    var d = MESCUT_STRIP_PT, n = boxes.length, i, sides = ['t', 'b', 'l', 'r'];
    var sc = (1 / mmPerPx) * (25.4 / 72);
    var rects = [], names = [], idx = [], meta = [];
    for (i = 0; i < n; i++) {
        var bx = boxes[i];
        if (!bx) continue;
        var L = bx[0], T = bx[1], R = bx[2], B = bx[3];
        var dd = Math.min(d, (R - L) / 2, (T - B) / 2);
        var r4 = { t: [L, T, R, T - dd], b: [L, B + dd, R, B], l: [L, T, L + dd, B], r: [R - dd, T, R, B] };
        for (var s = 0; s < 4; s++) {
            var k = sides[s], rc = r4[k];
            rects.push(rc); names.push('p' + i + k); idx.push(i + k);
            meta.push({ i: i, k: k, w: Math.round((rc[2] - rc[0]) * sc), h: Math.round((rc[1] - rc[3]) * sc) });
        }
    }
    if (!rects.length || rects.length > MESCUT_EFS_MAX_AB) { MESCUT_EFS_WHY = 'strip-ab ' + rects.length; return false; }
    var got = mesCut_efsRound(doc, rects, names, mmPerPx, false, tag, idx);
    if (!got) return false;
    for (i = 0; i < meta.length; i++) lines.push('S ' + meta[i].i + ' ' + meta[i].k + ' ' + meta[i].w + ' ' + meta[i].h + ' ' + got[i]);
    return true;
}

function mesCut_nestBakeAll(mmPerPx, padMm, fillClosed, tag, bleedTag, stripMm) {
    if (!MESCUT_NEST_ITEMS || !MESCUT_NEST_ITEMS.length) return 'ERROR 대상 없음 (nestBegin 먼저)';
    // ★띠 모드는 이 호출 안에서만 켠다 — 전역이 남으면 다음 마스크 굽기가 띠를 굽는다(finally 로 되돌린다)
    MESCUT_STRIP_PT = (stripMm > 0) ? stripMm * MESCUT_PT_PER_MM : 0;
    try { return mesCut_nestBakeAllCore(mmPerPx, padMm, fillClosed, tag, bleedTag); }
    finally { MESCUT_STRIP_PT = 0; }
}

function mesCut_nestBakeAllCore(mmPerPx, padMm, fillClosed, tag, bleedTag) {
    if (!mmPerPx || mmPerPx <= 0) mmPerPx = 0.5;
    if (!padMm || padMm < 0) padMm = 0;
...
function mesCut_bakeOneDoc(srcDoc, idxs, srcBB, cellW, cellH, padPt, mmPerPx, fillClosed, tag, bleedTag, n) {
    ...
    try {
        ...
        //    ★기준은 (0,0) 이 아니라 **아트보드 위쪽**이다 — 새 문서의 아트보드는 [0, h, w, 0] 이라
        //    y=0 에서 아래로 깔면 아트가 통째로 아트보드 **밖**에 놓인다. 캔버스는 아트보드 중심
        //    기준 ±2,886mm 뿐이라 판이 길면 그대로 'AOoC' 다(2026-09-04 실사용: 950x2380 조각 6개).
        var curX = 0, curY = tmp.artboards[0].artboardRect[1], rowMax = 0;
        var boxes = [];
        ...
        // ★띠 모드에는 옛 경로가 없다 — 아래 조각별 export 는 **전체**를 굽는다. 조용히 그리로 가면 띠 대신 전체 PNG 가 나와
        //   패널이 띠로 읽는다. 통째로 실패시키면 패널이 종전 경로로 간다(catch 가 임시 문서를 닫는다).
        if (MESCUT_STRIP_PT > 0 && !fastBake) throw new Error('strip-bake ' + (out.efswhy || 'efs-off'));
        if (!fastBake) {
...
function mesCut_rasterizeItem(idx, mmPerPx, padMm, fillClosed) {
    ...
    try {
        ...
        // ★★복제본은 **원본 좌표를 그대로** 들고 온다 → **아트보드 자리로** 끌어다 놓고 굽는다.
        //   안 옮기면 아트보드를 원본 자리로 옮겨야 하는데, 조각이 멀면 캔버스 밖이라 죽는다.
        //   ⚠️**(0,0) 이 아니다** — 새 문서의 아트보드는 [0, h, w, 0](y가 0에서 위로)라,
        //   원점에 맞추면 아트가 아트보드 **아래**로 내려가 h 가 크면 그대로 캔버스를 벗어난다
        //   (실측 2026-09-04: 950x2380 조각을 y=-2385 로 보내 `AOoC` 1095724867 — 하한 -1,696).
...
// 규약은 A0 와 동일: 지름 6mm · 코너에서 7mm 바깥 · 50cm 초과 구간엔 중간 돔보 · 짧은 변에 방향마크 60mm.
 ...
 *
 * ★`코너 + 지름`(13mm)이 아니라 `코너 + 반지름`(10mm)이다 — 돔보 중심은 코너에서 7mm 바깥이고
...
/** ★조각 번호 레이어 — 재단선·돔보와 **분리**하고 인쇄는 켠다(꼬리표는 인쇄물이다 · DXF 에는 안 간다) (0.51.0) */
var MESCUT_NUM_LAYER = '번호';
function mesCut_ensureNumLayer(doc) {
    for (var i = 0; i < doc.layers.length; i++) if (doc.layers[i].name === MESCUT_NUM_LAYER) return doc.layers[i];
    var prev = null;
    try { prev = doc.activeLayer; } catch (e0) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
    var l = doc.layers.add(); l.name = MESCUT_NUM_LAYER;
    try { l.printable = true; } catch (eP) { /* ignore: 바로 위 layers.add() 가 성공한 레이어라 예외가 날 수 없다 — 방어용 */ }
    try { if (prev) doc.activeLayer = prev; } catch (e1) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
    return l;
}
...
/**
 * 꼬리표 하나 = 번호 텍스트를 아웃라인으로 그 사각 안에 (0.51.0).
 *   tag = {x,y,w,h,rot,label} (mm · 시트 좌표 · y 아래로). 세로 꼬리표(rot 90/270)는 글자를 눕힌다 —
 *   rot 90(조각 오른쪽)은 위→아래로, rot 270(조각 왼쪽)은 아래→위로 읽힌다(판을 돌려 보지 않아도 된다).
 *   글자 높이 = 두께의 ~52%(대문자 높이 기준) · 길이가 넘치면 비율로 줄인다(하한 없음 — 꼬리표는 이미 20×8mm 다).
 *   ★아웃라인 = 폰트 임베드 없이 나가게(`mesCut_needFontEmbed` 가 텍스트를 못 보게) · 기본 폰트가 없으면 그대로 둔다.
 * @return 그렸으면 true. 실패는 호출자가 `numfail=` 로 센다(조용히 삼키지 않는다).
 */
function mesCut_drawTag(layer, tag, sheetH) {
    try {
        var PT = MESCUT_PT_PER_MM;
        if (!tag.label) return false;
        var vertical = (tag.rot === 90 || tag.rot === 270);
        var boxW = tag.w * PT, boxH = tag.h * PT;
        var along = vertical ? boxH : boxW;      // 글자가 흐르는 방향 길이
        var across = vertical ? boxW : boxH;     // 글자 높이 방향(꼬리표 두께)
        var tf = layer.textFrames.add();
        tf.contents = tag.label;
        try { tf.textRange.characterAttributes.textFont = app.textFonts.getByName('ArialMT'); } catch (eF) { /* ignore: 이 PC 에 없으면 문서 기본 폰트 — 숫자·대시뿐이라 어느 폰트든 읽힌다 */ }
        tf.textRange.characterAttributes.size = across * 0.75;
        var k = new CMYKColor(); k.cyan = 0; k.magenta = 0; k.yellow = 0; k.black = 100;
        tf.textRange.characterAttributes.fillColor = k;
        tf.textRange.characterAttributes.strokeColor = new NoColor();
        var g = tf.createOutline();
        var vb = g.visibleBounds, gw = vb[2] - vb[0], gh = vb[1] - vb[3];
        var pad = Math.min(1 * PT, across * 0.15);
        var sc = Math.min(1, (along - pad * 2) / Math.max(gw, 0.01));
        if (sc < 1) { g.resize(sc * 100, sc * 100); vb = g.visibleBounds; gw = vb[2] - vb[0]; gh = vb[1] - vb[3]; }
        if (vertical) { g.rotate(tag.rot === 90 ? -90 : 90); vb = g.visibleBounds; gw = vb[2] - vb[0]; gh = vb[1] - vb[3]; }
        var L = tag.x * PT, T = sheetH - tag.y * PT;    // 꼬리표 왼쪽 위 (y-down mm → y-up pt)
        var dx, dy;
        if (vertical) { dx = L + (boxW - gw) / 2 - vb[0]; dy = (tag.rot === 90 ? (T - pad) : (T - (boxH - gh) + pad)) - vb[1]; }
        else { dx = (L + pad) - vb[0]; dy = (T - (boxH - gh) / 2) - vb[1]; }
        g.translate(dx, dy);
        return true;
    } catch (e) { return false; }
}
...
function mesCut_scaleAsPlaced(doc, artLayer, srcItem, srcDoc, pct, tagIdx, rotDeg) {
    ...
    try {
        ...
        po.viewAfterSaving = false;   // ★아래 3097 과 같은 이유 — 반드시 끈다(PC 프리셋을 물려받는다)
        mesCut_pdfNoMarks(po);        // ★마크·도련 여백도 명시(0.52.0) — 조각 모서리 검정 「집게」 = 프리셋의 재단 표시
    ...
    try {
        ...
        // ★굳히기 임베드가 만든 사각 클립을 걷어낸다 (2026-09-18 용준님 — 레이어 패널에 남는 `<Clipping Path>`).
        //   원본에는 클립이 없다 — `embed()` 가 만드는 것이다(:964 실측). 0.33.0 이 같은 클립을 치웠지만
        //   **칼선 계산용 복제본에서만**이라(:1136) 판에 실제로 놓이는 조각에는 그대로 남아 있었다.
        //   ⚠️검산(위)을 **지난 뒤**에 한다 — 검산은 여태와 같은 상태에서 이뤄져야 값이 안 바뀐다.
        //   ⚠️`dropNoopClips` 는 **자르는 게 없는** 클립만 지운다. 진짜로 자르던 클립(embed 가
        //     마스크로 가린 여분을 되살리는 경우 = ink 800 vs vis 876 같은 조각)은 남는다 —
        //     지우면 숨어 있던 아트가 드러나 실루엣이 커지고 재단선이 그림 밖으로 나간다.
        try { mesCut_dropNoopClips(got); } catch (eNC) { /* ignore: 클립 정리는 보정 단계 — 실패해도 배치된 조각은 그대로다 */ }
...
/** 굽기 실행 일련번호 — 폴더 이름을 실행마다 다르게 만드는 축(같은 ms 에 두 번 불릴 수 있다). */
var MESCUT_EFS_SEQ = 0;
...
function mesCut_hardenGrid(srcDoc, idxList, limitPt, tag) {
    var PT = MESCUT_PT_PER_MM;
    // ★줄바꿈·높이 한계 — 기본은 PDF 한계. 확대(배율)가 있으면 호출부가 **확대 후** 캔버스에 맞춘 값을 준다(0.55.0).
    var LIM = (limitPt > 0) ? Math.min(limitPt, MESCUT_PDF_MAX_PT) : MESCUT_PDF_MAX_PT;
    ...
    if (rowW > totW) totW = rowW;
    totH += rowH;
    if (!(totW > 0) || !(totH > 0) || totH > LIM) { MESCUT_HARDEN_ERR = 'size' + Math.round(totW) + 'x' + Math.round(totH); return null; }

    var pdfPath = Folder.temp.fsName.replace(/\\/g, '/') + '/mes_cut_harden' + String(tag || '').replace(/[^A-Za-z0-9_]/g, '') + '.pdf';
    var tmp = null, cells = {};
    try {
        tmp = mesCut_newDocMM(totW, totH);
        var lay = tmp.layers[0];
        ...
        for (i = 0; i < n; i++) {
            ...
            if (curX > 0 && curX + cw[i] > LIM) { curX = 0; curY -= rowMax; rowMax = 0; }
        ...
        po.generateThumbnails = false;
        // ★「저장 후 PDF 보기」를 반드시 끈다 (2026-09-17 실기) — 이 값은 **그 PC 일러의 마지막
        //   PDF 프리셋**을 물려받는다. 켜져 있는 PC 에서는 굳히기 PDF 를 저장할 때마다 기본 뷰어
        //   (Chrome)가 떠서, 굳히기가 조각 수만큼 브라우저 창을 열고 적용 시간이 통째로 늘어난다.
        //   ⚠️기본값이 false 인 PC 도 있어 **개발기에서는 재현되지 않는다** — PC별 상태 함정이다
        //   ([[feedback-log-encoding-per-pc]] 와 같은 형태). 명시하지 않으면 PC 가 정한다.
        po.viewAfterSaving = false;
        mesCut_pdfNoMarks(po);        // ★마크·도련 여백도 명시(0.52.0) — 위와 같은 축, 굳히기 PDF 두 곳 모두
...
/**
 * ★격자를 **확대 후 크기**에 맞춰 여러 개로 나눈다 (0.55.0 · 2026-09-24 실기).
 *   종전에는 격자를 파일 크기 그대로 PDF 한계(4.9m)로 줄바꿈해 하나만 만들고, **확대한 뒤** 캔버스(5.6m)를
 *   넘으면 통째로 포기(`hardenwhy=canvas`)해 **전 조각이 조각별 옛 경로**(조각당 PDF·임시 문서)로 떨어졌다.
 *   1/10 축소 파일이 정확히 그랬다 — 14조각이 한 줄(≈2m)에 들어가 10배 확대하면 20m → 적용 332초.
 *   → 줄바꿈·높이 한계를 `캔버스 ÷ 배율` 로 잡고, 한 격자에 다 안 들어가면 **격자를 더 만든다**.
 *   비용 = 격자 수 × (PDF 1 + 쓰인 회전 수 × 배치·임베드). 여전히 조각 수와 무관하다.
 *   결과물은 종전 격자 경로와 같다(같은 평탄화·같은 자리의 회전·같은 나누기 검산).
 *   못 만든 격자의 조각은 마스터에 없으므로 호출부가 조각별 옛 경로로 처리한다(부분 성공).
 * @returns {grids:[grid], fail:'사유'} — 격자 좌표는 mesCut_hardenGrid 와 같다
 */
function mesCut_hardenGrids(srcDoc, idxList, pct) {
    var GAP = MESCUT_HARDEN_GAP_MM * MESCUT_PT_PER_MM;
    var k = (pct > 0 ? pct : 100) / 100;
    var LIM = Math.min(MESCUT_PDF_MAX_PT, MESCUT_CANVAS_MAX_PT / Math.max(1, k));
    var chunks = [], cur = [], rowW = 0, rowH = 0, totH = 0, i;
    for (i = 0; i < idxList.length; i++) {
        var b = null;
        try { b = mesCut_inkBounds(MESCUT_NEST_ITEMS[idxList[i]]); } catch (e0) { /* ignore: 경계를 못 읽은 조각은 격자에서 빼고 조각별 경로로 보낸다 */ }
        if (!b) continue;
        var cw = (b[2] - b[0]) + GAP, ch = (b[1] - b[3]) + GAP;
        if (cw > LIM || ch > LIM) continue;                     // 혼자서도 안 들어가는 조각 = 조각별 경로
        var newRow = rowW > 0 && rowW + cw > LIM;
        var hIf = newRow ? (totH + rowH + ch) : (totH + Math.max(rowH, ch));
        if (cur.length && hIf > LIM) {                           // 이 격자는 꽉 찼다 → 새 격자
            chunks.push(cur); cur = []; rowW = 0; rowH = 0; totH = 0; newRow = false;
        }
        if (newRow) { totH += rowH; rowW = 0; rowH = 0; }
        cur.push(idxList[i]); rowW += cw; if (ch > rowH) rowH = ch;
    }
    if (cur.length) chunks.push(cur);
    var grids = [], fail = '';
    for (var g = 0; g < chunks.length; g++) {
        var gr = mesCut_hardenGrid(srcDoc, chunks[g], LIM, chunks.length > 1 ? ('_' + g) : '');
        if (gr) grids.push(gr);
        else if (!fail) fail = 'grid' + g + (MESCUT_HARDEN_ERR ? ('.' + MESCUT_HARDEN_ERR) : '');
    }
    return { grids: grids, fail: fail, chunks: chunks.length };
}

/**
 * ★굳힌 격자를 **칸에 하나씩 들어가는 조각까지** 푼다 (0.56.0 · mesCut_hardenSplit 주석).
 *   한 칸에 들어가면 그대로 담고, 여러 칸에 걸치면 안으로 내려간다 — 단 내려가도 되는 그룹만.
 *   클립 패스 자신은 담지 않는다(격자 상자 클립이라 자르는 것이 없다 — 판정 조건이 그것이다).
 *   종전 `mesCut_hardenKids`(맨 위 한 층만·개수 비교)를 대신한다. 그것이 막던 위험 「한 조각의 내부 그룹까지
 *   풀어 한 조각이 여러 개로 보임」은 여기서도 없다 — **한 칸에 들어가는 순간 더 내려가지 않고 통째로** 담는다.
 * @param root 임베드된 마스터 · gb 마스터 상자 · cellOf(bounds) → {hit, hits}
 * @returns {ok, items:[{it, cell}], why}
 */
function mesCut_hardenLeaves(root, gb, cellOf) {
    var tol = Math.max(2, Math.max(gb[2] - gb[0], gb[1] - gb[3]) * 0.01);
    var items = [], why = '', guard = 0;
    function isNoopClip(g) {
        // 클립 패스가 **격자 상자 전체**를 덮으면 이 클립은 아무것도 안 자른다(페이지 상자 클립)
        for (var q = 0; q < g.pageItems.length; q++) {
            var c = g.pageItems[q], cb = null;
            try { if (!c.clipping) continue; cb = c.geometricBounds; } catch (e) { continue; }
            if (!cb) return false;                          // 경계를 못 읽는 클립 = 안 자른다고 단정하지 않는다
            return cb[0] <= gb[0] + tol && cb[2] >= gb[2] - tol && cb[1] >= gb[1] - tol && cb[3] <= gb[3] + tol;
        }
        return false;
    }
    function walk(it, depth) {
        if (why) return;
        if (++guard > 20000) { why = 'leaves-many'; return; }
        try { if (it.hidden) return; } catch (eH) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
        var kb = null;
        try { kb = mesCut_inkBounds(it); } catch (eB) { kb = null; }
        if (!kb) return;                                    // 경계 없는 개체(빈 그룹 등)는 그림이 없다
        var c = cellOf(kb);
        if (c.hits === 1) { items.push({ it: it, cell: c.hit }); return; }
        var t = '';
        try { t = it.typename; } catch (eT) { t = ''; }
        if (t !== 'GroupItem' || depth > 12) { why = (c.hits ? 'ambig' : 'outside') + ':' + t; return; }
        var clipped = false;
        try { clipped = !!it.clipped; } catch (eC) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
        if (clipped && !isNoopClip(it)) { why = 'clip-span'; return; }   // 진짜로 자르는 클립이 여러 칸에 걸침
        for (var k = 0; k < it.pageItems.length && !why; k++) {
            var ch = it.pageItems[k];
            try { if (ch.clipping) continue; } catch (eP) { /* ignore: 개체 종류·상태에 따라 없는 속성 — 기본값으로 건너뛴다 */ }
            walk(ch, depth + 1);
        }
    }
    walk(root, 0);
    return why ? { ok: false, items: [], why: why } : { ok: true, items: items, why: '' };
}
 ...
 */
function mesCut_hardenSplit(destLayer, master, grid, rot, pct, need) {
    var gb = null;
    ...
    // ★배정은 **셀 상자 안에 들어가느냐**로 한다 — 「가장 가까운 중심」은 틀린다.
    //   굳힌 PDF 는 조각별 그룹이 아니라 **낱개 패스 수십 개**로 풀린다(실측 4조각=57개).
    //   그러면 넓은 조각의 **끝쪽 패스**가 자기 중심보다 옆 조각 중심에 더 가까워진다 —
    //   실측: 셀 2(폭 86.9 · 중심 1650.5) vs 셀 3(폭 130.6 · 중심 1779.2) 에서
    //   조각 3 의 왼쪽 패스가 셀 2 로 넘어갔다. 크기 편차가 클수록 반드시 어깋난다.
    //   셀은 서로 20mm 떨어져 있으므로 **상자 + 절반** 으로 보면 답이 하나다.
    var slack = MESCUT_HARDEN_GAP_MM * MESCUT_PT_PER_MM / 2;
    var cellOf = function (kb) {
        var hit = -1, hits = 0;
        for (var q = 0; q < idxs.length; q++) {
            var hw = size[q][0] / 2 + slack, hh = size[q][1] / 2 + slack;
            if (kb[0] >= want[q][0] - hw && kb[2] <= want[q][0] + hw
                && kb[3] >= want[q][1] - hh && kb[1] <= want[q][1] + hh) { hit = q; hits++; }
        }
        return { hit: hit, hits: hits };
    };
    // ★여러 칸에 걸친 덩어리는 **안으로 내려가 푼다** (0.56.0 · 2026-09-24 실기 `kids4of11`).
    //   PDF 평탄화가 여러 조각의 그림을 「페이지 크기 클립 그룹」 하나(자식 898개)에 몰아 넣었다 —
    //   맨 위 층만 세던 종전 코드는 「덩어리 4 < 조각 11」 로 포기해 **전 조각이 조각별 굳히기**(315초)로 갔다.
    //   그 클립은 페이지 전체라 **자르는 것이 없고**, 안의 패스는 각자 한 칸 안에 있다(프로브 실측).
    //   → 내려가도 되는 것은 둘뿐: ①일반 그룹 ②클립이 격자 상자 전체인(= 아무것도 안 자르는) 클립 그룹.
    //   ⚠️**진짜로 자르는 클립**이 여러 칸에 걸치면 풀면 그림이 바뀌므로 **포기한다**(조각별 경로 = 결과물 불변).
    var got = mesCut_hardenLeaves(master, gb, cellOf);
    if (!got.ok) { MESCUT_HARDEN_ERR = got.why; return null; }
    for (i = 0; i < got.items.length; i++) buckets[got.items[i].cell].push(got.items[i].it);
    var map = {}, nOk = 0;
    ...
    //      부분 수용하면 안 된다.
    for (i = 0; i < idxs.length; i++) {
        // ★이 판에 안 쓰이는 조각은 옮기지 않는다(0.56.0) — 격자엔 전 조각이 있지만 판마다 몇 개만 쓴다.
        //   푼 개체가 수백 개라 다 옮기면 판 수만큼 시간이 붙는다. 남은 것은 호출부가 임시 레이어째 지운다.
        if (need && !need[idxs[i]]) continue;
        var g = null;
        try {
 ...
 */
function mesCut_hardenMasters(doc, grid, rots, pct, need) {
    MESCUT_HARDEN_ERR = '';
    ...
    for (var r = 0; r < rots.length; r++) {
        ...
        var map = got ? mesCut_hardenSplit(lay, got, grid, rot, pct, need ? need[String(rot)] : null) : null;
...
function mesCut_nestApply(vecOffsetMm, vecFillClosed, vecBleedMm, vecBleedMode, cutMode) {
    ...
    var sheets = [], cur = null, bleedSz = {}, bleedStrip = {};
    for (var i = 0; i < lines.length; i++) {
        ...
        // ★LS = 사각 빠른 경로의 **띠 도련**(0.54.0) = `LS idx W H g d` (mm · 잉크 W×H · 도련 g · 띠 깊이 d). 파일은 패널이 쓴 4장.
        if (p[0] === 'LS') { bleedStrip[parseInt(p[1], 10)] = { W: parseFloat(p[2]), H: parseFloat(p[3]), g: parseFloat(p[4]), d: parseFloat(p[5]) }; continue; }
        if (p[0] === 'S') { cur = { w: parseFloat(p[2]), h: parseFloat(p[3]), items: [], cuts: [], segs: [], tags: [] }; sheets.push(cur); continue; }
        if (!cur) continue;
        if (p[0] === 'I') {
            cur.items.push({ idx: parseInt(p[1], 10), x: parseFloat(p[2]), y: parseFloat(p[3]), rot: parseFloat(p[4]) || 0 });
        } else if (p[0] === 'T') {
            // ★조각 번호 꼬리표(0.51.0) = `T idx x y w h rot label` (mm · 시트 좌표 · y 아래로).
            //   자리는 패널이 배치 결과에서 되돌린 값(piece-number.js placedGeom)이라 여기서 기하를 다시 계산하지 않는다.
            //   label 은 ASCII(숫자·대시)만 — 브릿지가 아니라 params 파일이지만 규약을 같게 둔다.
            if (p.length >= 8) {
                cur.tags.push({ idx: parseInt(p[1], 10), x: parseFloat(p[2]), y: parseFloat(p[3]), w: parseFloat(p[4]), h: parseFloat(p[5]),
                    rot: parseFloat(p[6]) || 0, label: String(p[7]).replace(/[^0-9A-Za-z\-]/g, '') });
            }
        } else if (p[0] === 'C') {
    ...
    // ★★판 규격 관문 — **판을 하나라도 만들기 전에** 전부 검사한다 (2026-09-04).
    //   판 = 생산 단위 = 등록 1건이다(아래 saveOneSheet 가 판마다 등록 폴더를 만든다).
    //   일러 아트보드 한계를 넘는 판은 만들 수도 없고, 만들 수 있어도 존재하지 않는 생산 단위다.
    //   ⚠️ 여기서 안 막으면 `documents.add` 가 `an Illustrator error occurred: 1346458189 ('PARM')`
    //      로 죽는다 — 실사용 2026-09-04: 맞붙임이 판 길이 상한을 안 봐서 1050x13,442mm 가 넘어왔다.
    //      `PARM` 은 어느 인자가 왜 틀렸는지 말해 주지 않아 원인 추적에 반나절이 든다.
    //   ⚠️ **문서를 만들기 전에** 끝낸다 — 뒤에서 막으면 이전 판을 이미 닫아 버려 되돌릴 것이 없다.
    var chkMaxMm = Math.round(MESCUT_CANVAS_MAX_PT / MESCUT_PT_PER_MM);
    for (var chkS = 0; chkS < sheets.length; chkS++) {
        var chkW = sheets[chkS].w, chkH = sheets[chkS].h;
        if (!(chkW > 0) || !(chkH > 0)) {
            return 'ERROR 판 ' + (chkS + 1) + ' 규격이 잘못됐습니다 (' + chkW + 'x' + chkH + 'mm)';
        }
        if (chkW * MESCUT_PT_PER_MM > MESCUT_CANVAS_MAX_PT || chkH * MESCUT_PT_PER_MM > MESCUT_CANVAS_MAX_PT) {
            return 'ERROR 판 ' + (chkS + 1) + ' 이 ' + Math.round(chkW) + 'x' + Math.round(chkH)
                + 'mm 로 일러 한계 ' + chkMaxMm + 'mm 를 넘습니다 — 판을 나눠야 합니다'
                + ' (맞붙임을 끄면 판이 나뉩니다)';
        }
    }
    ...
    var nNum = 0, nNumFail = 0;                    // 조각 번호 꼬리표 — 그린 수 / 못 그린 수 → 결과 `num=`·`numfail=`(0.51.0)
    ...
    MESCUT_CLIPSKIP = 0;   // 판마다 초기화 — 「넓혀도 나올 그림이 없던」 클립 수(결과에 싣는다)
    ...
    for (var bk2 in bleedStrip) { if (bleedStrip.hasOwnProperty(bk2)) { hasBleedPng = true; break; } }
    ...
    try {
        ...
        //   = 종전 경로와 완전히 같다(회귀 0).
        var hardenGrids = [], nFast = 0, nMasters = 0, hardenWhy = '', nGrids = 0;
        ...
        if (needIdx.length && MESCUT_HARDEN_OFF) hardenWhy = 'off';
            ...
            // ★격자를 **확대 후** 캔버스에 맞춰 여러 개로(0.55.0 · mesCut_hardenGrids 주석). 종전 단일 격자는
            //   배율이 크면 통째로 'canvas' 포기 → 전 조각 조각별 경로였다.
            var hg = mesCut_hardenGrids(srcDoc, needIdx, resizePct);
            mesCut_tm('harden', tmH);
            for (var hgi = 0; hgi < hg.grids.length; hgi++) {
                var gg = hg.grids[hgi];
                // 안전망 — 나눈 규칙상 넘을 수 없지만, 넘으면 그 격자만 버린다(그 조각들은 조각별 경로)
                if (gg.w * (resizePct / 100) > MESCUT_CANVAS_MAX_PT || gg.h * (resizePct / 100) > MESCUT_CANVAS_MAX_PT) {
                    if (!hardenWhy) hardenWhy = 'canvas';
                    continue;
                }
                hardenGrids.push(gg);
            }
            nGrids = hardenGrids.length;
            if (!nGrids && !hardenWhy) hardenWhy = hg.fail || 'grid';
            else if (hg.fail && !hardenWhy) hardenWhy = hg.fail;
        }
        for (var s = 0; s < sheets.length; s++) {
            ...
            var masters = null;
            if (hardenGrids.length) {
                // ★격자마다 **이 판에 쓰이는 회전만** 만든다 — 이 판에 조각이 없는 격자는 건너뛴다(0.55.0).
                //   마스터들은 하나의 {layers, byRot} 로 합친다 — 아래 배치 코드는 byRot[rot][idx] 만 본다.
                var tmM = (new Date()).getTime();
                for (var gI = 0; gI < hardenGrids.length; gI++) {
                    var gd = hardenGrids[gI];
                    var rotSeen = {}, rotList = [], ra, needByRot = {};
                    for (ra = 0; ra < sh.items.length; ra++) {
                        var itR = sh.items[ra];
                        if (!(resizePct !== 100 || itR.rot)) continue;
                        if (!gd.cells[String(itR.idx)]) continue;
                        var rk = String(itR.rot || 0);
                        // 이 판·이 회전에 쓰이는 조각만 푼다(mesCut_hardenSplit `need`)
                        if (!needByRot[rk]) needByRot[rk] = {};
                        needByRot[rk][String(itR.idx)] = 1;
                        if (rotSeen[rk]) continue;
                        rotSeen[rk] = 1; rotList.push(itR.rot || 0);
                    }
                    if (!rotList.length) continue;
                    var mg = mesCut_hardenMasters(doc, gd, rotList, resizePct, needByRot);
                    if (!mg) { if (!hardenWhy) hardenWhy = 'split' + (MESCUT_HARDEN_ERR ? ('.' + MESCUT_HARDEN_ERR) : ''); continue; }
                    nMasters += mg.n;
                    if (!masters) masters = { layers: [], byRot: {}, n: 0 };
                    masters.layers.push(mg.layer); masters.n += mg.n;
                    for (var rkey in mg.byRot) {
                        if (!mg.byRot.hasOwnProperty(rkey)) continue;
                        if (!masters.byRot[rkey]) masters.byRot[rkey] = {};
                        for (var ikey in mg.byRot[rkey]) { if (mg.byRot[rkey].hasOwnProperty(ikey)) masters.byRot[rkey][ikey] = mg.byRot[rkey][ikey]; }
                    }
                }
            ...
            //   가장 이른 안전 지점이다.
            if (masters) {
                for (var mlI = 0; mlI < masters.layers.length; mlI++) {
                    try { masters.layers[mlI].remove(); } catch (eML) { /* ignore: 임시 개체·파일 정리 — 이미 지워졌거나 참조 무효 */ }
                }
                masters = null;
            }
                ...
                for (var vi = 0; vi < copies.length; vi++) {
                    ...
                    if (vecBleedMm > 0) {
                        ...
                        if (!hasBleedPng) {
                            ...
                            if (!bMode && grow > 0) {
                                try {
                                    var stp = bleedStrip[sh.items[vi].idx];
                                    if (stp) { if (mesCut_bleedPlaceStrips(doc, artLayer, copies[vi], sh.items[vi].idx, stp, sh.items[vi].rot)) bMode = 'pixel'; }
                                    else if (mesCut_bleedPlaceItem(doc, artLayer, copies[vi], sh.items[vi].idx,
            ...
            //   기준 = **조각 + 조각별 칼선 전체를 감싼 사각**. 그 바깥 7mm 가 돔보 자리다.
            //   (조각만으로 잡으면 gap/2 만큼 바깥에 있는 조각 칼선이 테두리를 삐져나온다)
            //   배치 영역이 이미 margin 만큼 안쪽이라 확장해도 시트 규격을 넘지 않는다.
            // ── ★조각 번호 꼬리표 (0.51.0 → 0.53.0 자리 이동) — 칼선·도련 **뒤**, 아트보드 맞추기 **앞**, 전용 레이어에 ──
            //   0.51.0 은 돔보 뒤(맨 끝)에 그렸다. 그러면 아래 `unionOf(topItems)` 가 글자를 못 세어 아트보드가
            //   조각+칼선에만 맞춰지고, 바깥 조각의 꼬리표(조각 위·옆 8mm)는 **돔보 여백(10mm) 안**에 놓인다 —
            //   실기 2026-09-23 「번호가 돔보 밖으로 나갔다」. 배치 영역은 이미 돔보 여백만큼 안쪽이고 꼬리표는
            //   그 안에 있었는데, 아트보드를 다시 맞추며 그 여백을 조각 기준으로 새로 만든 것이 원인이다.
            //   → 맞추기 **앞**에 그려 글자가 합집합에 들어가게 한다(칼선·도련은 이미 끝나 글자를 조각으로 보지 않는다).
            //   좌표는 패널이 준 시트 mm(y 아래로) 그대로 — `translate` 와 같은 변환(sheetH 기준).
            if (sh.tags && sh.tags.length) {
                var numLayer = null;
                try { numLayer = mesCut_ensureNumLayer(doc); } catch (eNL) { numLayer = null; }
                for (var ti = 0; ti < sh.tags.length; ti++) {
                    if (!numLayer) { nNumFail++; continue; }
                    if (mesCut_drawTag(numLayer, sh.tags[ti], sheetH)) nNum++; else nNumFail++;
                }
            }
        ...
        return 'ok;sheets=' + made + ';items=' + items + ';dombo=' + dombo
            ...
            + ';num=' + nNum + ';numfail=' + nNumFail
            + ';fast=' + nFast + ';masters=' + nMasters + ';grids=' + nGrids + (hardenWhy ? (';hardenwhy=' + hardenWhy) : '')
            ...
            // ★넓혀도 나올 그림이 없어 클립 확장을 **거절한** 수 — 0.45.0. 이 값이 크면 원본들이
            //   클립에 딱 맞게 잘려 온다는 뜻이라, 도련 비용(픽셀 경로)이 늘 수밖에 없는 근거가 된다.
            + (MESCUT_CLIPSKIP ? (';clipskip=' + MESCUT_CLIPSKIP) : '')
        ...
        try {
            if (masters && masters.layers) {
                for (var fmI = 0; fmI < masters.layers.length; fmI++) {
                    try { masters.layers[fmI].remove(); } catch (eFM1) { /* ignore: 임시 개체·파일 정리 — 이미 지워졌거나 참조 무효 */ }
                }
                masters = null;
            }
        } catch (eFM) { /* ignore: 임시 개체·파일 정리 — 이미 지워졌거나 참조 무효 */ }
        try {
            // 만든 격자 PDF 전부(캔버스 안전망에 걸려 안 쓴 것까지) — `hg` 는 굳히기를 안 탔으면 undefined
            var allGrids = (typeof hg !== 'undefined' && hg && hg.grids) ? hg.grids : [];
            for (var hfI = 0; hfI < allGrids.length; hfI++) { var hf = new File(allGrids[hfI].pdf); if (hf.exists) { hf.remove(); } }
...
function mesCut_exportPair() {
    ...
    try {
        ...
        // embedAllFonts 는 판마다 다시 정한다(아래) — A0 0.9.0 정정의 형제 스윕. 여기서 고정하지 않는다.
        for (var dz = 0; dz < docs.length; dz++) {
            var doc = docs[dz];
            app.activeDocument = doc;
            epsOpts.embedAllFonts = mesCut_needFontEmbed(doc);
...
/**
 * 오늘 **같은 이름의 산출물**이 이미 있나 — A0 호스트 `mesA0_pickupClash` 의 형제다 (2026-09-18).
 *   픽업 폴더(`_출력/<날짜>`)는 날짜당 한 자리이고 에이전트는 크기가 다르면 **덮어쓴다** →
 *   등록·주문·카드는 둘 다 멀쩡한데 **재단기에 가는 실물만 나중 것 하나**가 된다.
 *   이름을 시스템이 바꾸지 않는 이유·두 곳을 보는 이유는 A0 쪽 주석에 적어 두었다.
 * @return 충돌한 자리의 이름 · 없으면 빈 문자열
 */
function mesCut_pickupClash(ymd, fileName, myFolder, myBytes) {
    try {
        if (!fileName || !ymd) return '';
        var pf = new File(MESCUT_REGISTER_ROOT + '/_출력/' + ymd + '/' + fileName);
        if (pf.exists) {
            var pl = -1;
            try { pl = pf.length; } catch (ePl) { pl = -1; }   // 못 읽으면 같다고 단정하지 않는다 = 경고 쪽
            if (!(pl > 0 && pl === myBytes)) return '_출력/' + ymd;
        }
        ...
        for (var i = 0; i < subs.length && i < 300; i++) {
            ...
            if (nm === myFolder || nm.charAt(0) === '_' || nm.charAt(0) === '.') continue;
            if (nm.length < 8 || nm.substring(0, 8) !== ymd) continue;   // 이름으로 먼저 거른다(왕복 절약)
            if (new File(subs[i].fsName + '/' + fileName).exists) return nm;
        }
        return '';
    } catch (eP) { return ''; }   // ignore: 경고용 조회 — 못 보면 경고를 못 할 뿐 등록은 그대로다
}
 ...
 * 반환 'ok;folders=<n>;pickdup=<n>' 또는 'ERROR ...'
 */
function mesCut_nestRegister() {
    ...
    $.global.mesCutPickDup = [];     // 〃 (픽업 이름 충돌 = 경고용)
    $.global.mesCutBatchFolder = ''; // ★판이 여럿이면 판 1 의 등록 폴더 = 묶음 이름(0.58.0 · saveOneSheet 주석)
    var srcDoc = app.activeDocument;
    var made = 0, folders = [];
...
function mesCut_saveOneSheet(doc, idx, R, clientName, pcName, userName) {
    ...
    // ★판 묶음(0.58.0 · 2026-09-26 작업지시서 「가」 안) — 한 장을 판 여러 개로 나누면 판마다 등록이 따로 생겨
    //   대기함에서 **서로 모르는 별개 건**으로 보였다(`batch_folder` 를 null 로 보냈다). 판 1 의 등록 폴더를
    //   모든 판의 `batch_folder` 로 싣는다 — 웹 대기함은 이미 이 값으로 「한 작업」으로 묶어 보여 준다(intake.js batch_key).
    //   판이 1장이면 종전처럼 null(단독 작업).
    if (MESCUT_NEST_DOCS.length > 1 && idx === 0) $.global.mesCutBatchFolder = jobFolder.fsName.replace(/\\/g, '/');
    ...
    // ★`work.ai` 를 더 이상 쓰지 않는다 (2026-09-15 용준님 「없어도 될 것 같아」 · A0 호스트 0.16.0 과 같은 결정).
    //   ①읽는 코드가 없다 — 판짜기 `mes-sheet.jsx` 는 09-11 은퇴했고, 이 등록은 `mode:"single"` 이라
    //     서버가 고르는 `file_path` 는 어차피 **EPS** 다(`workbench.ts:721`). 웹은 표시용 파일명 폴백으로만 본다.
    //   ②여기는 **판(sheet)** 이라 이 호스트에서 가장 큰 파일이고, 실패하면 `return 'ERROR'` 로
    //     **등록 자체를 막고 있었다** — 아무도 안 읽는 파일 때문에 재단기 파일까지 못 나가던 자리다.
    //   산출물 크기 자리는 EPS 로 잰다(아래). 되돌리려면 이 블록을 복원하고 `work_ai` 를 다시 채우면 된다.
    var workBytes = 0;
    ...
    try {
        ...
        eo.embedAllFonts = mesCut_needFontEmbed(doc);
        var epsFileC = new File(jobFolder.fsName + '/' + epsName);
        doc.saveAs(epsFileC, eo);
        // 산출물 크기 = **EPS**(2026-09-15, work.ai 폐지분 이관). 공유에 쓴 길이가 바로 안 보일 수 있어
        //   0 이 나오면 한 번 쉬고 다시 읽는다 — 멀쩡한 파일이 `0MB` 로 보고되던 전례(A0 09-15).
        try {
            workBytes = epsFileC.length;
            if (!(workBytes > 0)) { $.sleep(250); workBytes = epsFileC.length; }
        } catch (eB) { workBytes = 0; /* ignore: 길이를 못 읽어도 등록은 진행한다 — 표시용 값이다 */ }
    } catch (eE) { return 'ERROR EPS 저장 실패: ' + eE }
    // ★같은 이름이 오늘 또 나왔나 — 픽업에서 덮어써진다(사실만 알린다. mesCut_pickupClash 주석 참조).
    var pickClash = mesCut_pickupClash(ymd, epsName, folderName, workBytes);
    if (pickClash) {
        if (!$.global.mesCutPickDup) $.global.mesCutPickDup = [];
        $.global.mesCutPickDup.push(epsName);
    }
    ...
    // ★조각 번호 산출물 (0.51.0) — 패널이 temp 에 만든 PNG 를 이 폴더로 옮긴다.
    //   `thumb_hi.png`(이 판의 조각만 강조한 원본 배치) · `overview.png`(전체 번호 · 고객용).
    //   에이전트는 `files.thumb_hi` 가 있으면 base64 로 실어 주고(Program.cs:1093) 웹은 R2 로 외부화해
    //   작업지시서가 **이미** 우선 표시한다 — 그래서 이 파일명 하나로 축1·웹 변경 없이 작업지시서까지 간다.
    //   없으면(번호 꺼짐·구 패널) 아무것도 안 하고 manifest 에도 키가 안 생긴다(종전과 동일).
    var hiName = null, ovName = null;
    try {
        var hiSrc = R['THUMBHI_' + idx];
        if (hiSrc) { var hf = new File(hiSrc); if (hf.exists && hf.copy(jobFolder.fsName + '/thumb_hi.png')) hiName = 'thumb_hi.png'; }
        if (R.OVERVIEW) { var ovf = new File(R.OVERVIEW); if (ovf.exists && ovf.copy(jobFolder.fsName + '/overview.png')) ovName = 'overview.png'; }
    } catch (eNum) { /* ignore: 번호 이미지는 부속 — 없어도 등록은 성립하고 manifest 에 키가 빠져 드러난다 */ }
    // 이 판의 조각 목록 — 패널이 만든 ASCII JSON 배열만 받는다(한글·제어문자가 섞이면 버린다 = manifest 가 깨지는 것보다 낫다)
    var piecesJson = null;
    var pjRaw = R['PIECES_' + idx];
    if (pjRaw && /^\[[\x20-\x7E]*\]$/.test(pjRaw)) piecesJson = pjRaw;
    ...
    var mf = '{'
        ...
        + ',"order_item_id":null'
        + ',"files":{"work_ai":null,"eps":"' + mesCut_jsonEsc(epsName) + '"'
        + ',"dxf":' + (dxfName ? ('"' + mesCut_jsonEsc(dxfName) + '"') : 'null')
        + ',"dxf_error":' + (dxfErr ? ('"' + mesCut_jsonEsc(dxfErr) + '"') : 'null')   // null 이면 DXF 가 있다(2026-09-11)
        + ',"thumb":"thumb.png"'
        + (hiName ? ',"thumb_hi":"thumb_hi.png"' : '') + (ovName ? ',"overview":"overview.png"' : '')
        + ',"work_bytes":' + workBytes + '}'
        // ★조각 번호(0.51.0) — 이 판에 든 조각 [{n,w,h,x,y,r}] (n=번호 · w/h 실물 mm · x/y 판 위 자리 · r 회전).
        //   에이전트가 manifest 를 통째로 넘기므로 웹이 받아 쓰는 것은 Phase 2 (ai_analysis_requests.pieces_json).
        + ',"pieces":' + (piecesJson || 'null')
        + ',"piece_numbering":' + (R.NUMBERING ? ('"' + mesCut_jsonEsc(R.NUMBERING) + '"') : 'null')
        // 오늘 같은 이름의 산출물이 이미 있던 자리 — 픽업에서 **뒤엣것이 앞엣것을 덮는다**(2026-09-18).
        //   경고용 기록이라 ingest 는 무시한다. null = 충돌 없음.
        + ',"pickup_clash":' + (pickClash ? ('"' + mesCut_jsonEsc(pickClash) + '"') : 'null')
        + ',"batch_folder":' + ((MESCUT_NEST_DOCS.length > 1 && $.global.mesCutBatchFolder) ? ('"' + mesCut_jsonEsc($.global.mesCutBatchFolder) + '"') : 'null')
        + ',"batch_index":' + (MESCUT_NEST_DOCS.length > 1 ? (idx + 1) : 'null')
        + ',"batch_total":' + (MESCUT_NEST_DOCS.length > 1 ? MESCUT_NEST_DOCS.length : 'null')
