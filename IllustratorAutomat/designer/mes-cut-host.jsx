...
function mesCut_hardenSplit(destLayer, master, grid, rot, pct) {
    ...
    for (i = 0; i < kids.length; i++) {
        ...
        for (j = 0; j < idxs.length; j++) {
            var hw = size[j][0] / 2 + slack, hh = size[j][1] / 2 + slack;
            if (kb[0] >= want[j][0] - hw && kb[2] <= want[j][0] + hw
                && kb[3] >= want[j][1] - hh && kb[1] <= want[j][1] + hh) { hit = j; hits++; }
        }
        // 어느 상자에도 안 들어가거나 둘 이상에 걸치면 사상 전제가 깨진 것이다.
        if (hits !== 1) { MESCUT_HARDEN_ERR = (hits ? 'ambig' : 'outside') + i; return null; }
        buckets[hit].push(kids[i]);
    }
    var map = {}, nOk = 0;
        ...
        for (var s = 0; s < sheets.length; s++) {
            ...
            if (hardenGrids.length) {
                ...
                for (var gI = 0; gI < hardenGrids.length; gI++) {
                    ...
                    var mg = mesCut_hardenMasters(doc, gd, rotList, resizePct);
