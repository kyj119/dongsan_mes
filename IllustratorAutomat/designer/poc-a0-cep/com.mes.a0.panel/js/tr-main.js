...
(function () {
   ...
   * @param pre    { [panel]: {L:{j:code}, H:{j:code}} } — buildBleedPngs 가 **도련 래스터**에서 이미 정한 색(있으면 이것이 우선)
   */
  /** 창(±r px) 평균 밝기 → 'k'(밝은 바탕에 검정) | 'w'(어두운 바탕에 백) | 'x'(애매·투명 — 흑선+백테두리). 굽기·도련 래스터 공용. */
  function lumCodeAt(img, px, py, r) {
    var sum = 0, cnt = 0, opaque = 0;
    for (var yy = Math.round(py - r); yy <= py + r; yy++) {
      if (yy < 0 || yy >= img.H) continue;
      for (var xx = Math.round(px - r); xx <= px + r; xx++) {
        if (xx < 0 || xx >= img.W) continue;
        var o = (yy * img.W + xx) * 4, a = img.data[o + 3];
        cnt++;
        if (a < 128) continue;
        opaque++;
        sum += (0.299 * img.data[o] + 0.587 * img.data[o + 1] + 0.114 * img.data[o + 2]) / 255;
      }
    }
    if (!cnt || opaque < cnt / 2) return 'x';
    var lum = sum / opaque;
    return (lum > 0.6) ? 'k' : ((lum < 0.4) ? 'w' : 'x');
  }
  /**
   * ★표시 색은 **실제로 깔리는 도련 래스터**에서 정한다(2026-09-23 실기 — 고양 소노 판-1, 가운데 끈고리가 안 보였다).
   *   굽기 가장자리 창(±2.5mm 평균)으로 추정하면 틀린다: 사진 가장자리에 1~3mm 밝은 기둥이 있으면 픽셀 반복이 그것을
   *   시접 폭 23mm 로 늘려 띠 전체가 밝아지는데, 창 평균은 안쪽 어두운 그림에 끌려 「어둡다 → 백선」이 되고 백선이 흰 띠에 묻힌다.
   *   그래서 도련 그림(res)이 손에 있는 순간 그 자리를 직접 본다. 선은 길이를 따라 5점을 보고 **한 점이라도 다르면 x**(흑선+백테두리 — 어디서나 보인다).
   * @param res  repeatLastPixel 결과(pads 포함) · d = 그 벌의 원본 사각(판 mm) · mpp = 굽기 mm/px · lines = 접는선+끈고리(afterBleed 와 같은 순서)
   */
  function judgeMarksOnBleed(res, d, mpp, lines, holes, panelIdx) {
    var out = { L: {}, H: {} }, j, k;
    if (!res || !res.pads || !(mpp > 0)) return out;
    var r = Math.max(1, Math.round(2.5 / mpp));
    var at = function (xMm, yMm) { return lumCodeAt(res, (xMm - d.x) / mpp + res.pads.l, (yMm - d.y) / mpp + res.pads.t, r); };
    for (j = 0; j < lines.length; j++) {
      var ln = lines[j];
      if (ln.panel !== panelIdx) continue;
      var code = null;
      for (k = 0; k < 5; k++) {
        var c = at(ln.x + ln.len * (0.1 + 0.2 * k), ln.y);
        if (code === null) code = c; else if (c !== code) { code = 'x'; break; }
      }
      out.L[j] = code || 'x';
    }
    for (j = 0; j < holes.length; j++) {
      if (holes[j].panel !== panelIdx) continue;
      out.H[j] = at(holes[j].cx, holes[j].cy);
    }
    return out;
  }
  function sampleMarkColors(plan, cache, pre, cb) {
    ...
    for (i = 0; i < holes.length; i++) need[holes[i].panel] = true;
    var idxs = Object.keys(need).map(function (k) { return parseInt(k, 10); });
    var q = 0;
    (function step() {
      if (q >= idxs.length) {
        // ★도련 래스터에서 이미 정한 자리는 그 값을 쓴다(정확) — 없는 벌(단색·늘리기·굽기 실패)만 굽기 가장자리로 추정한다.
        for (i = 0; i < loops.length; i++) { var pl = pre && pre[loops[i].panel]; lc[i] = (pl && pl.L && pl.L[i]) ? pl.L[i] : judge(loops[i].panel, loops[i].x + loops[i].len / 2, loops[i].y); }
        for (i = 0; i < holes.length; i++) { var ph = pre && pre[holes[i].panel]; hc[i] = (ph && ph.H && ph.H[i]) ? ph.H[i] : judge(holes[i].panel, holes[i].cx, holes[i].cy); }
        cb(lc, hc);
        return;
      }
      var pi = idxs[q];
      if (cache[pi] && cache[pi].img) { q++; step(); return; }
      var d = plan.design[pi];
      var srcW = (lastEdges[pi] && lastEdges[pi].w > 0) ? lastEdges[pi].w : d.w;
      var mmppSrc = 1.0 * (srcW / d.w);                 // 배치 1mm/px 를 원본 단위로
      host('mesTr_bakeSlot(' + pi + ',' + mmppSrc.toFixed(5) + ')', function (rz, bad) {
        var m = String(rz).match(/path=(.+)$/);
        if (bad || !m) { cache[pi] = { img: null }; q++; step(); return; }
        IO.readPng(m[1], function (err, img) {
          cache[pi] = { img: (err || !img) ? null : img, mppPlaced: (img && img.W) ? (d.w / img.W) : 0 };
          q++; step();
        });
      });
    })();
    function judge(pi, xMm, yMm) {
      var c = cache[pi], d = plan.design[pi];
   ...
   */
  var bakeCacheRef = null;   // make() 가 잡아 준다 — 도련 굽기를 색 판정이 다시 쓴다
  var markPreRef = null;     // make() 가 잡아 준다 — 도련 래스터에서 미리 정한 표시 색(벌별 {L:{j:code}, H:{j:code}})
    ...
    (function step() {
      ...
      host('mesTr_bakeSlot(' + i + ',' + mmppSrc.toFixed(5) + ')', function (rz, bad) {
        ...
        IO.readPng(path, function (err, img) {
          ...
          if (typeof bakeCacheRef === 'object' && bakeCacheRef) bakeCacheRef[i] = { img: img, mppPlaced: d.w / img.W };
          ...
          // ★표시 색을 **여기서** 정한다 — 도련 그림이 손에 있는 유일한 순간이다(벌당 50MB 라 들고 있지 않고, 판정값만 남긴다).
          if (res && markPreRef && lastPlan && lastPlan.plate) {
            var pp = lastPlan.plate;
            try { markPreRef[i] = judgeMarksOnBleed(res, d, mpp, (pp.folds || []).concat(pp.loops || []), pp.holes || [], i); } catch (eJ) { markPreRef[i] = null; }
          }
  ...
  function make() {
    ...
    function afterBleed(note) {
      ...
      sampleMarkColors(p, bakeCache, markPre, function (lc, hc) {
        var xs = 0, j;
