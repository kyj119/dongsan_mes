  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function rescuePending(cb) {
      hostEval('mesA0_manifestPending()', function (ps, pbad) {
        ...
        // ★커밋이 끝났다 = 에이전트가 읽어 갈 수 있다. 픽업 복사는 **에이전트가** 한다(2026-09-15).
        //   여기서 호스트를 한 번 더 부르는 이유는 복사가 아니라 **인계 상태 비우기**다 —
        //   안 비우면 다음 건의 실패 판정이 앞 건의 잔여를 본다(`mesA0MfPending`).
        hostEval('mesA0_manifestDone()', function () { cb(rr, ''); });
      });
    }
    ...
    // 결과 요약 규칙 정본 = js/panel-ux.js(재단 탭과 공용). 모듈이 없으면 종전처럼 전문.
    var a0OutState = { open: false };
    // 요약에서도 위에 남길 줄 — 등록·실물·EPS·DXF·폴더·산출물 용량(panel:smoke 가 이 줄들이 **보이는지** 검사한다)
    var A0_KEEP_LINES = /^(등록|실물|EPS|DXF|폴더)[: ]|산출물 /;
    function out(t, cls) {
      if (!elOut) return;
      if (window.MesPanelUx) { window.MesPanelUx.renderSummary(elOut, t, cls, a0OutState, A0_KEEP_LINES); return; }
      elOut.textContent = t; elOut.className = 'out' + (cls ? ' ' + cls : '');
    }
    ...
    if (elBtnEnv && elEnvOut) elBtnEnv.addEventListener('click', function () {
      ...
      hostEval('typeof mesA0_envCheck === "function" ? mesA0_envCheck() : "na"', function (res, bad) {
        ...
        // ★일러 버전을 **실적과 대조한다** (2026-09-15). 여태 이 줄은 버전을 보여만 줬다.
        //   Z: 등록 폴더 20건을 PC 별로 갈라 보니 실패가 한쪽에만 쏠려 있었다:
        //     DESKTOP-62VJ5FA(일러 30.7.0) 9건 중 잔해 0 · DESKTOP-6JSH6OL(30.3.0) 11건 중 잔해 9.
        //   용량 탓도 아니다 — 30.7 쪽은 25.2MB 를 성공시켰고 30.3 쪽은 2.2MB 에서 실패했다.
        //   ⚠️PC 와 버전이 묶여 있어 **원인을 버전으로 단정할 수는 없다**(그 PC 자체일 수 있다).
        //   그래서 「버그」라 쓰지 않고 **「실적이 없는 버전」**이라고 쓴다 — 올리면 분리되는 실험이기도 하다.
        var AI_OK_FROM = 30.7;   // 무결점 실적이 있는 최저 버전. 실적이 쌓이면 내린다
        var aiNum = parseFloat(String(e.ai || '').split('.').slice(0, 2).join('.'));
        var aiLow = !isNaN(aiNum) && aiNum < AI_OK_FROM;
        L.push('PC ' + e.pc + ' · 일러 ' + e.ai + (aiLow ? '  ⚠ 실적 없는 버전' : ''));
        // ★문구가 **이 PC 의 버전에 없는 전적을 뒤집어씌우면 안 된다** (2026-09-17 정정).
        //   종전 문구는 「등록 실패는 전부 이 버전 PC 에서 나왔다(잔해 9/11)」였는데, 그 9/11 은
        //   **30.3.0** 의 기록이다. 30.6.0 PC 에 그대로 떠서 사실이 아닌 말을 했다 —
        //   바로 위 주석에 「원인을 버전으로 단정할 수 없다」고 써 놓고 문구는 단정했다.
        //   실측한 것만 쓴다: 무결점 실적이 있는 최저 버전은 30.7 이고, 그 아래는 **모름**이다.
        if (aiLow) bad2.push('일러 ' + e.ai + ' 는 이 시스템에서 **무결점 실적을 확인하지 못한** 버전입니다 — 실적이 있는 최저 버전은 '
          + AI_OK_FROM + ' 입니다(그 아래가 나쁘다는 뜻이 아니라, 확인된 바가 없다는 뜻입니다). '
          + '이 PC 에서 등록이 실패한 적이 있다면 업데이트 후 1건만 다시 돌려 보세요 — 버전 탓인지 PC 탓인지는 그것으로만 갈립니다');
    ...
    var warnKo = { R: '원본 RGB→CMYK 변환', T: '아웃라인 안 된 텍스트', L: '링크(미임베드) 이미지', O: '아웃라인 일부 실패',
      ...
      // P = 오늘 같은 이름의 산출물이 이미 있다(호스트 0.20.0 mesA0_pickupClash).
      //     픽업 폴더 `_출력/<날짜>` 는 **날짜당 한 자리**이고 에이전트는 크기가 다르면 덮어쓴다 →
      //     등록·주문·카드는 둘 다 멀쩡한데 **재단기에 가는 실물만 나중 것 하나**가 된다.
      //     시스템이 이름을 안 바꾸는 이유 = 이 파일명이 그대로 RIP 에 나가는 이름이다(08-27 규약).
      P: '오늘 같은 이름의 출력 파일이 이미 있습니다 — 픽업 폴더(_출력)는 날짜당 한 자리라 나중 것이 앞 것을 덮습니다.'
        + '\n   다른 디자인이면 [내용]을 다르게 적어 다시 등록하세요 (같은 건을 다시 낸 것이면 그대로 두어도 됩니다)' };
    function mbText(bytes) { // 용량을 눈에 보이게 = 커지는 걸 알아채는 유일한 지점
      // ★재는 대상이 **산출물**로 바뀌었다 (2026-09-15, 호스트 0.16.0) — 단건은 EPS, 모아찍기는 work.ai.
      //   work.ai 를 단건에서 더 이상 안 쓰기 때문이고, 「커지는 걸 알아챈다」는 목적은 그대로다.
      // ★`0MB` 를 함부로 쓰지 않는다 — 호스트가 저장 직후 길이를 못 읽으면 -1 을 보낸다.
      //   1.7MB 파일이 `0MB` 로 떠서 산출물이 깨진 줄 알았던 실기 전례가 있다.
      //   0.1MB 미만도 `0MB` 로 반올림되면 같은 오해를 부르므로 `<0.1MB` 로 쓴다.
      if (bytes == null || bytes === '') return '';
      if (bytes < 0) return ' · 산출물 크기 확인 못 함';
      if (!bytes) return '';
      var mb = Math.round(bytes / 1048576 * 10) / 10;
      return ' · 산출물 ' + (mb < 0.1 ? '<0.1' : mb) + 'MB';
    }
    function warnText(w) {
      if (!w) return '';
      var out = [];
      ...
      hostEval('mesA0_paramsPath()', function (pp, bad) {
        ...
        function renderOk(r, extra) {
          var msg = '가공 완료 ✓\n등록: ' + (params.client_name || '(파일명)') + ' · 수량 ' + params.qty +
            ...
            '\n→ 에이전트가 대기함에 올리고 `_출력` 픽업까지 맡습니다.';
          out(msg + '\n(후가공 설정은 초기화됨 — 다음 건에 상속되지 않습니다)' + (extra || ''), 'okmsg');
          clearFinishing();  // 등록 완료 = 후가공 리셋. 연속 작업에서 앞 건 마감이 뒤 건에 새는 사고 차단.
          saveSettings();
          // ★대기함 수신 확인 — 조회 함수는 재단 호스트(0.59.0+)에 있다(한 엔진에 두 호스트가 같이 로드된다)
          if (window.MesPanelUx && r.folder) window.MesPanelUx.watchIngest({ hostCall: hostEval, names: String(r.folder),
            lineEl: $('a0RegWatch'), isBusy: function () { return hostBusy; }, state: a0Ingest });
        }
    ...
    var a0Tick = null, a0Ingest = { timer: null };
    function setHostBusy(on, label) {
      hostBusy = !!on;
      // ★경과 초 — 가공은 호스트 호출 하나가 수십 초라 일러가 멈춘 것처럼 보인다(패널은 별도 프로세스라 센다)
      if (a0Tick) { a0Tick.stop(); a0Tick = null; }
      if (on && window.MesPanelUx) a0Tick = window.MesPanelUx.ticker($('a0Prog'), (label || '작업') + ' 중');
      ...
      hostEval('mesA0_batchBegin()', function (bres, bad0) {
        ...
        function finishBatch() {
          ...
          for (var k = 0; k < results.length; k++) {
            ...
            if (r && r.ok) { okN++; okIdx.push(k); lines.push('#' + (k + 1) + ' ✓ ' + (r.eps || '(모아찍기 work.ai)') + (r.dxf ? ' +DXF' : '') + mbText(r.bytes) + warnText(r.warn).replace(/\n/g, ' ')); }
