...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function mbText(bytes) { // 용량을 눈에 보이게 = 커지는 걸 알아채는 유일한 지점
      // ★`0MB` 를 함부로 쓰지 않는다 (2026-09-15) — 호스트가 저장 직후 길이를 못 읽으면 -1 을 보낸다.
      //   1.7MB 파일이 `work.ai 0MB` 로 떠서 산출물이 깨진 줄 알았던 실기 전례가 있다.
      //   0.1MB 미만도 `0MB` 로 반올림되면 같은 오해를 부르므로 `<0.1MB` 로 쓴다.
      if (bytes == null || bytes === '') return '';
      if (bytes < 0) return ' · work.ai 크기 확인 못 함';
      if (!bytes) return '';
      var mb = Math.round(bytes / 1048576 * 10) / 10;
      return ' · work.ai ' + (mb < 0.1 ? '<0.1' : mb) + 'MB';
    }
    function warnText(w) {
      if (!w) return '';
      var out = [];
    ...
    if (elBtnProcess) elBtnProcess.addEventListener('click', function () {
      ...
      hostEval('mesA0_paramsPath()', function (pp, bad) {
        ...
        function renderOk(r, extra) {
          ...
          out(msg + (r.outcopy ? ('\n· `_출력` 픽업은 에이전트가 맡습니다 — 일러 폴백 실패: ' + r.outcopy +
              '\n  → 잠시 뒤 재단기 폴더에 들어갑니다(5분 넘게 안 보이면 알려 주세요)') : '') +
