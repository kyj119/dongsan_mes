...
(function () {
  ...
  var SHELL_VERSION = '0.104.0';   // 0.104.0 = ★판짜기 사용감 2차(2026-09-27) — 결과창 요약(첫 줄+경고, 상세는 접기) · 진행 단계·경과 초·[취소](적용 전까지) · 등록 후 대기함 수신 확인(호스트 0.59.0+). 잃는 것 = 결과창 상세가 기본 접힘(한 번 펼치면 기억).
  ...
  //
  // ★코너 7mm + **반지름 3mm** = 10. 지름(6)을 더하면 아무것도 없는 3mm 를 매 변마다 버린다 —
  //   돔보 중심은 코너에서 7mm 이고 원은 반지름만큼만 더 뻗기 때문이다.
  //   실물 30쌍도 원 바깥끝이 판 가장자리에 정확히 접한다(2026-08-02 실측). 변당 3mm 절감·위험 0.
  var DOMBO_MARGIN_MM = 10;
  ...
  var hostStaleZ = '';
  /** a 가 b 보다 새 버전인가 — 접두어(`CUT-CEP-`)를 떼고 숫자 부분만 비교. 못 읽으면 「다르면 새것」(종전 동작) */
  function verNewer(a, b) {
    var num = function (s) { var m = /(\d+(?:\.\d+)*)\s*$/.exec(String(s)); return m ? m[1].split('.').map(function (x) { return parseInt(x, 10) || 0; }) : null; };
    var pa = num(a), pb = num(b);
    if (!pa || !pb) return String(a) !== String(b);
    for (var i = 0; i < Math.max(pa.length, pb.length); i++) { var d = (pa[i] || 0) - (pb[i] || 0); if (d) return d > 0; }
    return false;
  }
  ...
  function checkHostFresh(cb) {
    host(Z_VER_JS, function (zv) {
      ...
      // ★「다르다」가 아니라 **Z: 가 더 새로울 때만**(0.98.0) — 주석(위)이 원래 그렇게 적고 있었다.
      //   로드본이 더 새 것(이 PC 에서 새 호스트를 원격으로 시험하는 중)은 옛 규칙이 아니므로 막을 이유가 없다.
      hostStaleZ = (known && hostVersion && verNewer(z, hostVersion)) ? z : '';
      if (elVer) {
  ...
  // ★조각 번호 — 조각 경계(`mesCut_nestBounds`)·원본 PNG(`mesCut_exportOverview`)·`T` 줄을 아는 호스트인가 (2026-09-23).
  //   못 받으면 번호를 만들지 않고 **결과창에 그 사실을 쓴다**(조용히 빠지지 않는다).
  var NUM_MIN_HOST = [0, 51, 0];
  function hostSupportsNumber() { return hostAtLeast(NUM_MIN_HOST); }
  // ★사각 빠른 경로 — 벡터 사각 판정(`mesCut_rectProbe`)·띠 굽기(nestBakeAll stripMm)·띠 도련(`LS`)을 아는 호스트인가 (2026-09-23).
  var RECT_MIN_HOST = [0, 54, 0];
  function hostSupportsRect() { return hostAtLeast(RECT_MIN_HOST); }
  function rectFastOn() { var el = document.getElementById('nestRectFast'); return !!(el && el.checked); }
  // ★도련 색(2026-09-26) — 조각별 사진 여부(`mesCut_photoFlags`)를 아는 호스트인가 · 선택값(기본 자동)
  // ★벡터 칼선 잡의 굽기 해상도(2026-09-26 용준님 확정 1.0mm/px · 실물). 칼선을 호스트가 벡터로 그리면
  //   미세 마스크는 **배치(3mm 격자로 내려 씀)와 도련**에만 쓰인다 — 0.25~0.75mm 로 구울 이유가 없다.
  //   원격 실측: 14조각 1/10 60→47초 · 이형 9조각 75→60초, 판·길이 동일, 도련 계단 ≈1mm(칼선 3mm 바깥).
  //   래스터 칼선은 칼선 정밀도가 이 해상도에 달려 있으므로 **적용하지 않는다**. 이미 더 거칠면 그대로 둔다.
  var VEC_BAKE_MM = 1.0;
  var nestBakeCoarse = false;          // runNest 가 nestPrepare 직전에 켜고 prep 콜백에서 끈다
  var PHOTO_MIN_HOST = [0, 57, 0];
  function hostSupportsPhoto() { return hostAtLeast(PHOTO_MIN_HOST); }
  function bleedColorMode() { var el = document.getElementById('bleedColor'); var v = el && el.value; return (v === 'edge' || v === 'skip') ? v : 'auto'; }
  ...
  /**
   * 호스트가 보낸 ASCII 사유 코드 → 사람 말.
   * ★브릿지는 ASCII 만 나르므로(호스트 :1966) 번역은 **패널 한 곳**에서 한다.
   *   호출부마다 번역하면 새 코드가 생길 때 어디는 번역되고 어디는 코드가 그대로 뜬다.
   */
  function hostWhy(s) {
    var t = String(s);
    if (t.indexOf('outline-cmd-noop') >= 0) {
      return '획→면 변환(Outline Stroke)이 이 일러에서 먹지 않았습니다.\n'
        + '메뉴 명령 ID 가 일러 업데이트로 바뀐 것으로 보입니다 — **칼선을 만들지 않았습니다**\n'
        + '(틀린 칼선이 재단기로 나가는 것보다 안전합니다).\n'
        + '관리자에게 이 줄을 그대로 전달하세요: ' + t.replace(/^ERROR\s*/, '');
    }
    return t;
  }

  /**
   * 일괄 굽기가 왜 안 됐나 — 호스트 `efswhy=` 코드를 사람 말로.
   * ★조용한 격하를 세운다: 옛 경로로 떨어져도 **판은 나온다**. 느려질 뿐이라 아무도 못 알아채고,
   *   그 사이 원인(조각이 너무 크다·이전 파일이 안 지워진다)은 그대로 남는다.
   */
  function efsWhyText(code) {
    var t = String(code || '');
    var m = /^px-side-over\s+(\S+)=(\d+)x(\d+)\s+>\s+(\d+)/.exec(t);
    if (m) {
      return '조각 ' + m[1] + ' 이 ' + m[2] + '×' + m[3] + 'px 로 굽기 상한 ' + m[4] + 'px 을 넘습니다.\n'
        + '  → 「굽기 격자(mm/px)」를 키우거나 조각을 나누세요(면적이 아니라 **한 변**이 문제입니다).';
    }
    var m2 = /^missing\s+(\S+)/.exec(t);
    if (m2) {
      return '일러가 ' + m2[1] + ' 을(를) 만들지 못했습니다 — 「…을(를) 전송할 수 없습니다」 대화상자가 떴다면 그것입니다.\n'
        + '  → 「굽기 격자(mm/px)」를 키워 보세요. 그래도 나면 관리자에게 이 줄을 전달하세요: ' + t;
    }
    var m3 = /^purge-left\s+(\d+)/.exec(t);
    if (m3) {
      return '이전 굽기 파일 ' + m3[1] + '개를 지우지 못했습니다 — 덮어쓰기가 막혀 굽기가 거절됩니다.\n'
        + '  → 일러를 껐다 켜거나 %TEMP%\\mes_cut_efs_* 폴더를 지우세요.';
    }
    if (t.indexOf('no-efs-api') === 0) return '이 일러 버전에 일괄 굽기 API(exportForScreens)가 없습니다 — 느린 경로로 동작합니다.';
    return t;
  }
  ...
  function host(expr, cb) {
    cs.evalScript(expr, function (res) {
      ...
      // ★호스트가 아예 안 실렸다 — 「함수가 아닙니다」만으로는 원인도 조치도 알 수 없다(2026-09-09).
      //   실기: 파일 **읽기**가 죽어 $.evalFile 이 Z: 의 호스트를 못 읽었다.
      if (/is not a function|함수가 아닙니다/.test(s) && /mesCut_|mesA0_/.test(s)) {
        cb('ERROR Z: 의 호스트 스크립트를 못 읽었습니다 — 이 PC 의 일러가 파일을 못 읽는 상태입니다.\n'
          + '· 일러스트레이터를 **완전히 종료**했다 다시 켜 주세요(패널만 닫았다 여는 것으로는 안 바뀝니다).\n'
          + '· 그래도 같으면 Z: 연결을 확인하세요.\n'
          + '(원문: ' + s.slice(0, 120) + ')', true);
        return;
      }
      cb(hostWhy(s), s.indexOf('ERROR') === 0);
    });
  }
  ...
  /**
   * 점유 안내 — 사람이 **다음에 뭘 할지**까지 말한다 (2026-09-09).
   * ★여태 5곳이 같은 문구를 각자 만들어, 안내를 고치려면 5곳을 고쳐야 했다.
   * ★잠금은 OS 파일 잠금이 아니라 %TEMP% 의 **표식 파일 1개**다(정본 = mes-lock.jsx).
   *   아무것도 붙잡고 있지 않으므로 지우면 즉시 풀린다 — 그걸 사람이 알아야 한다.
   * ★범위는 **이 PC 의 일러 1대**다. 다른 디자이너·다른 PC 는 서로 막지 않는다.
   */
  function busyMsg(lk) {
    var body = String(lk).substring(5);
    var m = /:age=(\d+)ms$/.exec(body);
    var mins = m ? Math.floor(parseInt(m[1], 10) / 60000) : -1;
    if (m) body = body.substring(0, m.index);
    return '다른 쪽이 일러를 점유 중입니다: ' + body
      + (mins >= 0 ? (' · ' + mins + '분 경과') : '')
      + '\n· 가공 탭 작업이 끝나면 자동으로 풀립니다.'
      + '\n· 10분이 지나면 자동으로 해제됩니다.'
      + '\n· 그래도 안 풀리면 %TEMP% 폴더의 mes_host_lock.txt 를 지우세요(이 PC 안에서만 쓰는 표식 파일입니다).';
  }
  ...
  // ★정본은 `js/png-io.js` 다 — **전사와 같은 코드**를 쓴다(도련 엔진이 공유인데 입출구가
  //   사본이면 한쪽에서 고친 것이 다른 쪽에 안 온다). 여기 남은 것은 호출 이름뿐이다.
  function readPng(path, cb) { return window.MesPngIo.readPng(path, cb); }
  ...
  /**
   * ★실제로 쓰는 판 길이 상한(파일 좌표) — 사람이 정한 **실물** 최대 길이와 일러 한계 중 작은 것 (2026-09-26 A안).
   *   칸이 비었거나 이상한 값(500mm 미만)이면 **종전과 똑같이** NEST_ROLL_MAX_MM 만 쓴다(기본 동작 불변).
   *   사람 값은 장비(재단기·RIP)의 실물 한계라 toFileMm 으로 파일 좌표로 바꾼다 — 일러 한계(NEST_ROLL_MAX_MM)는
   *   위 주석대로 파일 좌표 그대로다(저장 배율을 낮추면 한 판에 더 담는 성질을 지킨다).
   *   ⚠️해상도 고르기(nestResolution)는 여전히 NEST_ROLL_MAX_MM 을 쓴다 — 판 상한을 줄여도 격자가 바뀌지 않게(배치 안정).
   */
  var NEST_MAXLEN_KEY = 'mesCut.nestMaxLen';
  function userMaxLenReal() {
    var el = document.getElementById('nestMaxLen');
    var v = el ? parseFloat(el.value) : NaN;
    return (v >= 500) ? v : 0;
  }
  function rollMaxMm() {
    var u = userMaxLenReal();
    return u ? Math.min(NEST_ROLL_MAX_MM, toFileMm(u)) : NEST_ROLL_MAX_MM;
  }
  (function wireMaxLen() {
    var el = document.getElementById('nestMaxLen');
    if (!el) return;
    try { var sv = window.localStorage.getItem(NEST_MAXLEN_KEY); if (sv) el.value = sv; } catch (e) { /* ignore: 저장소가 막힌 PC — 빈 칸(종전 동작)으로 시작 */ }
    el.addEventListener('change', function () {
      try { window.localStorage.setItem(NEST_MAXLEN_KEY, userMaxLenReal() ? String(userMaxLenReal()) : ''); } catch (e2) { /* ignore: 저장 실패해도 이번 실행 값은 칸에 남는다 */ }
    });
  })();
  ...
  /**
   * ★조각을 사각으로 바꾼 사실을 **어느 조각인지까지** 말한다 (2026-09-04 용준님 지적).
   *
   * 전에는 「조각 6개는 … 사각으로 잘랐습니다」 라고 **개수만** 말했다. 번호가 없으니 화면의 어느
   * 조각인지 대조할 수 없고, 그래서 사각으로 보이는 조각이 ⓐ원래 테두리가 사각인 아트인지
   * ⓑ이 규칙(떨어진 덩어리 → bbox)에 걸린 것인지 **사용자도 우리도 못 갈랐다.**
   * 번호는 [조각 수량] 목록의 `#N` 과 같고 그 행을 누르면 일러에서 그 조각이 선택된다
   * (`mesCut_nestSelect`) — 번호 하나면 대조가 끝난다.
   *
   * ★★그리고 **벡터 모드에서 「잘랐습니다」는 거짓이다.** 이 사각은 **배치용 마스크**일 뿐이고,
   *   칼선은 호스트가 아트에서 직접 딴 실루엣이다(`mesCut_vecSilhouette`). 실제로 패널은
   *   벡터일 때 `pieceCutLines` 를 아예 안 부른다(`wantPieceCut && !useVec`).
   *   문구가 동작과 달라 **사각의 원인을 잘못 짚게 만들었다** — 모드를 받아 사실대로 쓴다.
   *   (같은 종류의 전례: 도련 방식 칸이 「이 버튼 전용」이라 적힌 채 판짜기가 읽고 있었다)
   */
  function weldNote(prep, vec, vecDropStr) {
    // ★개행은 String.fromCharCode(10) 으로 만든다 — 역슬래시 이스케이프가
    //   패치 중에 한 번 풀리면 문자열이 깨진다(2026-09-04 실제로 그랬다).
    var BR = String.fromCharCode(10);
    var f = prep.filledIds || [], b = prep.boxedIds || [];
    var drop = [];
    if (vecDropStr) {
      var parts = String(vecDropStr).split(',');
      for (var p = 0; p < parts.length; p++) {
        var v = parseInt(parts[p], 10);
        if (!isNaN(v)) drop.push(v);
      }
    }
    if (!f.length && !b.length && !drop.length) return '';
    var list = function (a) { return '#' + a.join('·#'); };
    var s = '';
    if (f.length) {
      s += BR + '※ 조각 ' + list(f) + ' 은 **속을 메워** 외곽 하나로 잡았습니다'
        + '(배치 기준 — 칼선은 아래 줄을 보세요).';
    }
    if (b.length) {
      // ★벡터 모드에서 이 사각은 **배치용**이다 — 칼선은 호스트가 아트에서 직접 딜다.
      //   예전에 「잘랐습니다」라고 써 사각 칼선의 원인을 잘못 짚게 만들었다.
      s += BR + '※ 조각 ' + list(b) + ' 은 떨어진 덩어리가 여럿이라 **배치**를 바깥 사각으로 잡았습니다'
        + (vec ? ' — 칼선은 요소별로 그대로 나옵니다.' : ' — 래스터 칼선은 이 사각을 따릅니다.');
    }
    if (drop.length) {
      // ★조용히 지우지 않는다 — 칼선을 한 줄이라도 건드렸으면 말한다.
      s += BR + '※ 조각 ' + list(drop) + ' 에서 **자를 수 없는 부스러기**를 걷어냈습니다'
        + '(0.1mm 미만 — 두면 재단기가 제자리에서 칼을 내렸다 올립니다).';
    }
    s += BR + '   번호 = [조각 수량] 목록의 #N. 그 행을 누르면 일러에서 그 조각이 선택됩니다.';
    return s;
  }
  ...
  function nestPrepare(G, rez, gapMm, offsetMm, fillMode, fail, cb) {
    var numBounds = null, numOv = null;   // 조각 번호 — 경계·원본 PNG(번호가 켜져 있고 호스트가 받을 때만 채워진다)
    ...
    host('mesCut_nestBegin(' + (pieceQty ? '1' : '') + ')', function (bg, bad) {
      ...
      host('mesCut_nestSizes()', function (szStr) {
        ...
        // ★조각 번호 — 원본이 **활성인 지금**(판 문서를 만들기 전) 경계와 원본 PNG 를 받는다. 꺼져 있으면 즉시 다음으로.
        numFetchForNest(function (nb, nov) {
          numBounds = nb; numOv = nov;
      ...
      /**
       * ★사각 빠른 경로 (2026-09-23 · 정본 js/rect-fast.js) — 켜져 있으면 **굽기 전에** 시도한다.
       *   3겹 판정(①벡터 모양 ②저해상도 채움 — 여기 · ③띠 불투명 — 도련 단계)을 하나라도 못 넘으면
       *   **종전 경로가 그대로 돈다**(결과물 불변의 전제). 못 쓴 사유는 결과창에 한 줄로 남긴다(조용한 격하 금지).
       *   통과하면 굽은 마스크 대신 **같은 규격의 합성 사각 마스크**를 넘긴다 — 배치·판 나누기·번호 꼬리표·효율%·
       *   맞붙임이 종전 코드 그대로 돈다(사각을 완벽하게 구운 것과 같다).
       */
      function prepareWith(fv, selAreaMm2, sizeList) {
        if (!rectFastOn()) { prepareSlow(fv, selAreaMm2, sizeList, ''); return; }
        rectFast(fv, sizeList, function (prepRect, why) {
          if (prepRect) { cb(prepRect); return; }
          prepareSlow(fv, selAreaMm2, sizeList, '\n※ 사각 빠른 경로를 쓰지 않았습니다 — ' + why + ' (종전 방식으로 짰습니다).');
        });
      }

      function rectFast(fv, sizeList, done) {
        var RF = window.MesCutRectFast;
        if (!RF) { done(null, 'rect-fast.js 미로드(패널 설치본 확인)'); return; }
        var d0 = RF.decide({ enabled: true, hostOk: hostSupportsRect(), fill: !!fv.fill, probe: [{ rect: true }] });
        if (!d0.on) { done(null, d0.why + (d0.stage === 'host' ? '(' + (hostVersion || '?') + ' < CUT-CEP-0.54.0)' : '')); return; }
        out('사각 판정 중...');
        host('mesCut_rectProbe()', function (pr, badP) {
          if (badP || String(pr).indexOf('ok;') !== 0) { done(null, '조각 모양을 읽지 못했습니다(' + pr + ')'); return; }
          var parts = String(pr).substring(3).split(','), probe = [], i;
          for (i = 0; i < parts.length; i++) {
            var t = parts[i].split(':');
            probe.push(t[0] === 'r' ? { rect: true, w: parseFloat(t[1]), h: parseFloat(t[2]) } : { rect: false, why: t[1] || '?' });
          }
          if (probe.length !== n) { done(null, '조각 수가 맞지 않습니다(' + probe.length + '/' + n + ')'); return; }
          var d1 = RF.decide({ enabled: true, hostOk: true, fill: false, probe: probe, check: null });
          if (!d1.on) { done(null, d1.why); return; }
          // ② 저해상도 채움 — 굽기 좌표 규약은 종전 굽기와 같다(격자 × S/F · buttPlace 의 `w / k` 와 같은 환산)
          var k = fileToSave() || 1;
          var chk = toFileMm(RF.CHECK_MMPP);
          var padMm = Math.max(offsetMm, 0) + gapMm + 1;
          out('사각 확인 굽기 중...');
          host('mesCut_nestBakeAll(' + (chk * k) + ',' + (padMm * k) + ',false,"rchk")', function (rz, badB) {
            if (badB || String(rz).indexOf('ok;') !== 0) { done(null, '확인 굽기 실패(' + String(rz).split(/[\r\n]/)[0] + ')'); return; }
            var rows = String(rz).split(/[\r\n]+/), list = [];
            for (var r = 1; r < rows.length; r++) {
              var tt = rows[r].split(' ');
              if (tt[0] === 'P') list.push({ id: parseInt(tt[1], 10), path: tt.slice(6).join(' ') });
            }
            if (list.length !== n) { done(null, '확인 굽기 ' + list.length + '/' + n + '개'); return; }
            var check = [], q = 0;
            (function step() {
              if (q >= list.length) {
                var d2 = RF.decide({ enabled: true, hostOk: true, fill: false, probe: probe, check: check });
                if (!d2.on) { done(null, d2.why); return; }
                done(buildRectPrep(RF, fv, probe, k, padMm), '');
                return;
              }
              out('사각 확인 ' + (q + 1) + '/' + list.length);
              readPng(list[q].path, function (err, img) {
                if (err) { done(null, '확인 이미지를 읽지 못했습니다'); return; }
                var pr0 = probe[list[q].id];
                check[list[q].id] = RF.strictRect(RF.alphaMask(img), (pr0.w / k) / chk, (pr0.h / k) / chk);
                q++; step();
              });
            })();
          });
      ...
      /** 합성 사각 마스크로 prep 을 만든다 — 종전 finishPrep 과 **같은 모양**(하류가 구분하지 못해야 한다). */
      function buildRectPrep(RF, fv, probe, k, padMm) {
        var mmpp = rez.mmPerPx, padPx = Math.round(padMm / mmpp);
        var pieces = [], rawInk = 0, sizesS = {}, sizesF = [];
        for (var id = 0; id < n; id++) {
          var wS = probe[id].w / k, hS = probe[id].h / k;
          var s = RF.syntheticBase(wS, hS, mmpp, padPx);
          pieces.push({ id: id, ink: s.inkW * s.inkH, W: s.W, H: s.H, m: growMask(G, s.m, s.W, s.H, rez.rPx), base: { W: s.W, H: s.H, m: s.m } });
          rawInk += s.inkW * s.inkH;
          sizesS[id] = { w: wS, h: hS };
          sizesF.push({ w: probe[id].w, h: probe[id].h });   // 맞붙임(buttPlace)이 쓰는 크기 — 벡터 실측(소수 3자리)
        }
        var sub = subPxFactor(mmpp), fineMmpp = mmpp / sub;
        return {
          n: n, pieces: pieces, rawInkPx: rawInk, mmpp: mmpp, sizes: sizesF,
          rPx: rez.rPx, exact: rez.exact, sub: sub, fineMmpp: fineMmpp, softened: 0, safetyMm: rez.safetyMm,
          edgeNote: '', offsetMm: offsetMm, cutFinePx: Math.floor(offsetMm / fineMmpp),
          fillNote: fv.note, lineArt: fv.lineArt, fill: fv.fill,
          bakeNote: '\n사각 빠른 경로 ON — 조각 ' + n + '개 (전체 굽기 없음 · 칼선은 계산 · 도련은 네 변 띠)',
          bakeWay: 'rect', inkList: [], filledIds: [], boxedIds: [],
          bounds: numBounds, overview: numOv,
          rect: { sizesS: sizesS, k: k },
        };
      }

      function prepareSlow(fv, selAreaMm2, sizeList, rectWhy) {
      ...
      //   있었는데 **사후에 판별할 방법이 없었다** — 호스트는 `fastbake=`·`docs=` 를 응답 첫 줄에
      //   실어 보내는데 패널이 그 줄을 버리고 있었고, 콘솔에도 아무것도 안 남았다.
      //   경로가 갈리면 마스크가 갈리고, 마스크가 갈리면 잉크·효율이 갈린다 → 반드시 보이게 둔다.
      var bakeWay = 'seq';
      ...
      // ★시험용 스위치(2026-09-25) — 굽기 해상도만 바꿔 속도·도련 품질을 비교한다. **배치 격자는 그대로**
      //   (sub 만 바꾼다 → 미세 마스크를 같은 배치 격자로 내려 쓰므로 판 배치가 흔들리지 않는다).
      //   원격(CDP)에서 `window.__mesCutBakeSub = 2|3|4` 로만 켠다 — 평소엔 undefined 라 동작 불변.
      if (!(window.__mesCutBakeSub > 0) && nestBakeCoarse) {
        // sub 를 줄여 미세 격자를 목표(1.0mm)에 가깝게 — 배치 격자(rez.mmPerPx)는 그대로라 판 배치가 흔들리지 않는다
        var subV = Math.max(1, Math.round(rez.mmPerPx / toFileMm(VEC_BAKE_MM)));
        if (subV < sub) {
          var oldFineV = fineMmpp;
          sub = subV; fineMmpp = rez.mmPerPx / sub;
          cutFinePx = Math.floor(offsetMm / fineMmpp);
          bakeNote += '\n※ 칼선이 벡터라 굽기를 ' + toRealMm(oldFineV).toFixed(2) + ' → ' + toRealMm(fineMmpp).toFixed(2)
            + 'mm/px 로(배치·도련 전용 — 칼선·판 배치 불변).';
        }
      }
      if (window.__mesCutBakeSub > 0) {
        sub = Math.max(1, Math.round(window.__mesCutBakeSub));
        fineMmpp = rez.mmPerPx / sub;
        cutFinePx = Math.floor(offsetMm / fineMmpp);
        bakeNote += '\n※ [시험] 굽기 해상도 강제 sub=' + sub + ' → ' + fineMmpp.toFixed(3) + 'mm/px(배치 격자 ' + rez.mmPerPx.toFixed(3) + ' 그대로).';
      }
      ...
      function finishPrep() {
        cb({
          ...
          bakeNote: bakeNote, bakeWay: bakeWay,
          // ★도련이 다시 구울 필요가 없게 경로를 넘긴다. 비면 옛 경로가 스스로 굽는다(하위호환).
          inkList: inkList,
          // ★조용히 바꾸지 않는다 — **어느 조각을** 바꿨는지 번호로 남긴다. 문장은 벡터/래스터를
          //   아는 보고 지점에서 만든다(weldNote) — 여기서 만들면 모드를 몰라 거짓을 쓴다.
          filledIds: filledIds, boxedIds: boxedIds,
          // 조각 번호 — 경계(mm·문서 좌표)와 원본 PNG 메타. 없으면 null(번호 꺼짐·구 호스트).
          bounds: numBounds, overview: numOv,
        });
      }
      ...
      function bakeAll() {
        ...
        host('mesCut_nestBakeAll(' + (fineMmpp * bakeK) + ',' + (padMm * bakeK) + ',' + (fv.fill ? 'true' : 'false')
          ...
          if (badB || rz.indexOf('ok;') !== 0) {
            ...
            bakeWay = 'fallback';
            // ★여기가 `p0.png 를 내보낼 수 없음` 이 나는 자리다 — 단계별 파일쓰기를 꺼내 붙인다.
            //   굽기 **전에** 이미 X 였으면 굽기 탓이 아니고, 굽기 **후**에 X 면 굽기가 방아쇠다.
            host('typeof mesCut_ioProbeGet === "function" ? mesCut_ioProbeGet() : ""', function (ipr) {
              if (ipr && String(ipr).indexOf('=') > 0) bakeNote += '\n[단계별 파일쓰기] ' + ipr;
              i = 0; next();
            });
            return;
          }
          var rows = String(rz).split(/[\r\n]+/), list = [];
          // ★응답 첫 줄이 경로를 말한다 — 여태 버리고 있었다(위 bakeWay 주석 참조).
          var fbM = /;fastbake=(\d+)/.exec(rows[0] || ''), dcM = /;docs=(\d+)/.exec(rows[0] || '');
          bakeWay = 'batch' + (fbM ? (fbM[1] === '1' ? '' : '-mixed') : '')
            + (dcM ? ('/' + dcM[1] + '문서') : '');
          // ★일괄이 **부분적으로라도** 옛 경로로 떨어졌으면 그 사유를 화면에 쓴다(2026-09-08).
          //   여태는 `fastbake=0` 이어도 판이 나왔으므로 아무도 몰랐다 — 느려진 것만 겪었다.
          var ewM = /;efswhy=([^;\r\n]*)/.exec(rows[0] || '');
          if (ewM && ewM[1]) bakeNote += '\n※ 일괄 굽기가 안 돼 조각별로 구웠습니다(느립니다) — ' + efsWhyText(ewM[1]);
          // ★굽기 **전후**로 파일을 만들 수 있었나 — `p0.png 를 내보낼 수 없음` 이 굽기 탓인지
          //   이미 못 쓰는 상태로 들어온 탓인지 가른다(2026-09-09). 이상할 때만 띄운다.
          var ipM = /;ioprobe=([^;\r\n]*)/.exec(rows[0] || '');
          if (ipM && ipM[1] && /X|\*/.test(ipM[1])) bakeNote += '\n[단계별 파일쓰기] ' + ipM[1];
          for (var r = 1; r < rows.length; r++) {
            var t = rows[r].split(' ');
  ...
  // ★정본은 `js/png-io.js` — 파일명 규약(`mes_cut_bleed_<id>.png`)만 여기 남는다.
  function writeBleedPng(dir, id, r) { return window.MesPngIo.writePng(dir + 'mes_cut_bleed_' + id + '.png', r); }
  ...
  function buildBleedPngs(prep, growMm, cb) {
     ...
     * ★도련 색 규칙(0.98.0 · bleed.js insetFor) — 「자동」이면 조각별 사진 여부를 호스트에서 받는다.
     *   못 받으면(구 호스트·실패) 사각 아닌 조각은 전부 사진으로 본다 = 종전 동작 그대로.
     */
    function withList(list) {
      var mode = bleedColorMode();
      if (mode === 'auto' && hostSupportsPhoto()) {
        host('mesCut_photoFlags()', function (pf, badP) {
          var fl = (!badP && String(pf).indexOf('ok;') === 0) ? String(pf).substring(3).split(',') : null;
          buildFrom(list, mode, fl);
        });
        return;
      }
      buildFrom(list, mode, null);
    }
    function buildFrom(list, mode, photoFlags) {
      // temp 경로는 호스트만 안다 — 굽기 결과 경로에서 폴더를 떼어 쓴다(경로가 브릿지를 안 탄다)
      var dir = String(list[0].path).replace(/[^\/\\]+$/, '');
      var map = {}, skipped = 0, tooBig = 0, coarse = 0, q = 0, squared = 0;
      var whyN = { edge: 0, photo: 0, skip: 0 };
      (function step() {
        if (q >= list.length) {
          var note = '';
          ...
          if (squared) note += (note ? ' ' : '') + '사각 조각 ' + squared + '개는 모서리까지 각지게 채웠습니다.';
          // 도련 색을 어디서 뽑았는지 — 품질이 갈리는 자리라 조각 수까지 밝힌다
          var cparts = [];
          if (whyN.edge) cparts.push('가장자리 그대로 ' + whyN.edge + '개');
          if (whyN.photo) cparts.push('안쪽 색(사진) ' + whyN.photo + '개');
          if (whyN.skip) cparts.push('테두리 2mm 건너뛰기 ' + whyN.skip + '개');
          if (cparts.length) note += (note ? ' ' : '') + '도련 색[' + ({ auto: '자동', edge: '가장자리', skip: '건너뛰기' }[mode] || mode) + '] — ' + cparts.join(' · ') + '.';
          cb(map, note);
          return;
        }
        ...
        readPng(it.path, function (err, img) {
          ...
          // ★사각 조각은 모서리를 **각지게** 잇는다(용준님 2026-09-23). 실루엣 조각은 종전처럼 둥글게 — 판정은 엔진(isRectLike).
          var sq = false;
          try { sq = !!(B.isRectLike && B.isRectLike(src)); } catch (eSq) { sq = false; }
          if (sq) squared++;
          var res = null;
          // ★사각 조각은 **가장자리 픽셀을 그대로 연장**한다(`srcInsetPx: 0` · 2026-09-23 실기 「모서리 검정 집게」).
          //   안정점 걸음(기본 2px)은 AA·소프트 에지를 건너뛰려는 것인데, 사각 조각은 AA 를 끄고 구워 가장자리가
          //   디자인 그 자체다(테두리선). 걸음이 1px 테두리선을 「블렌드」로 보고 안쪽 흰색을 띠로 삼으면서도
          //   테두리선 열 위에서는 「어둠·어둠」으로 안정해 줄무늬를 남기고, 모서리 대각선 셀은 또 다른 답을 냈다 —
          //   흰 띠에 검은 줄·검은 계단이 섞인 비일관이 실기의 「집게」다. 그대로 연장하면 띠 = 테두리 색으로
          //   일관되고, 재단이 바깥으로 밀려도 흰 틈 대신 테두리 색이 이어진다(도련의 본래 목적).
          //   실루엣·사진 조각은 종전 그대로(걸음 있음·둥근 모서리).
          // ★도련 색 규칙 — 사진 여부를 모르면(구 호스트) 사진으로 본다 = 종전 2px 걸음 그대로
          var photo = photoFlags ? (photoFlags[it.id] !== '0') : true;
          var rule = B.insetFor ? B.insetFor({ mode: mode, rect: sq, photo: photo, mmpp: mpp, skipMm: 2 })
            : { srcInsetPx: sq ? 0 : 2, why: sq ? 'edge' : 'photo' };
          whyN[rule.why] = (whyN[rule.why] || 0) + 1;
          var ropt = { srcInsetPx: rule.srcInsetPx };
          if (rule.srcFixedPx) ropt.srcFixedPx = rule.srcFixedPx;
          if (sq) ropt.corner = 'square';
          try { res = B.repeatLastPixel(src, gpx, ropt); } catch (e) { res = null; }
  ...
  /**
   * ★사각 빠른 경로 ③ 띠 도련 (2026-09-23) — 조각 전체가 아니라 **네 변의 잉크 쪽 띠**만 굽고(`nestBakeAll` stripMm)
   *   종전과 **같은 엔진·같은 옵션**(repeatLastPixel · square · srcInsetPx 0)으로 바깥으로만 늘린다.
   *   띠가 전부 불투명이면 결과는 buildBleedPngs 의 전체 PNG 와 같은 픽셀이다(하네스 cut:rectfast 가 대조한다).
   *   비용이 면적이 아니라 둘레에 비례한다 — 100×500cm 조각의 도련 PNG 16M px → 띠 4장 수십만 px.
   * ⚠️하나라도 안 되면(굽기 실패·띠에 투명) **종전 전체 도련**으로 되돌린다. 조각 하나만 빼면 호스트가 그 조각을
   *   단색으로 떨어뜨린다 — 그건 결과물이 바뀌는 것이다.
   * @param cb(map, note) map = { idx: {strip:{W,H,g,d}} } (mm) — afterBleed 가 `LS` 줄로 싣는다
   */
  function buildStripBleeds(prep, growMm, cb) {
    var B = window.MesCutBleed, RF = window.MesCutRectFast, IO = window.MesPngIo;
    var fallback = function (why) {
      buildBleedPngs(prep, growMm, function (map, note) {
        cb(map, '띠 도련을 못 만들어 전체 도련으로 만들었습니다(' + why + ')' + (note ? ' · ' + note : ''));
      });
    };
    if (!B || !RF || !IO) { fallback('엔진 미로드'); return; }
    // ★띠는 가장자리 그대로 잇기 전용이다 — 「테두리 건너뛰기」는 띠 깊이보다 깊이 들어가야 해서 여기서 못 지킨다
    //   (2026-09-26 점검: 빠른 경로를 켜면 건너뛰기가 조용히 무시되던 자리). 전체 도련으로 가서 설정을 지킨다.
    if (bleedColorMode() === 'skip') { fallback('테두리 건너뛰기'); return; }
    var k = prep.rect.k, maxS = 0;
    for (var sid in prep.rect.sizesS) {
      if (!prep.rect.sizesS.hasOwnProperty(sid)) continue;
      maxS = Math.max(maxS, prep.rect.sizesS[sid].w, prep.rect.sizesS[sid].h);
    }
    // 해상도 = 종전 도련과 같은 격자(fineMmpp). 긴 변이 굽기 변 한계(16,384px)를 넘을 때만 그만큼 성글게 —
    //   종전은 이 경우 도련 전체를 18M px 상한에 맞춰 훨씬 더 거칠게 만들었다(같거나 더 곱다).
    var mmpp = Math.max(prep.fineMmpp, maxS / 16000);
    var dPx = RF.STRIP_DEPTH_PX;
    out('도련 띠 굽는 중...');
    host('mesCut_nestBakeAll(' + (mmpp * k) + ',0,false,"strip","",' + (dPx * mmpp * k) + ')', function (rz, bad) {
      if (bad || String(rz).indexOf('ok;') !== 0) { fallback(String(rz).split(/[\r\n]/)[0]); return; }
      var rows = String(rz).split(/[\r\n]+/), by = {}, ids = [], dir = '';
      for (var r = 1; r < rows.length; r++) {
        var t = rows[r].split(' ');
        if (t[0] !== 'S') continue;
        var id = parseInt(t[1], 10), path = t.slice(5).join(' ');
        if (!by[id]) { by[id] = {}; ids.push(id); }
        by[id][t[2]] = path;
        if (!dir) dir = String(path).replace(/[^\/\\]+$/, '');
      }
      if (ids.length !== prep.n) { fallback('띠 ' + ids.length + '/' + prep.n + '개'); return; }
      var map = {}, q = 0, SIDES = ['t', 'b', 'l', 'r'];
      (function step() {
        ...
        (function readSide() {
          if (si >= SIDES.length) {
            var res = null;
            try { res = RF.stripBleeds(B, strips, growMm / mmpp); } catch (eS) { res = null; }
            if (!res) { fallback('#' + (cur + 1) + ' 가장자리가 완전 불투명이 아닙니다'); return; }
            for (var w = 0; w < SIDES.length; w++) {
              if (!IO.writePng(dir + 'mes_cut_bleeds_' + cur + '_' + SIDES[w] + '.png', res[SIDES[w]])) { fallback('띠 PNG 쓰기 실패'); return; }
            }
            map[cur] = { strip: { W: strips.t.W * mmpp, H: strips.l.H * mmpp, g: Math.ceil(growMm / mmpp) * mmpp, d: strips.t.H * mmpp } };
            q++; step();
            return;
          }
          var side = SIDES[si];
          if (!by[cur][side]) { fallback('#' + (cur + 1) + ' 띠 누락(' + side + ')'); return; }
          readPng(by[cur][side], function (err, img) {
            if (err) { fallback('띠 이미지를 읽지 못했습니다'); return; }
            strips[side] = img; si++; readSide();
          });
        })();
      })();
    });
  ...
  /**
   * ★적용 단계 내역 + 굳히기 경로 (2026-09-24 실기 — 「적용 332.2초」 하나로는 어디가 느린지 몰랐다).
   *   호스트는 처음부터 `ms=`(단계별)·`fast=`/`masters=`/`hardenwhy=`(굳히기 경로)를 돌려줬는데 패널이 **버리고 있었다**.
   *   그래서 격자 굳히기가 통째로 포기돼 **조각별 느린 경로**로 떨어져도 결과창엔 「PDF 로 굳혀 배치」만 떴다(조용한 격하).
   * @param a 호스트 결과 kv  @returns 결과창에 붙일 줄('' 이면 없음)
   */
  var APPLY_STAGE = { harden: '굳히기', master: '격자 배치', scale: '조각별 굳히기', newdoc: '판 문서', vecsil: '벡터 칼선', bleed: '도련', fit: '판 맞추기' };
  var HARDEN_WHY = { canvas: '확대 후 캔버스 초과', off: '꺼짐(시험용)' };
  function applyBreakdown(a) {
    var s = '';
    var parts = [], ms = String(a.ms || '').split(',');
    for (var i = 0; i < ms.length; i++) {
      var kv2 = ms[i].split(':');
      var v = parseInt(kv2[1], 10);
      if (!kv2[0] || !(v > 0)) continue;
      parts.push({ k: kv2[0], v: v });
    }
    parts.sort(function (x, y) { return y.v - x.v; });
    if (parts.length) {
      s += '\n  └ 적용 내역: ' + parts.map(function (p) { return (APPLY_STAGE[p.k] || p.k) + ' ' + (Math.round(p.v / 100) / 10) + '초'; }).join(' · ');
    }
    var placed = parseInt(a.placed, 10) || 0, fast = parseInt(a.fast, 10) || 0;
    var slow = placed - fast;
    if (placed > 0 && slow > 0) {
      var why = String(a.hardenwhy || '');
      var whyTxt = HARDEN_WHY[why] || (why.indexOf('split') === 0 ? '격자 나누기 실패' : why.indexOf('grid') === 0 ? '격자 만들기 실패' : why);
      s += '\n⚠ 배율·회전 ' + slow + '/' + placed + '개 조각이 **조각별 굳히기**(느린 경로)로 처리됐습니다'
        + (whyTxt ? (' — ' + whyTxt + (whyTxt !== why && why ? ' (' + why + ')' : '')) : '')
        + '. 결과물은 같고 시간만 더 듭니다.';
    } else if (placed > 0 && a.grids) {
      s += '\n  └ 굳히기 격자 ' + a.grids + '개로 ' + fast + '개 조각을 한 번에 처리했습니다.';
    }
    return s;
  }
  window.__mesCutApplyBreakdown = applyBreakdown;
  ...
  function buttPlace(BT, prep, sheetWmm, sheetHmm, allowRot) {
    ...
    var maxH = Math.max(1, (sheetHmm || rollMaxMm()) - domboMm() * 2);
  ...
  function nestPlace(NST, prep, sheetWmm, sheetHmm, allowRot, opts) {
    ...
    return NST.nest(prep.pieces, {
      ...
      rollMaxH: Math.floor(Math.max(10, rollMaxMm() - domboMm() * 2) / prep.mmpp),
      step: opts.step || 4,
      tries: opts.tries,
  ...
  function runNest() {
    ...
    // ★★도련이 **이웃 아트를 침범**하면 클립 확장(자동 ①)을 쓸 수 없다 (2026-09-05 실사용).
    //   ①은 아트 사본의 **클립 사각 자체를 벌린다**(호스트 mesCut_vecGrowClips) → 도련이 아트와
    //   **같은 객체**라 따로 뒤로 보낼 수단이 없다. 그래서 나중에 놓인 조각의 도련이 먼저 놓인
    //   조각의 **진짜 아트를 덮는다**(실측: 맞붙임 2조각이 y=2380.4 에서 맞닿고 확장 3mm →
    //   겹침 띠 6mm 를 앞 조각이 통째로 이겼다). ②PNG 는 별도 개체라 `zOrder(SENDTOBACK)` 로
    //   항상 아트 뒤로 간다 — 같은 판에서 조각마다 동작이 갈리던 원인이다.
    //   → 겹치는 잡에서는 ①을 건너뛴다. 안쪽 도련은 어차피 이웃이 덮고 잘려 나가므로
    //     무손실 이점이 성립하지 않는다. 바깥 테두리 도련은 ②(정본 경로)가 그대로 만든다.
    //   ⚠️ 판정은 **간격과 실제 도련**으로 한다(맞붙임 여부로 하지 않는다) — 간격 분할이
    //      바뀌어도 따라오게. 간격 분할 후에는 도련 ≤ 간격/2 라 일반 잡은 애초에 안 걸린다.
    var bleedOverlaps = effBleedMm > 0 && (gapMm / 2) < effBleedMm;
    var bleedModeNote = '';
    if (bleedOverlaps && nestBleedMode === 'auto') {
      nestBleedMode = 'region';
      bleedModeNote = '\n※ 도련이 옆 조각과 겹쳐 **클립 확장(무손실)을 쓰지 않았습니다**'
        + ' — 확장한 클립은 아트와 한 개체라 도련만 뒤로 보낼 수가 없고, 그러면 뒤쪽 조각의'
        + ' 실제 그림이 이웃 도련에 가립니다. 가장자리 색 잇기로 만들어 **도련을 전부 아트 뒤**에 둡니다.';
    }
    ...
    function go() {
      ...
      // 벡터 칼선(또는 칼선 없음)이면 굽기를 VEC_BAKE_MM 로 — 판정은 여기(vecProbe 뒤)서만 안다
      nestBakeCoarse = !!(useVec || !wantPieceCut);
      nestPrepare(G, rez, gapMm, offsetMm, fillMode, function (m) { nestBakeCoarse = false; done(m, 'err'); }, function (prep) {
        ...
        var numInfo = attachNumberTabs(prep, sheetWmm, buttMode);
        ...
        var pick = PLC.choose({
          ...
          sheetHmm: sheetHmm, rollMaxMm: rollMaxMm(),
        ...
        // ★꼬리표를 떼고 **조각 사각만** 남긴다 — 아래 I 줄·칼선·도련은 종전 좌표계 그대로 돈다(꼬리표는 pl.tab 에).
        numNormalize(res, prep, numInfo);
        ...
        for (var s = 0; s < res.sheets.length; s++) {
          ...
          for (var k = 0; k < sh.placements.length; k++) {
            ...
            // ★조각 번호 꼬리표 = `T idx x y w h rot label` (시트 mm). 자리는 numNormalize 가 되돌린 값이라 호스트는 기하를 모른다.
            if (pl.tab && numInfo && numInfo.labels && numInfo.labels[pl.id]) {
              lines.push('T ' + pl.id + ' ' + (pl.tab.x * mmpp + domboMm()).toFixed(2) + ' ' + (pl.tab.y * mmpp + domboMm()).toFixed(2)
                + ' ' + (pl.tab.w * mmpp).toFixed(2) + ' ' + (pl.tab.h * mmpp).toFixed(2) + ' ' + (pl.rot || 0) + ' ' + numInfo.labels[pl.id]);
              numInfo.sent = (numInfo.sent || 0) + 1;
            }
            ...
            // ★사각 빠른 경로 — 추적하지 않고 **계산**한다: 잉크 사각(I 줄과 같은 원점) + 여백. 여백 > 0 이면 반경 = 여백 원호
            //   (래스터 팽창·벡터 jntp=0 이 만드는 모양과 같다 — 모양을 바꾸면 결과물이 바뀐다). 크기는 벡터 실측.
            if (wantPieceCut && !useVec && !res.butt && prep.rect) {
              var rsz = prep.rect.sizesS[pl.id], rq = ((pl.rot || 0) % 180 + 180) % 180 === 90;
              var rline = rsz ? window.MesCutRectFast.cutLine(pl.x * mmpp + half + domboMm(), pl.y * mmpp + half + domboMm(),
                rq ? rsz.h : rsz.w, rq ? rsz.w : rsz.h, prep.offsetMm) : null;
              if (rline) lines.push(rline);
            } else if (wantPieceCut && !useVec && !res.butt) holeOut += (pieceCutLines(lines, res, prep, pl, mmpp, wantCurve) || 0);
          }
        ...
        function afterBleed(bmap, note) {
          ...
          for (var bid in bmap) {
            ...
            if (bmap[bid].strip) { var sb = bmap[bid].strip; lines.push('LS ' + bid + ' ' + sb.W.toFixed(3) + ' ' + sb.H.toFixed(3) + ' ' + sb.g.toFixed(3) + ' ' + sb.d.toFixed(3)); continue; }
        ...
        host('mesCut_paramsPath()', function (pp) {
          ...
          host('mesCut_nestApply(' + offsetMm + ',' + (prep.fill ? 'true' : 'false') + ',' + effBleedMm + ',"' + nestBleedMode + '"' + (useVec ? '' : ',"raster"') + ')', function (ap, bad3) {
            ...
            var R = toRealMm;
            // ★★롤 길이는 **판 전부의 합**이다 (2026-09-05). 여태 `sheets[0]` 만 재서, 판이 나뉘면
            //   재료 길이를 첫 판 하나로 보고했다 — 실측 3판 4807/3927/4807 = 13,541mm 인데
            //   화면에는 4801mm 가 떴다(**1/2.8**). 맞붙임이 판을 나누기 시작하며 상시로 드러난다.
            //   ⚠️효율%의 분모(sheetAreaMm2)는 처음부터 전 판을 합산했다 — 표시만 틀렸다.
            //   폭 추천도 이미 합산한다(0.76.0) — 같은 규칙으로 맞춘다.
            var rollTotMm = 0, rollMaxMm2 = 0;
            for (var rs = 0; rs < res.sheets.length; rs++) {
              var oneMm = Math.round(R(Math.ceil(res.sheets[rs].usedH * mmpp) + domboMm() * 2));
              rollTotMm += oneMm;
              if (oneMm > rollMaxMm2) rollMaxMm2 = oneMm;
            }
            var lenTxt = isRoll
              ? ('롤 길이 ' + rollTotMm.toLocaleString() + 'mm'
                 + (res.sheets.length > 1 ? (' (판 ' + res.sheets.length + '장 · 최장 ' + rollMaxMm2.toLocaleString() + 'mm)') : '')
                 // ★판 최대 길이를 사람이 정했으면 그 사실을 쓴다 — 판이 왜 이 길이에서 나뉘었는지 보이게
                 + (userMaxLenReal() ? (' · 판 최대 ' + userMaxLenReal().toLocaleString() + 'mm(설정)') : ''))
              : (res.sheets.length + '장');
            ...
            refreshPairName();
            setNestReady(true);
            // ★조각 번호 산출물(고객용·판별 강조본 PNG)은 비동기로 만든다 — 끝나면 결과창에 한 줄 붙고 미리보기가 뜬다.
            //   등록은 numArt.pending 이 풀릴 때까지 막힌다(registerNest).
            startNumberArt(prep, res, numInfo, mmpp, half);
              ...
              // ★★효율%가 어디서 나온 값인지 **분자·분모를 그대로** 보여준다 (2026-09-05).
              //   같은 파일·같은 배치인데 87.8% → 67.3% 로 갈린 실행이 있었는데, 화면에 비율만
              //   있어서 잉크가 준 건지 재료가 는 건지조차 못 갈랐다. 굽기 경로까지 함께 싣는다 —
              //   경로가 갈리면 마스크가 갈리고, 마스크가 갈리면 잉크가 갈린다.
              + ('\n  └ 잉크 ' + Math.round(prep.rawInkPx * mmpp * mmpp * scaleN * scaleN / 100).toLocaleString()
                 + 'cm² / 재료 ' + Math.round(areaMm2 * scaleN * scaleN / 100).toLocaleString() + 'cm²'
                 + ' · 굽기 경로 ' + (prep.bakeWay || '?'))
              ...
              + (prep.fillNote || '') + (prep.bakeNote || '') + (prep.edgeNote || '') + weldNote(prep, useVec, a.vecdrop) + qtyNote
              + (prep.exact ? '' : ' ⚠ 해상도 한계로 올림 적용')
              + (allowRot ? ' · 회전 허용' : '')
              + '\n돔보 ' + (a.dombo || 0) + '판 — 별도 레이어(인쇄 ON) · 재단선 레이어는 인쇄 OFF'
              + numResultLine(numInfo, a)
              ...
              + (effBleedMm > 0 ? ('\n도련 ' + R(effBleedMm) + 'mm (조각마다)'
                  ...
                  + bleedHow(a)
                  // ★방식을 **우리가 바꿨으면** 그 사실과 이유를 말한다 — 사용자가 고른 「자동」과
                  //   다르게 돌았는데 조용하면, 무손실이 왜 안 나왔는지 알 길이 없다(조용한 격하 금지).
                  + bleedModeNote
        ...
        // ★사각 빠른 경로면 띠 도련 — 띠가 하나라도 안 되면 **종전 전체 도련**으로 되돌린다(단색으로 떨어뜨리지 않는다)
        //   「테두리 건너뛰기」는 띠만으로는 안쪽 색을 못 보므로(띠 깊이 < 건너뛸 두께) 전체 도련으로 간다
        if (wantBleedPng && prep.rect && bleedColorMode() !== 'skip') buildStripBleeds(prep, growMm, afterBleed);
        else if (wantBleedPng) buildBleedPngs(prep, growMm, afterBleed);
        else afterBleed({}, '');
      });
    }
  ...
  function recommendWidth() {
    ...
    host('mesCut_acquireLock("' + PANEL_OWNER + '","nest-scan")', function (lk) {
      ...
      nestPrepare(G, rez, gapMm, offsetMm, fillMode, function (m) { done(m, 'err'); }, function (prep) {
        ...
        var qtyNote = expandByQty(prep);
        // ★조각 번호 꼬리표도 [네스팅 실행]과 같은 조건으로 붙인다 — 추천과 실행이 다른 마스크를 보면 안 된다.
        //   폭 후보가 여럿이라 가장 넓은 폭 기준으로 붙인다(폭 여유 판정이 가장 후함 → 꼬리표 최대 = 보수적 추천).
        attachNumberTabs(prep, toFileMm(Math.max.apply(null, ROLL_WIDTHS_MM)), (offsetMm <= 0 && gapMm <= 0));
        ...
        var widthPlace = function (wFile) {
          ...
          return PLCw.choose({
            ...
            sheetHmm: 0, rollMaxMm: rollMaxMm(),
  ...
  /**
   * 자동완성 — **가공 탭과 같은 규칙**(부분 일치·공백 무시·상위 15건 · 맞았는지 표시).
   *
   * 왜 `<datalist>` 를 버렸나 (2026-09-05 용준님 두 번째 지적):
   *   CEF 의 datalist 는 **앞부분 일치**만 건다. 실측 — 거래처 2,890건에서 「디자인」이
   *   앞부분 75건 / 부분 286건이라 **211건(74%)이 화면에 아예 안 떴다**(「(주)모노디자인」 등).
   *   이 프로젝트 거래처는 대부분 `(주)…` 로 시작하므로 앞부분 일치는 사실상 무용하다.
   ...
   *
   * @param inputId  입력칸 id (`<id>Sug`·`<id>Hit` 를 같은 접두사로 찾는다)
   * @param dlId     후보를 담은 datalist id
   * @param hitFn    (값) → {ok, text} · 없으면 표시하지 않는다
   */
  function attachSug(inputId, dlId, hitFn) {
    var el = document.getElementById(inputId), sug = document.getElementById(inputId + 'Sug');
    var hit = document.getElementById(inputId + 'Hit');
    // ?raw 로 실린 JS 는 HTML 이 바뀌면 조용히 죽는다 — 없으면 반드시 알린다(review-checklist §12)
    if (!el) { console.warn('[cut] #' + inputId + ' not found'); return null; }
    if (!sug) { console.warn('[cut] #' + inputId + 'Sug not found'); return null; }
    function cands() {
      var dl = document.getElementById(dlId), out = [];
      if (!dl) { console.warn('[cut] #' + dlId + ' not found'); return out; }
      var op = dl.getElementsByTagName('option');
      for (var i = 0; i < op.length; i++) out.push(op[i].value || '');
      return out;
    }
    function hide() { sug.className = 'sug hidden'; sug.innerHTML = ''; }
    function upd() {
      if (!hit || !hitFn) return;
      var r = hitFn(String(el.value || '').replace(/^\s+|\s+$/g, '')) || { ok: false, text: '' };
      hit.textContent = r.text;
      hit.className = 'achit' + (r.ok ? ' ok' : '');
    }
    function render() {
      var q = String(el.value || '').replace(/^\s+|\s+$/g, '');
      if (!q) { hide(); return; }
      var qq = cutSquash(q), list = cands(), hits = [];
      for (var i = 0; i < list.length && hits.length < 15; i++) {
        if (cutSquash(list[i]).indexOf(qq) !== -1) hits.push(list[i]);
      }
      // 이미 정확히 그 값이면 목록을 띄우지 않는다 — 고른 뒤 다시 뜨면 방해만 된다
      if (!hits.length || (hits.length === 1 && hits[0] === q)) { hide(); return; }
      var html = '';
      for (var h = 0; h < hits.length; h++) {
        html += '<div class="sgi" data-name="' + cutEsc(hits[h]) + '">' + cutEsc(hits[h]) + '</div>';
      }
      sug.innerHTML = html;
      sug.className = 'sug';
      // ⚠️이름을 `g` 로 줄이지 말 것 — undeclared.mjs 가 위 정규식 플래그(/…/g)를 식별자로
      //   읽어 「var 선언 전 사용」 오탐을 낸다(가공 탭이 sgis 인 것도 같은 이유).
      var sgis = sug.getElementsByClassName('sgi');
      for (var k = 0; k < sgis.length; k++) {
        // mousedown — blur 로 목록이 닫히기 전에 선택을 확정한다(가공 탭과 같은 이유)
        sgis[k].addEventListener('mousedown', function (ev) {
          ev.preventDefault();
    ...
    el.addEventListener('input', function () { render(); upd(); });
    el.addEventListener('focus', render);
    el.addEventListener('blur', function () { window.setTimeout(hide, 150); upd(); });
    upd();
    return { refresh: upd, hide: hide };
  }
  ...
  function registerNest() {
    ...
    host('mesCut_acquireLock("' + PANEL_OWNER + '","register")', function (lk) {
      ...
      // ★조각 번호 산출물 — 호스트가 등록 폴더로 옮긴다(thumb_hi.png·overview.png) + manifest `pieces`.
      //   PIECES 는 ASCII JSON(번호·실물 mm·판 위 자리) — 호스트가 그대로 manifest 에 박는다.
      if (numArt && !numArt.pending && numArt.pieces) {
        lines.push('NUMBERING ' + (numArt.mode || 'seq'));
        if (numArt.overview) lines.push('OVERVIEW ' + numArt.overview);
        for (var nt = 0; nt < numArt.thumbs.length; nt++) if (numArt.thumbs[nt]) lines.push('THUMBHI_' + nt + ' ' + numArt.thumbs[nt]);
        for (var np = 0; np < numArt.pieces.length; np++) lines.push('PIECES_' + np + ' ' + JSON.stringify(numArt.pieces[np]));
      }
      // ★완료 문구를 함수로 뺀다 — manifest 를 패널이 대신 쓴 경우에도 **같은 화면**이 나와야
      //   한다(등록은 실제로 완성됐다). 두 벌이면 한쪽만 고쳐지는 자리가 된다.
      /**
       * 오늘 같은 이름의 판이 이미 있었다 — 픽업 폴더(`_출력/<날짜>`)는 **날짜당 한 자리**이고
       * 에이전트는 크기가 다르면 덮어쓴다(`Program.cs EnsurePickupCopy`). 등록·주문·카드는 둘 다
       * 멀쩡한데 **재단기에 가는 실물만 나중 것 하나**가 된다 → 사실만 알린다(2026-09-18).
       * ⚠️시스템이 이름을 안 바꾼다 — 이 파일명이 그대로 RIP 에 나가는 이름이다(08-27 규약).
       */
      function pickDupText(n) {
        var k = parseInt(n, 10);
        if (!(k > 0)) return '';
        return '\n⚠ 오늘 같은 이름의 출력 파일이 이미 있습니다 — ' + k + '건'
          + '\n   픽업 폴더(_출력)는 날짜당 한 자리라 나중 것이 앞 것을 덮습니다.'
      ...
      /**
       * manifest 를 **패널이 대신 쓴다** (2026-09-09) — A0 의 rescueManifest 와 같은 계약.
       * ★일러 프로세스의 파일 자원이 고갈되면 ExtendScript 도 일러 자신도 파일을 못 만든다
       *   (실기: temp·Z: 동시 I/O 오류 · p0.png 내보내기 실패 · config 180KB 가 0바이트).
       *   CEP 는 별도 프로세스라 그 순간에도 쓸 수 있다 — 일러 버전·폰트 수와 무관한 길이다.
       * ★시트가 여러 장이면 manifest 도 여러 장이다. **하나라도 못 쓰면 실패**로 본다 —
       *   일부만 올라가면 판 일부가 주문서에서 사라져 더 나쁘다.
       */
      ...
      host('mesCut_regPath()', function (rp) {
        ...
        host('mesCut_nestRegister()', function (res, bad) {
          // ★manifest **만** 못 쓴 경우 = 시트·EPS·썸네일은 이미 다 나왔다. 여기서 끝내지 않는다.
  ...
  var NEST_PREF_IDS = ['sheetPreset', 'nestNumber', 'nestNumberMode', 'nestRectFast', 'nestOffset', 'nestGap', 'nestRotate',
    'nestBleed', 'bleedMode', 'bleedColor', 'nestCut', 'nestBakeMm'];
  var NEST_ADV_IDS = ['nestRectFast', 'bleedMode', 'bleedColor', 'nestCut', 'nestBakeMm'];   // 「고급 설정」 안에 접힌 칸
  ...
  function renderPieceQty() {
    ...
    for (var i = 0; i < pieceQty.sizes.length; i++) {
      ...
      row.className = 'qrow';
      var n = document.createElement('span'); n.className = 'qn';
      // 조각 번호(줄-순번·일련)가 있으면 #N 옆에 붙인다 — 화면·판·고객 이미지가 같은 값을 본다
      n.textContent = '#' + (i + 1) + ((pieceQty.labels && pieceQty.labels[i]) ? (' ' + pieceQty.labels[i]) : '');
  ...
  // ── ★조각 번호 (2026-09-23 용준님 확정) — 정본 = js/piece-number.js · 게이트 = cut:number ─────────────
  //   시트 주문(한 파일에 조각 수십 장)에서 「어느 조각이 어느 자리인가」를 번호 하나로 잇는다. 번호는
  //   원본 배치(고객용 이미지)·판의 조각 옆(꼬리표)·등록 조각 목록 세 곳에 **같은 조각 id 에서 파생**돼 실린다.
  //   여기는 배선만 — 줄 판정·꼬리표 기하는 piece-number.js(순수) 가 하고 하네스가 그 파일을 직접 검증한다.
  var NUM_TAB_W_MM = 20, NUM_TAB_H_MM = 8;   // 꼬리표 실물 mm(글자 ≈ 5mm). 회전 배치에서는 8mm 가 폭에 더해진다.
  // 번호 이미지가 아직 안 끝났을 때 결과창에 넣는 자리표시 — numSyncLine 이 이 문구를 찾아 완료 문구로 바꾼다
  var NUM_PENDING_TXT = '\n  └ 고객용 번호 이미지·판별 강조본을 만드는 중… (끝나면 아래 미리보기)';
  var numArt = null;                           // 마지막 네스팅의 번호 산출물 {pending, mode, overview, thumbs[], pieces[][]}
  var paramsDir = '';                          // params 파일이 있는 temp — 번호 PNG 도 여기에 쓴다
  function numOn() { var el = $('nestNumber'); return !!(el && el.checked); }
  function numMode() { var el = $('nestNumberMode'); return (el && el.value) || 'auto'; }
  function numParseBounds(s) {
    if (!s || String(s).indexOf('ok;') !== 0) return null;
    var parts = String(s).substring(3).split(','), out = [];
    for (var i = 0; i < parts.length; i++) {
      var q = parts[i].split(':');
      out.push({ x: parseFloat(q[0]) || 0, y: parseFloat(q[1]) || 0, w: parseFloat(q[2]) || 0, h: parseFloat(q[3]) || 0 });
    }
    return out;
  }
  /** [문서 ↻] 용 — 경계만. 호스트가 못 주면 null(번호 없이 종전 화면). */
  function numFetchBounds(cb) {
    if (!hostSupportsNumber() || !window.MesPieceNumber) { cb(null); return; }
    host('mesCut_nestBounds()', function (b, bad) { cb(bad ? null : numParseBounds(b)); });
  }
  /** 네스팅 용 — 경계 + 원본 PNG. **번호가 켜져 있을 때만** 호스트를 부른다(꺼져 있으면 왕복 0). */
  function numFetchForNest(cb) {
    if (!numOn() || !hostSupportsNumber() || !window.MesPieceNumber) { cb(null, null); return; }
    host('mesCut_nestBounds()', function (b, bad) {
      var bounds = bad ? null : numParseBounds(b);
      host('mesCut_exportOverview(1600)', function (ov, bad2) {
        var meta = null;
        if (!bad2 && String(ov).indexOf('ok;') === 0) {
          var k = kv(ov.substring(3));
          meta = { path: k.path, w: parseInt(k.w, 10) || 0, h: parseInt(k.h, 10) || 0,
            abL: parseFloat(k.abl) || 0, abT: parseFloat(k.abt) || 0, abW: parseFloat(k.abw) || 0, abH: parseFloat(k.abh) || 0 };
        }
        cb(bounds, meta);
      });
    });
  }
  /** 목록의 번호를 다시 매긴다(모드가 바뀌었거나 경계를 새로 읽었을 때) */
  function numRefreshLabels() {
    if (!pieceQty) { numHint(); return; }
    var PN = window.MesPieceNumber;
    if (!PN || !pieceQty.bounds) { pieceQty.labels = null; pieceQty.numWhy = ''; numHint(); return; }
    var r = PN.assign(pieceQty.bounds, numMode());
    pieceQty.labels = r.labels; pieceQty.numWhy = r.why; pieceQty.numMode = r.mode;
    numHint();
  }
  function numHint() {
    var el = $('nestNumberInfo'); if (!el) return;
    if (hostVersion && !hostSupportsNumber()) { el.textContent = '호스트 구버전(' + hostVersion + ' < CUT-CEP-0.51.0) — 번호 불가'; return; }
    if (!pieceQty || !pieceQty.bounds) { el.textContent = '[문서 ↻] 로 조각을 읽으면 판정이 뜹니다'; return; }
    el.textContent = pieceQty.numWhy || '';
  }
  /**
   * 꼬리표를 마스크에 붙인다 — 배치 **전**. 반환 = 번호 정보(꺼져 있으면 null).
   *   off: 'host'(구버전) · 'bounds'(경계 못 읽음) · 'butt'(맞붙임 — 조각이 맞닿아 자리가 없다 · 번호는 이미지·목록에만)
   *   수량 확장으로 같은 객체가 여러 번 오므로 `pc.tab` 유무로 한 번만 붙인다. 폭 여유가 없는 조각은 건너뛰고 센다.
   */
  function attachNumberTabs(prep, sheetWmm, buttMode) {
    if (!numOn()) return null;
    var PN = window.MesPieceNumber;
    if (!hostSupportsNumber() || !PN) return { off: 'host', labels: null };
    if (!prep.bounds || prep.bounds.length !== prep.sizes.length) return { off: 'bounds', labels: null };
    var r = PN.assign(prep.bounds, numMode());
    var info = { off: buttMode ? 'butt' : '', labels: r.labels, mode: r.mode, why: r.why, rows: r.rows, tabbed: 0, skipped: 0, sent: 0, tw: 0, th: 0 };
    if (buttMode) return info;
    var mmpp = prep.mmpp;
    var tw = Math.max(1, Math.ceil(toFileMm(NUM_TAB_W_MM) / mmpp)), th = Math.max(1, Math.ceil(toFileMm(NUM_TAB_H_MM) / mmpp));
    info.tw = tw; info.th = th;
    var usableW = Math.floor(Math.max(10, sheetWmm - domboMm() * 2) / mmpp);
    var NST = window.MesCutNest;
    for (var i = 0; i < prep.pieces.length; i++) {
      var pc = prep.pieces[i];
      if (pc.tab !== undefined) continue;
      // ★★굽기 여백을 **먼저 걷어낸다**(2026-09-23 실기 — 번호가 재단선 안쪽으로). 굽기 캔버스는 잉크 둘레에
      //   `offset+gap+1` mm 를 비워 두는데 배치 팽창은 `offset+gap/2+안전` 이라, 마스크 가장자리에 빈 줄이 남는다.
      //   엔진은 조각을 놓기 전에 스스로 trim 하므로 여태 무해했지만, 꼬리표를 그 빈 줄 **위**에 붙이면 꼬리표와
      //   조각 사이에 빈 줄이 낀 채로 한 덩어리가 되고, 되돌릴 때(`placedGeom`)는 「조각이 꼬리표 바로 아래」로
      //   가정하므로 잉크가 그 빈 줄만큼(≈2mm) 위로 놓였다 → 칼선이 꼬리표 쪽으로 파고들었다.
      var tight = NST ? NST.trim({ W: pc.W, H: pc.H, m: pc.m }) : { W: pc.W, H: pc.H, m: pc.m };
      if (!(tight.W > 0 && tight.H > 0)) { pc.tab = null; info.skipped++; continue; }
      if (!PN.canTab(tight.W, tight.H, th, usableW)) { pc.tab = null; info.skipped++; continue; }
      var t = PN.attachTab({ W: tight.W, H: tight.H, m: tight.m }, tw, th);
      pc.tab = { tw: t.tabW, th: th, x0: t.tabX0, Wg: tight.W, Hg: tight.H };
      pc.W = t.W; pc.H = t.H; pc.m = t.m;
      info.tabbed++;
    }
    return info;
  }
  /** 배치 결과에서 꼬리표를 떼어 **조각 사각**만 남긴다 — 뒤의 I 줄·칼선·도련은 종전 좌표계로 돈다. 꼬리표 자리는 pl.tab. */
  function numNormalize(res, prep, info) {
    if (!info || !info.tabbed || !res || !res.sheets) return;
    var PN = window.MesPieceNumber, byId = {};
    for (var i = 0; i < prep.pieces.length; i++) byId[prep.pieces[i].id] = prep.pieces[i];
    for (var s = 0; s < res.sheets.length; s++) {
      var pls = res.sheets[s].placements || [];
      for (var k = 0; k < pls.length; k++) {
        var pl = pls[k], pc = byId[pl.id];
        if (!pc || !pc.tab) continue;
        var g = PN.placedGeom(pl, pc.tab.Wg, pc.tab.Hg, pc.tab.tw, pc.tab.th, pc.tab.x0);
        pl.x = g.piece.x; pl.y = g.piece.y; pl.W = g.piece.W; pl.H = g.piece.H;
        pl.tab = g.tab;
      }
    }
  }
  /** 결과창 한 줄 — 켜져 있으면 **반드시** 무언가를 쓴다(안 만들었으면 왜 안 만들었는지). */
  function numResultLine(info, a) {
    if (!numOn() || !info) return '';
    if (info.off === 'host') return '\n⚠ 조각 번호 — 호스트 구버전(' + (hostVersion || '?') + ' < CUT-CEP-0.51.0)이라 만들지 않았습니다. Z: 의 mes-cut-host.jsx 를 배포하세요.';
    if (info.off === 'bounds') return '\n⚠ 조각 번호 — 조각 경계를 읽지 못해 만들지 않았습니다([문서 ↻] 후 다시).';
    var s = '\n조각 번호 ' + (info.mode === 'row' ? '줄-순번' : '일련') + ' (' + info.why + ')';
    if (info.off === 'butt') return s + ' — 맞붙임이라 판에는 표기하지 않습니다(조각이 맞닿아 자리가 없음). 번호는 고객용 이미지·목록에만 갑니다.';
    var drawn = parseInt(a.num, 10) || 0, failed = parseInt(a.numfail, 10) || 0;
    s += ' — 판 표기 ' + drawn + '개';
    if (info.skipped) s += ' · 건너뜀 ' + info.skipped + '개(폭 여유 부족 — 번호는 이미지·목록에만)';
    if (failed) s += ' · ⚠ 못 그림 ' + failed + '개';
    else if (drawn !== (info.sent || 0)) s += ' · ⚠ 꼬리표 ' + (info.sent || 0) + '개를 보냈는데 ' + drawn + '개만 그려졌습니다(호스트 응답 확인)';
    s += NUM_PENDING_TXT;
    return s;
  }
  /** 파일→이미지·캔버스→파일은 **png-io 한 벌**만 쓴다(전사와 공유 — 사본을 들면 게이트 3r/17f 가 막는다) */
  function numLoadImage(path, cb) {
    if (!window.MesPngIo || !window.MesPngIo.readImage) { cb(null); return; }
    window.MesPngIo.readImage(path, cb);
  }
  /** 원본 PNG 위에 번호를 얹는다. members 가 있으면 그 밖의 조각은 흐리게(판별 강조본). 반환 = canvas */
  function numDraw(img, meta, bounds, labels, members) {
    var PN = window.MesPieceNumber;
    var cv = document.createElement('canvas');
    cv.width = img.naturalWidth; cv.height = img.naturalHeight;
    var ctx = cv.getContext('2d');
    ctx.drawImage(img, 0, 0);
    var kx = cv.width / meta.abW, ky = cv.height / meta.abH;
    if (members) {
      ctx.fillStyle = 'rgba(255,255,255,0.72)';
      for (var d = 0; d < bounds.length; d++) {
        if (members[d]) continue;
        var q = PN.mmToPng(meta, bounds[d].x, bounds[d].y);
        ctx.fillRect(q.x - 1, q.y - 1, bounds[d].w * kx + 2, bounds[d].h * ky + 2);
      }
    }
    // ★번호 = 조각 **한가운데**, 크기는 조각에 비례·인쇄 숫자 3mm 하한(`PN.labelLayout` — 게이트 cut:number M7).
    //   종전 모서리 배지는 작업지시서 인쇄에서 글자 ≈1mm 였다. 굵은 글자 + 흰 테두리라 어떤 그림 위에서도 읽힌다.
    var lay = PN.labelLayout({ abL: meta.abL, abT: meta.abT, abW: meta.abW, abH: meta.abH, w: cv.width, h: cv.height }, bounds, labels);
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.lineJoin = 'round';
    for (var i = 0; i < bounds.length; i++) {
      var L = lay[i];
      if (!L) continue;
      var on = !members || members[i];
      ctx.font = 'bold ' + L.font + 'px "Malgun Gothic", sans-serif';
      ctx.lineWidth = L.stroke * 2; ctx.strokeStyle = '#fff';
      ctx.strokeText(labels[i], L.cx, L.cy);
      ctx.fillStyle = on ? '#1E3A5F' : '#9ca3af';
      ctx.fillText(labels[i], L.cx, L.cy);
    }
    return cv;
  }
  function numWritePng(path, cvOrUrl) {
    return !!(window.MesPngIo && window.MesPngIo.writeCanvas && window.MesPngIo.writeCanvas(path, cvOrUrl));
  }
  function numPreview(dataUrl) {
    var box = $('numPreview'); if (!box) return;
    box.innerHTML = '';
    if (!dataUrl) { box.className = 'hidden'; return; }
    var img = document.createElement('img');
    img.src = dataUrl; img.style.maxWidth = '100%'; img.style.border = '1px solid #ccc'; img.style.display = 'block';
    img.title = '고객용 번호 이미지 미리보기 — 등록하면 overview.png 로 등록 폴더에 저장됩니다';
    box.appendChild(img);
    box.className = 'hint';
  }
  /**
   * 번호 산출물 — 판별 조각 목록(manifest `pieces`)은 즉시, PNG 두 벌(고객용·판별 강조본)은 비동기로.
   *   끝나면 결과창에 한 줄을 **붙이고** 미리보기를 띄운다. 등록은 그동안 막힌다(numArt.pending).
   */
  /**
   * ★결과창의 「만드는 중…」 줄을 완료 문구로 바꾼다 — **누가 먼저 끝나든** (0.97.1 · 2026-09-25 원격 실측).
   *   번호 이미지는 판 배치 직후 비동기로 만들어지고, 최종 결과창은 호스트 잠금을 푼 **뒤에** 쓰인다.
   *   번호가 먼저 끝나면 종전 코드가 고친 문구를 최종 결과창이 **덮어써** 「만드는 중」이 영원히 남았다
   *   (파일·미리보기·등록은 정상 — 표시만 틀렸다. 사용자가 붙여 준 결과창에도 그 줄이 남아 있었다).
   *   → 완료 문구를 numArt 에 두고, 결과창을 쓸 때마다(out) 여기서 다시 맞춘다. 줄이 없으면 아무것도 안 한다.
   */
  ...
  /**
   * ★조각이 원본 전체 그림(overview.png)의 어디에 있는가 — 그림 폭·높이 대비 % [x, y, w, h] (2026-09-26 판짜기 작업지시서).
   *   웹이 이 상자 위에 **판별 색 테두리**를 겹쳐 그린다 — 그림 자체(고객용)는 색을 바꾸지 않는다.
   *   경계(mm·문서 좌표)와 그림 메타(abL·abT·abW·abH)가 없으면 null(구 호스트·번호 꺼짐) — 웹은 표만 그린다.
   */
  function numOverviewBox(prep, id) {
    var ov = prep && prep.overview, bd = prep && prep.bounds && prep.bounds[id];
    if (!ov || !bd || !(ov.abW > 0) || !(ov.abH > 0)) return null;
    var r2 = function (v) { return Math.round(v * 100) / 100; };
    return [r2((bd.x - ov.abL) / ov.abW * 100), r2((bd.y - ov.abT) / ov.abH * 100), r2(bd.w / ov.abW * 100), r2(bd.h / ov.abH * 100)];
  }

  function startNumberArt(prep, res, info, mmpp, half) {
    numArt = null; numPreview(null);
    if (!info || !info.labels || !res || !res.sheets) return;
    var art = { pending: true, mode: info.mode, overview: null, thumbs: [], pieces: [] };
    numArt = art;
    for (var s = 0; s < res.sheets.length; s++) {
      var arr = [], pls = res.sheets[s].placements || [];
      for (var k = 0; k < pls.length; k++) {
        var pl = pls[k], sz = prep.sizes[pl.id] || { w: 0, h: 0 };
        var pc = { n: info.labels[pl.id] || '', w: Math.round(toRealMm(sz.w)), h: Math.round(toRealMm(sz.h)),
          x: Math.round(toRealMm(pl.x * mmpp + half + domboMm())), y: Math.round(toRealMm(pl.y * mmpp + half + domboMm())), r: pl.rot || 0 };
        var ob = numOverviewBox(prep, pl.id);
        if (ob) pc.b = ob;
        arr.push(pc);
      }
      art.pieces.push(arr);
    ...
    if (!prep.overview || !prep.overview.path || !prep.bounds) { finish('없음 — 원본 PNG 를 받지 못했습니다(번호는 판·목록에만).'); return; }
    numLoadImage(prep.overview.path, function (img) {
      if (!img) { finish('없음 — 원본 PNG 를 읽지 못했습니다(' + prep.overview.path + ').'); return; }
      var meta = { abL: prep.overview.abL, abT: prep.overview.abT, abW: prep.overview.abW, abH: prep.overview.abH, w: img.naturalWidth, h: img.naturalHeight };
      var ovUrl = window.MesPngIo.canvasDataUrl(numDraw(img, meta, prep.bounds, info.labels, null));
      var ovPath = paramsDir + 'mes_cut_overview.png';
      art.overview = numWritePng(ovPath, ovUrl) ? ovPath : null;
      var okThumbs = 0;
      for (var s2 = 0; s2 < res.sheets.length; s2++) {
        var mem = {}, pls2 = res.sheets[s2].placements || [];
        for (var k2 = 0; k2 < pls2.length; k2++) mem[pls2[k2].id] = true;
        var tp = paramsDir + 'mes_cut_thumbhi_' + s2 + '.png';
        var okT = numWritePng(tp, numDraw(img, meta, prep.bounds, info.labels, mem));
        art.thumbs.push(okT ? tp : null);
        if (okT) okThumbs++;
      }
      numPreview(ovUrl);
      finish('준비됨 — 고객용 1장' + (art.overview ? '' : '(⚠ 저장 실패)') + ' · 판별 강조본 ' + okThumbs + '/' + res.sheets.length + '장. 등록하면 등록 폴더로 갑니다.');
    });
  }
  (function wireNumberUi() {
    var cb = $('nestNumber'), md = $('nestNumberMode');
    function refresh() { numRefreshLabels(); renderPieceQty(); }
    if (cb) cb.addEventListener('change', refresh);
    if (md) md.addEventListener('change', refresh);
  })();
  window.__mesCutNumber = { attach: attachNumberTabs, normalize: numNormalize, resultLine: numResultLine };
  ...
  function loadPieceQty() {
    ...
    host('mesCut_nestBegin()', function (bg, bad) {
      ...
      host('mesCut_nestSizes()', function (sz, bad2) {
        ...
        // ★조각 번호 — 경계를 같이 읽어 목록 행에 번호를 붙인다(호스트 구버전이면 null → 번호 없이 종전 화면)
        numFetchBounds(function (bounds) {
          if (pieceQty) { pieceQty.bounds = bounds; numRefreshLabels(); }
          renderPieceQty();
          out('조각 ' + list.length + '개 — 수량을 넣고 [네스팅 실행]을 누르세요.'
            + ((pieceQty && pieceQty.numWhy) ? ('\n조각 번호: ' + pieceQty.numWhy + (numOn() ? '' : ' (꺼져 있음 — 켜면 고객용 번호 이미지가 함께 나갑니다)')) : ''), 'ok');
        });
      });
    });
  }
  ...
  var sugProduct = attachSug('regProduct', 'productList', function (v) {
    return v ? (productIdOf(v) ? { ok: true, text: '✓등록' } : { ok: false, text: '미등록' }) : { ok: false, text: '' };
  });
  // 자재·후가공은 id 를 해소하지 않는다(이름 그대로 파일명·주문에 실린다) → 목록에 있는지만 알린다.
  function inDatalist(dlId, v) {
    var dl = document.getElementById(dlId);
    if (!dl || !v) return false;
    var op = dl.getElementsByTagName('option'), q = cutSquash(v);
