...
(function () {
  ...
  var SHELL_VERSION = '0.101.0';   // 0.101.0 = ★테두리 건너뛰기 모서리(2026-09-26 실기) — 사각 모서리 블록이 대각선으로 들어가 테두리색 덩어리가 남던 것을 축별 2mm 로(bleed.js) · 사각 빠른 경로는 건너뛰기면 전체 도련으로(띠는 건너뛰기를 못 지킨다). 잃는 것 = 건너뛰기+빠른 경로 조합의 띠 속도.
  ...
  function startNumberArt(prep, res, info, mmpp, half) {
    ...
    for (var s = 0; s < res.sheets.length; s++) {
      ...
      for (var k = 0; k < pls.length; k++) {
        ...
        arr.push({ n: info.labels[pl.id] || '', w: Math.round(toRealMm(sz.w)), h: Math.round(toRealMm(sz.h)),
