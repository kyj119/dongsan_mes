    ...
    function withList(list) {
      ...
      (function step() {
        ...
        readPng(it.path, function (err, img) {
          ...
          try { res = B.repeatLastPixel(src, gpx, sq ? { corner: 'square', srcInsetPx: 0 } : undefined); } catch (e) { res = null; }
  ...
  function runNest() {
    ...
    function go() {
      ...
      nestPrepare(G, rez, gapMm, offsetMm, fillMode, function (m) { done(m, 'err'); }, function (prep) {
        ...
        if (wantBleedPng && prep.rect) buildStripBleeds(prep, growMm, afterBleed);
