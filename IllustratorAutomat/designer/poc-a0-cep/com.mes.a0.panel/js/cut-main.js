...
(function () {
  ...
  var SHELL_VERSION = '0.61.0';   // 0.61.0 = 배율 확대 결과 보고(PDF 배치 / 예비 경로 폴백 경고) · 0.60.0 = 파일명 맨 앞 거래처 · 자재/후가공 행 분리(폭 맞춤) · 「품목」→「내용」 명칭 분리(MES 품목 마스터와 구분) · 0.59.0 = 재단 탭 [◎ 전체] · 0.58.0 = 굽기 통합(마스크+도련 1왕복)·등록 파라미터(자재·후가공·돔보·파일명) · 0.57.0 = 조각 속 메우기(그룹 하나=칼선 하나·맞붙임 복구) · 0.56.0 = 도련 겹침 분할(간격 존중·하한 1.5mm)
          ...
          host('mesCut_nestApply(' + offsetMm + ',' + (prep.fill ? 'true' : 'false') + ',' + effBleedMm + ',"' + nestBleedMode + '"' + (useVec ? '' : ',"raster"') + ')', function (ap, bad3) {
            ...
            setNestReady(true);
              ...
              + (effBleedMm > 0 ? ('\n도련 ' + R(effBleedMm) + 'mm (조각마다)'
                  ...
                  // ★배율 확대를 PDF 배치로 처리했는가 — 폴백했다면 **배경이 깨졌을 수 있다**.
                  //   resize 는 불투명도 마스크를 안 데려가 배경이 통째로 사라진다(2026-08-28 약국.ai 실측).
                  + (parseInt(a.placefail, 10) > 0
                    ? ('\n⚠ 배율 확대 ' + a.placefail + '개 조각이 예비 경로로 처리됐습니다 — '
                       + '배경이 사라졌을 수 있으니 판을 눈으로 확인하세요. '
                       + '안 깨지게 하려면 파일을 실물 크기로 두고 배율 1/1 로 짜세요.') : '')
                  + (parseInt(a.placed, 10) > 0
                    ? ('\n※ 배율 확대 ' + a.placed + '개 조각 — 아트를 직접 키우지 않고 PDF 로 굳혀 배치했습니다(마스크 보존).') : '')
